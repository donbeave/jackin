...
- **Usage surfaces = limits only (never token price or historical trend).** Capsule usage dialog, `jackin-usage` / `jackin-usage-ffi`, and **jackin❯ desktop** (native menu bar + glance + Usage window) show **subscription / quota usage limits only**: remaining/used %, reset countdowns, plan/status, and provider-supplied **limit windows** (including money *caps* when the provider exposes them as a quota bound). **Never** ship token unit prices ($/token, $/MTok, cost-of-session estimates), spend-over-time charts, usage-trend sparklines, Today/Yesterday/30-day token/spend histories, aggregate-spend donuts, or cost-legend rankings. OpenUsage/CodexBar may show those — we do **not** copy them. → crate rules under `crates/jackin-usage/`, `crates/jackin-usage-ffi/`, and `native/`.
...
- Telemetry is registry-first; emit only through the governed event, operation, metric, or spawn facade → [ENGINEERING.md](ENGINEERING.md).
