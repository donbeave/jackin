...
- `docs/content/reference/tui/` pages (`index.mdx`, `chrome.mdx`, `navigation.mdx`, likely more) encode termrock API specifics (`keymap::glyph`, `HintSpan`, `StatusBar` slot policy, `DetailTable`); repository law requires same-PR docs updates for TUI changes.
...
- Breaking renames that hit jackin❯ directly: crate-root re-exports purged (`termrock::Theme` — 314 uses — now `style::RolePalette`); `style::DesignTokens` → `DesignSystem`; `widgets::PanelEmphasis` → `PanelChrome` (8 uses); `interaction::FocusRing`/`FocusTarget`/`FocusOutcome` → `InteractionScene`/`FocusGraph`/`SemanticScene`; `interaction::ModalStack` → `OverlayStack`; `style::Role` tab-underline variants removed (~30 new variants added); `runtime::RunOptions` gained `synchronized_output` field.
...
- Does the brand-look invariant bind already in the bump PR? The upstream theme swap (`Theme::default` → `RolePalette::tailrocks_phosphor()`) may shift brand-composition colors even with brand code untouched. Recommendation: yes — the invariant binds from the bump PR onward, and the bump PR compensates in consumer code if the theme swap shifts brand rendering.

## Open research questions

- Full per-crate compile-break inventory: run `cargo check` across the six consuming crates against rev `e1d61f4d` and map each break to its upstream `migrations/00NN` doc. (High-level delta already surveyed 2026-08-19 — see References; this is the exhaustive pass for planning.)
