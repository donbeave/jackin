...
- Should the Codex adapter extract the `individual_limit` / spend-control monthly credit-pool lane from the existing `/wham/usage` response as a money-cap window (reference: CodexBar)?
- Should window classification move from response slot names to window-duration inference wherever the provider supplies durations (Codex, Z.AI), as OpenUsage does?
- Should Grok probing prefer the `cli-chat-proxy.grok.com` REST billing endpoints over the ACP stdio RPC and grpc-web protobuf wire-scan?
- How does the crate close the OpenCode adapter gap for the settled eight-surface host inventory (OpenUsage's `zen/go/v1/usage` Bearer API is the demonstrated source)?
- Should the broker adopt interaction- and power-aware adaptive refresh cadence (CodexBar's 2/5/15/30-minute ladder plus refresh-on-menu-open) instead of a single fixed floor?
- Should the projection add a limits-only pace exhaustion estimate ("runs out in N d") beside `pace_label`, and a "Not started" state for untouched short windows?
- Is browser-cookie scraping ever an acceptable credential lane (Codex code-review quota, richer Claude web windows), or does the CLI-file/Keychain/env boundary hold?

## Deferred

- Provider catalog expansion beyond the settled eight host / seven desktop surfaces (CodexBar ships ~69 providers — Cursor, Copilot, Gemini, Antigravity, Devin, Windsurf, Mistral, DeepSeek, OpenRouter, and others).
- Provider status-incident badges from public status pages (CodexBar polls Statuspage.io for OpenAI and Anthropic).
- Per-credit Codex reset-credit expiry timelines and expiry notifications (the crate already surfaces count and next expiry).
- Codex code-review quota window, reachable only through a web-dashboard cookie scrape.
- Every cost/spend/trend surface both reference apps ship (local log scans, admin cost APIs, spend dashboards, 30-day trends, aggregate-spend rings) — permanently excluded by the limits-only Must-not, listed here so the exclusion is deliberate rather than unconsidered.
