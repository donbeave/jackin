...
An `--on-demand` value is left out of the launch environment entirely. Only its
*name* reaches the container; the agent asks for the value through
`jackin-exec`, and the host resolves it after the operator approves that use.
Use it for credentials that must not sit in a long-lived container's
environment.
