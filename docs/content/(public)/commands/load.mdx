...
The plan is resolved, not guessed: `--dry-run` fetches the role repository and
reads its manifest, so the JSON envelope's `data` carries `published_image`
(the manifest's warm base, or `null`) alongside `image_decision` — the same
reuse-vs-build decision the launch itself would take, with `decision`,
`reason`, `image`, `base_image`, and `role_git_sha`.
| `--debug` | Enable operator troubleshooting output while keeping the launch surface clean. It does not create a telemetry file. |
...
Failures in the launch surface show a short diagnosis, the failed stage, and the invocation id for backend correlation. Raw Docker output, stack traces, and debug detail are not rendered in the failure popup. The live Docker build overlay is bounded in memory, uses a black terminal-style surface, and interprets ANSI colour from Docker output when present. No telemetry timeline or command-output sidecar is written locally.
