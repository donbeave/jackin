...
[Token & Cost Telemetry](/research/agents/telemetry/token-cost-telemetry/), not this
control surface.
...
Modern terminals interpret OSC escape sequences agents emit for window titles, clipboard writes, desktop notifications, and hyperlinks. jackin-capsule denies OSC 52 clipboard writes from the focused pane by default because they mutate the operator's host clipboard. Set `JACKIN_OSC52=allow` before `jackin load` only when a role is expected to copy from the agent pane into the host clipboard.

The lower-risk title, notification, and hyperlink families still default to passthrough so the agent's UI works the same as it would running directly on the host. Operators running an untrusted role can disable those families per-container with these environment variables (set on the host before `jackin load` so they reach the derived image's runtime):

| Env var | Behavior |
|---|---|
| `JACKIN_OSC52=allow` | Enables OSC 52 clipboard writes (agent → host clipboard); all other values keep them denied |
| `JACKIN_OSC_TITLE=deny` / `off` / `no` | Disables OSC 0 / 1 / 2 window-title and icon-name updates |
| `JACKIN_OSC_NOTIFY=deny` / `off` / `no` | Disables OSC 9 desktop notifications |
| `JACKIN_OSC_HYPERLINK=deny` / `off` / `no` | Disables OSC 8 clickable hyperlinks |
...
- `pointer` over clickable chrome owned by jackin❯, such as tabs, the top menu button, the bottom-row GitHub and Debug info entry points, and clickable dialog values such as Container ID, Invocation ID, and GitHub URL copy targets.
...
Use the `build-jackin-capsule` binary (defined in <RepoFile path="crates/jackin/src/bin/build_jackin_capsule/main.rs" />) to compile `jackin-capsule` for Linux and populate the cache. It uses `cargo-zigbuild` with Zig as the linker — no Docker required — and targets a glibc 2.17 floor to match CI:
...
**Debug builds for crash triage.** When a container crashes with an exit-101 panic and `docker logs` shows `<unknown>` frames, use the canonical [Debugging Capsule Crashes](/reference/capsule/session-lifecycle/#debugging-capsule-crashes-symbolicated-build) flow. That page owns the symbolicated-build recipe, `RUST_BACKTRACE=full` launch, and explicit log-inspection steps.
...
| <RepoFile path="crates/jackin/src/bin/build_jackin_capsule/main.rs" /> | Developer tool: `cargo run --bin build-jackin-capsule` — builds via `cargo-zigbuild` and populates the cache |
...
- [Agent Orchestration Program](/research/agents/orchestration/program-research/) — comparative analysis of Herdr and other tools that informed the design
