...
description: "jackin❯ desktop reuses Rust usage projections through UniFFI and limits Swift to native macOS presentation."
---
...
jackin❯ desktop is a host process that displays account quota limits without a running Capsule. Rust owns provider probing, credential resolution, account selection, host-wide snapshot coordination, cooldowns, refresh policy, quota semantics, severity, provider ordering, and every displayed domain string. Swift owns AppKit/SwiftUI presentation, accessibility, lifecycle, settings, signing, and other macOS integration.

The boundary is a synchronous coarse-grained UniFFI facade compiled as a static library and assembled into an arm64 XCFramework. Swift polls immutable DTO snapshots through an off-main-actor scheduler; it never implements HTTP/RPC/OAuth/CLI probes or a second provider matrix.
...
| Provider glance | transient `NSPopover` hosting SwiftUI `Form` | native host; focused provider content |
| Usage shell | retained `NSWindow`, unified `NSToolbar`, `NavigationSplitView` | native functional hierarchy |
| Usage sidebar | sidebar `List` plus noninteractive generated wordmark | native navigation plus identity |
...
jackin❯ phosphor is an adaptive identity/healthy-state token. Warning and danger retain text plus system semantic color. Provider marks come from the canonical generated SVG assets. The quiet sidebar wordmark is `jackin❯ by tailrocks`; it is hidden from accessibility because it is not an action.

## Interaction architecture

`StatusBarController` reconciles native status items from the Rust projection. A primary click opens one real transient `NSPopover` focused on that provider. The popover has no cross-provider navigation. It shows provider identity, native account selection, Rust-owned detail/limit rows, visible recovery, Refresh, and Open Usage. A secondary click opens the fixed AppKit menu: Open Usage Window, Refresh, Quit jackin❯ desktop.

Cross-provider navigation belongs in the Usage window. `UsageWindowController` lazily creates and retains one `NSWindow`. `UsageWindowRoot` uses a two-column `NavigationSplitView`: Overview and Rust-ordered providers in the sidebar, native `Table` Overview, and native provider detail. It removes the framework's automatic sidebar toolbar item and installs exactly one `.navigation` toolbar button so the control remains visible and stationary in expanded/collapsed states. The window hides its redundant `Usage` title.
...
Loading, empty, refreshing, fresh, stale-last-good, depleted-with-reset, needs-login, unsupported, provider failure, and global bridge failure remain separate states. Refresh failures preserve last-good data. Global and provider failures expose native Retry actions.

## Credential and snapshot ownership

Host credentials are resolved by Rust, preferring normal host roots and using container handoff paths only as a last resort. Important roots include Claude Keychain/file credentials, Codex `~/.codex/auth.json`, provider home sessions, and Z.AI/MiniMax environment keys.

Host processes and containers coordinate through `~/.jackin/data/usage-shared/{snapshots,cooldowns,locks}`; containers mount the same tree under `/jackin/usage-shared`. At most one probe per account owns a refresh. Durable menu-bar account/snapshot state lives under `~/.jackin/data/usage-menu-bar/`.
...
Synthetic F00–F14 visual-QA fixtures activate only through explicit launch arguments and never call the bridge, network, or real credentials. They drive the same production AppKit/SwiftUI hosts as live data.

Architecture tests enforce no custom glass/material, no Swift-composed domain strings, no hard-coded provider display names outside fixtures, and no probe imports. Real-host UI tests verify native commands, focused-popover routing, fixed sidebar position, retained window context, minimum-size scrolling, visible recovery, and accessibility audits for popover, Overview, and provider detail.
