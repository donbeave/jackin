...
| Terminal / run loop | `tui/run.rs`, `tui/terminal.rs` | Terminal lifecycle, backend adapter, event-loop policy, and routing of completed effects back into messages |
...
**Component ownership.** Product-neutral visual and interaction patterns live in [TermRock](https://github.com/tailrocks/termrock), including the first implementation of a pattern when it is reasonably reusable. Cross-surface jackin❯ compositions live in `jackin-tui` only when at least two product surfaces consume them. Surface-local composition lives under the owning crate's `src/tui/components/`. Neither jackin❯ layer copies neutral painting, geometry, focus, scroll, or hit-testing. TermRock owns its lookbook, generated SVG catalog, API/catalog drift checks, and exact primitive conformance tests; jackin❯ owns only product composition, state, adapter, and outcome tests.
...
**Rule: TUI code has exactly three ownership layers:**
...
2. **`crates/jackin-tui/`** — Product-specific compositions and Ratatui adapters shared by at least two jackin❯ surfaces. After the console modernization it carries three modules: brand `tokens.rs`, the `operator_info` Debug-info fact vocabulary, and `runtime` (`View` / `drive_frame` / `drive_render`, `SurfaceFocus`, subscription/update contracts) retained for the capsule and launch surfaces until their own modernization phases. It cannot own a generic theme facade, neutral widgets, terminal lifecycle, surface event loops, schedulers, or external effects.

3. **`crates/<surface-crate>/src/tui.rs` + `crates/<surface-crate>/src/tui/`** — The whole reusable TUI application layer for that surface: Model, Message, Update, Effects, Subscriptions, View, local components, terminal adapter, and event-loop policy. Single-screen crates stay flat. Multi-screen crates add `screens.rs` plus `screens/<screen>/` under `src/tui/`.

The top-level `jackin` binary has one composition-root exception: the host
console's concrete runtime, Docker, config, and terminal bindings live under
`crates/jackin/src/console/adapter/`. The adapter may execute effects described
by `jackin-console`, but it must not define widgets, neutral component behavior,
or a parallel TUI model/update/view. Calling this boundary `adapter` is
mandatory; a second root `console/tui/` tree would falsely claim presentation
ownership and is rejected by the architecture gate.

`jackin-core` remains terminal-library-free and owns domain facts, never product palettes, Ratatui, Crossterm, or TermRock types. Renderer-neutral identity tokens live in `jackin-brand`; `jackin-tui` adapts the TUI subset to Ratatui.
...
└── tui/
    ...
    ├── daemon/          ← capsule-only TUI adapters over daemon-owned session state
    ├── components.rs    ← thin local-component module root
    └── components/
```

The capsule keeps PTY/session authority and effectful daemon ports under
`src/daemon/`. Its `src/tui/daemon/` bridge contains only terminal-facing
`Multiplexer` implementations: composition, dialog/input dispatch, mouse hit
translation, and pane layout. `daemon.rs` declares those bridge modules with
explicit paths so the model remains daemon-owned while every visual or
terminal-interaction implementation follows the same `src/tui/` location rule.
...
- Event-loop policy and terminal lifecycle/backend adapters; a composition-root
  binding may live under `console/adapter/` only when it must cross from the
  surface crate into higher-tier runtime dependencies
...
surface tui/run.rs (event-loop policy)
  -> composition-root adapter when higher-tier runtime binding is required
