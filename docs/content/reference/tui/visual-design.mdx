...
Reusable component styling flows through TermRock's semantic `DesignSystem` and `Role` API ([TermRock source](https://github.com/tailrocks/termrock)). jackin❯ surfaces resolve neutral presentation with `DesignSystem::default().style(Role::…)` so ordinary, strong, muted, border, focus, danger, warning, tab, input, link, and scroll colors stay on TermRock's default phosphor preset. Product-owned brand/domain colors (brand pill, menu chips, status accents) live in `jackin_tui::tokens`.
...
|---|---|---|
| TermRock `Role` | `Text`, `TextStrong`, `TextMuted`, `Border`, `BorderFocused`, `Accent`, `Danger`, `Warning`, `Tab*`, `Link*`, `Scroll*` | Shared presentation via `DesignSystem::default().style(role)` |
...
- **Active interaction container**: `PHOSPHOR_GREEN` border — dialogs, pickers, forms, tab panels, and the currently active list/sidebar use the bright border whenever they own keyboard or scroll interaction. The paint path maps `focused = true` → `PanelChrome::Focused` with no overflow gate.
- **Passive list-detail scroll block that still owns focus**: `PHOSPHOR_GREEN` border — state keeps `focused = true` only while content overflows (see focusability / `list_pre_render_focus_plan`). The renderer does not second-guess `focused`.
...
TermRock `Viewport` paints border emphasis from `PanelChrome`. Console scrollable panels go through `scroll_block::render_scrollable_block_at`, which maps the caller's `focused` flag to `PanelChrome::Focused` / `Normal`. Passive list-detail surfaces clear focus state when content no longer overflows so a non-scrollable detail block does not keep a false green affordance. Active interaction containers (forms, dialogs, pickers) keep the bright border whenever they own interaction.
...
Any value the operator can click to copy (an invocation ID, container ID, file path, or URL) must read as a link and behave identically on every surface, following the W3C native-link convention:
