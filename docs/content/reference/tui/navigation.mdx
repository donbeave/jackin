...
The workspace list opens the host-wide Usage route with `u`. Usage keeps the
shipped `jackin❯ · usage` header, body-only 30/70 master/detail split, and
footer hints. The left pane contains one selectable row per canonical account;
multi-account providers are group headers, not selectable provider rows. The
right pane shows the selected account and Rust-owned limit windows. `↑`/`↓` (or
`j`/`k`) moves accounts, `Enter` toggles the detail presentation, `PageUp` /
`PageDown` scrolls, and `Esc` / `q` returns to the workspace list. Human CLI
output remains intentionally simpler; this route is the rich terminal view.
...
1. The console's tabbed screens drive TermRock's `FocusGraph` directly through the product `ConsoleFocus` wrapper (<RepoFile path="crates/jackin-console/src/tui/focus.rs">crates/jackin-console/src/tui/focus.rs</RepoFile>). It answers which region owns input: the tab bar, or one named content block. Borders, tab underlines, cursors, and footer scope derive from that one owner value. (The capsule still uses the `jackin_tui::runtime::SurfaceFocus<Target>` projection over `FocusGraph`; it retires with the capsule's own modernization phase.)
2. A per-dialog semantic enum is element-level focus inside a button dialog. Use named states such as `Yes` / `No` or `Save` / `Discard` / `Cancel`, and project them through the shared TermRock focus mechanics; do not store a parallel raw index or bool.
3. `PanelChrome` is border styling only. It is computed from screen-level or dialog-level focus state when rendering; do not store it as independent state.
...
Shared keys use `termrock::keymap::glyph` constants. A hint PR introducing `Tab`, `↑/↓`, `←/→`, `PgUp PgDn`, or `+`-joined modifiers is a design violation.
...
- Every console stage footer ends with `? help`: `?` opens the keyboard-shortcut overlay (TermRock `keyboard_help`, entries sourced from the live keymaps via `help_entries_from_keymap`), and `Esc` dismisses it with focus restored. Modal and inline-picker footers deliberately omit the hint — an open modal owns `?` as typed input, so the overlay is unreachable there.
...
Enforcement:
- Every root-console modal widget such as `crates/jackin-console/src/tui/op_picker/` must expose a `footer_items` function (or equivalent) and remove its internal hint row.
...
When a rich alternate-screen surface owns the terminal — the launch/loading cockpit, the workspace console, the in-container multiplexer, or the PR-verify surface — `--debug` must not print over it. Operator notices are deferred until terminal ownership ends, while governed telemetry exports directly to the configured backend. No local telemetry file is created.

The mechanism is the `rich_surface_active` flag plus the active diagnostics run: `emit_debug_line` / `active_debug` (<RepoFile path="crates/jackin/src/console/adapter.rs">crates/jackin/src/console/adapter.rs</RepoFile>, <RepoFile path="crates/jackin-diagnostics/src/run.rs">crates/jackin-diagnostics/src/run.rs</RepoFile>) route to the run file, and the command runner (<RepoFile path="crates/jackin-docker/src/shell_runner.rs">crates/jackin-docker/src/shell_runner.rs</RepoFile>) suppresses live streaming whenever `--debug` is on or a rich surface is active. Streaming child output straight to `stdout`/`stderr` while a rich surface is up is a bug; route it through the diagnostics run instead.

The launch UI may show the current `cli.invocation.id` as a copyable backend correlation handle. It never shows a reveal action for a nonexistent diagnostics path. Keep detailed evidence out of the alternate screen: add governed telemetry for registered semantic data or use the explicit operator-output port, then defer that output while a rich surface owns the terminal.
...
TermRock `Viewport` (console `scroll_block` adapter, with scroll offsets held in TermRock `ScrollAreaState`) paints `PanelChrome` from the caller's `focused` flag with **no overflow gate**. Caller `focused` means **interaction ownership** — forms, tab content, and other containers that accept keys stay green even when content fits. The passive-scroll focusability rule is enforced in state/input: list-detail surfaces clear `list_scroll_focus` when content no longer overflows (`list_pre_render_focus_plan`, fed the `focused_block_scrollable` fact derived from each block's `ScrollAreaState`), so a passive detail block does not remain the focus owner without overflow.

This means the rule is **impossible to accidentally violate** through the shared renderer. What callers must still ensure:

1. **Mouse click handler (`update_scroll_focus`)** — set the `focused` / `scroll_focused` state field for any block whose hit-test rect contains the click, **regardless of whether the block is currently scrollable**. Pre-render focus planning (`list_pre_render_focus_plan`, reading each block's overflow from its `ScrollAreaState`) clears passive list-detail focus when content no longer overflows, so the green border stays honest without the paint path second-guessing `focused`. Do NOT add `is_scrollable` guards in the click handler — that causes a different bug where clicking a non-scrollable block silently clears focus from the previously focused block without giving it to the clicked one.

2. **Resize guard** — when the terminal shrinks and content that previously overflowed now fits, `list_scroll_focus` (list view) is cleared by the same pre-render focus plan once the block's `ScrollAreaState` reports no overflow. Similar guards must exist for any persistent focus state that a terminal resize can invalidate.

Both checks must use the **same content height and width values** as TermRock `Viewport` / console `scroll_block` uses for the actual rendered lines. Any mismatch causes the bug where the input handler clamps scroll to the wrong maximum — the scrollbar thumb stops short of the track end even though the operator has not reached the actual content limit.

**Derive content extents from the rendered lines, never from a parallel column-sum.** A block's `content_width` must use TermRock's rendered-line width helpers over the exact `Vec<Line>` the block renders, and its `content_height` must be that line count. Build the lines once in a pure function and feed the same function to rendering and scroll clamping. A parallel column sum drifts from Unicode display width and trailing padding.

### Click hit-test area must cover the full rendered block

**The mouse hit-test rect for a scrollable block must exactly match (or be a superset of) the `area` passed to TermRock `Viewport` / console `scroll_block`.** Never compute the hit-test area from content dimensions or row counts — use the full rendered block height.
...
**Implementation rule:** Every `point_in(mouse, area)` check in `update_scroll_focus` must use the same `area` that the renderer passes to TermRock `Viewport` / console `scroll_block`. In practice this means:
...
**Enforcement:** TermRock `Viewport` paints the vertical overflow bar; surfaces that own independent horizontal scroll must paint the bottom track through TermRock `render_scrollbar` + `ScrollbarSpec` (or an equivalent TermRock helper). Any surface that tracks `scroll_x` independently must use the same overflow math as the painted content.
...
- `scroll_hint_spans(ScrollAxes { vertical, horizontal })` in [TermRock source](https://github.com/tailrocks/termrock) is the single source for the scroll-key `HintSpan`s. It returns `↑↓←→`/`↑↓`/`←→`/empty. Every surface composes its full hint from this primitive plus its own dismiss/copy keys; no surface hand-writes `↑↓←→ scroll`.
...
A screen's vertical and horizontal scrollbars must look like the same control at two angles, never two different controls. The weight is governed by `ScrollbarStyle` ([TermRock source](https://github.com/tailrocks/termrock)), and both axes of one style render at the same weight — only the orientation glyph rotates:
...
These are the exact formulas that match what each render function passes to TermRock `Viewport` / console `scroll_block`:
...
**All bordered scrollable content blocks must use TermRock `Viewport` / console `scroll_block`** (defined in [TermRock source](https://github.com/tailrocks/termrock), re-exported through `view.rs` for manager-internal callers). Never hand-roll block creation, Paragraph scroll, or scrollbar rendering inline.

TermRock `Viewport` (console call sites go through `scroll_block::render_scrollable_block_at`) guarantees:
- Border role `BorderFocused` (phosphor green) when `focused` is true (interaction owner); otherwise `Border` (neutral gray).
- Vertical scrollbar rendered only when content overflows vertically.
- Horizontal overflow is scrolled through `DialogScroll` offsets; paint horizontal tracks with TermRock `render_scrollbar` when a surface needs an explicit bottom bar.
- Scroll offsets are clamped to valid range every frame (eliminates stale overshoot).
- Scrollbar thumbs are proportional to visible content, so small overflow produces a long thumb and large overflow produces a short thumb, while a block scrolled to its maximum offset still shows the thumb at the visible end of the track.
...
Input hit-testing, focusability, cursor-follow scrolling, selected-list windowing, scrollbar dragging, wheel/key max-offset math, content width measurement, and scrollbar rendering must flow through `termrock::scroll` and canonical widget state. Those modules are the source of truth for clamping, top-relative offsets, tail-relative offsets, full-cell thumb conversion, and scrollbar metrics. Do not reimplement viewport, thumb, track, or offset formulas in product input handlers or renderers.

The shared model does **not** make every scrollbar look the same. jackin❯ owns product composition: capsule panes paint only thumb cells into the right border column via `render_scrollbar` + `ScrollbarSpec`, while console detail panels use `Viewport` chrome. Do not invent a second neutral scrollbar renderer in product code.

Console call shape (adapter around TermRock `Viewport`):
```rust
...
Pass `None` for `title` to suppress the block title. Pass a bare trimmed title (no padding); TermRock `Viewport` / `Panel` normalize title spacing.
...
**Inline picker focus rule:** An inline picker rendered inside a parent container must use the parent's focus state for its panel border. Use `focused = parent.list_names_focused` (or equivalent), never hardcoded `PanelChrome::Normal` when the parent owns interaction. The `▸` cursor inside an inline picker follows the same gating: only visible when the parent container is focused. Focus on a container must persist as long as the operator is interacting within that container. Navigation keys (`↑`/`↓`/`←`/`→`) executed inside a focused container must never transfer or clear focus — they navigate within the current owner. Only an explicit user action (mouse click on a different area, `Tab` to another area, `Esc` to a parent screen) may move focus.
...
1. Content no longer overflows after resize — the pre-render focus plan reads `focused_block_scrollable = false` (derived from the block's `ScrollAreaState`) and clears it.
...
The left pane scroll uses the shared `termrock::scroll` clamping path after each wheel/key mutation, then the renderer clamps defensively again on every frame.
