...
| Keyboard help overlay | [TermRock source](https://github.com/tailrocks/termrock) | Searchable keyboard-shortcut overlay (`KeyboardHelpState` + `KeyboardHelp`) whose entries derive from the live keymaps via `help_entries_from_keymap`; opened by `?` from every console stage, dismissed by `Esc` with focus restored, discovered through the `? help` footer hint | The console's shortcut reference — the one overlay every stage shares |
...
The root console run loop that drives the wizard lives under <RepoFile path="crates/jackin/src/console/adapter/run.rs">crates/jackin/src/console/adapter/run.rs</RepoFile>; it suspends and resumes the terminal while external minting and write validation run through non-TUI services.
