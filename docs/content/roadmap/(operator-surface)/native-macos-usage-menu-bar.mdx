...
- **2026-08-13 — Center product identity.** The operator required `jackin❯ desktop` centered in the Usage header and jackin❯ identity in the popover. Native principal placement and a noninteractive monogram/text row satisfy this without custom chrome or glass.
...
- Centered, noninteractive generated monogram plus `jackin❯ desktop` popover identity; native account menu, Rust-owned detail/limit rows, visible Retry/Refresh/Open Usage actions, and no cross-provider navigation.
