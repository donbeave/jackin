...
- Native account menu, Rust-owned detail/limit rows, visible Retry/Refresh/Open Usage actions, and no cross-provider popover navigation.
- Retained native Usage `NSWindow` with `NavigationSplitView`, Overview table, provider detail list, native toolbar/menu commands, hidden redundant title, and stable sidebar-button coordinates.
