...
**Current state**: The in-repository A1 native redesign is implemented for the macOS 26 production baseline. It uses provider-focused native popovers with centered product identity, a two-column Usage window with centered `jackin❯ desktop` toolbar identity, a fixed leading sidebar control, the generated `jackin❯ by tailrocks` sidebar signature, visible native recovery, retained window context, and system-owned Liquid Glass only. Rust remains authoritative for providers, accounts, quota semantics, refresh policy, and domain strings.
...
- **2026-08-12 — Select A1.** The operator selected a focused provider popover plus two-column Usage. Cross-provider comparison belongs in Usage, not a custom tab system inside the popover.
...
- **2026-08-12 — Confirm runnable A1 for production.** The operator confirmed the native proof. Production implementation preserves A1 hierarchy, system-owned material, Rust/Swift ownership, and the two identity/sidebar decisions.
- **2026-08-13 — Center product identity.** The operator required `jackin❯ desktop` centered in the Usage header and jackin❯ identity in the popover. Native principal placement and a non-interactive monogram/text row satisfy this without custom chrome or glass.
...
- Centered, non-interactive generated monogram plus `jackin❯ desktop` popover identity; native account menu, Rust-owned detail/limit rows, visible Retry/Refresh/Open Usage actions, and no cross-provider navigation.
- Retained native Usage `NSWindow` with `NavigationSplitView`, Overview table, provider detail list, centered native principal product title, native toolbar/menu commands, no `Usage` title, and stable sidebar-button coordinates.
...
- Real-host UI coverage for commands, routing, scrolling, recovery, retained context, fixed sidebar control, and accessibility audits of popover, Overview, and provider detail.
...
These remain separate product decisions, not unfinished A1 work:
...
Do not retire this roadmap item until public Developer ID/cask proof and the capture-gated provider requirement have authoritative evidence. The native A1 implementation may be complete inside the repository while the broader delivery item remains **Partially implemented**.

## Related work

- [Auth health and operator visibility](/roadmap/auth-health-and-visibility/)
- [jackin❯ daemon](/roadmap/jackin-daemon/) — optional later focus support, not an A1 dependency
