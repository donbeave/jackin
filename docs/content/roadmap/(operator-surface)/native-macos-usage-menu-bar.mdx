...
- Centered, non-interactive generated monogram plus `jackin❯ desktop` popover identity; provider and selected account, one Rust-owned activity phrase, native account menu, Limits before useful Details without repetition, visible Retry/Refresh/Open Usage actions, and no cross-provider navigation.
