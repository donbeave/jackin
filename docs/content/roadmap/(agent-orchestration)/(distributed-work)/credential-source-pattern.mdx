...
7. New docs page `docs/content/(public)/guides/credentials.mdx`: shipped baseline, target backends, delivery modes, security tradeoffs, recommended pattern per use case.
...
- Research: [Credential Source Pattern — Design](/research/platform/security/credential-exposure/credential-source-pattern/) — multicode prior art, target TOML shape, resolution/forwarding model, open design questions.
- [1Password integration](/roadmap/onepassword-integration/) — existing consumer; refactors onto the new type.
- [Claude auth strategy](/research/agents/authentication/claude-auth-strategy/) — existing consumer; this pattern should be the abstraction it uses, not a parallel design.
