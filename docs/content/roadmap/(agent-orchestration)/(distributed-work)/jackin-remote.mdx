...
**Current state**: Phase 5 design proposal; no `jackin-remote` binary, `--remote` global flag, remote host config, SSH bridge, rsync orchestration, or remote handler relay exists yet (Phase 5, [Agent Orchestration Program](/research/agents/orchestration/program-research/))
...
1. **Remote binary/subcommand and control channel.** No `jackin-remote` binary or `jackin --remote` command exists in <RepoFile path="crates/jackin/src/cli.rs">crates/jackin/src/cli.rs</RepoFile> today. V1 should use an SSH-based control channel plus rsync-based state sync rather than a custom protocol; see the [design research](/research/agents/orchestration/remote-execution/jackin-remote-design/) for the recommended shape.
...
- Research: [jackin-remote design research](/research/agents/orchestration/remote-execution/jackin-remote-design/) — multicode inspiration, config/CLI sketches, handler relay design, and open design questions.
