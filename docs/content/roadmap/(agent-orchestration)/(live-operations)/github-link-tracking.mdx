...
1. **Fetcher + cache.** Add an Octocrab-based fetcher with a per-state refresh cadence and a per-instance SQLite cache (depends on the not-yet-landed [persistent storage layer](/research/agents/orchestration/memory/)).
...
- [Persistent storage layer](/research/agents/orchestration/memory/) — cache home, not yet built.
