...
**Current state**: no idle-cleanup mechanism exists yet; only the underlying signals it would build on (clean-exit teardown, agent status reporting) are shipped (Phase 4, [Agent Orchestration Program](/research/agents/orchestration/program-research/))
...
- Research: [Idle runtime cleanup: design notes](/research/agents/orchestration/idle-runtime-cleanup-design/) — multicode precedent, recommended config shape, open questions
