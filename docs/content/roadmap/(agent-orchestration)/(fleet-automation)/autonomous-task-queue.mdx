...
**Current state**: no code exists yet; jackin❯ has reusable launch/isolation/status/telemetry primitives but no queue, task source, or dispatcher (Phase 4, [Agent Orchestration Program](/research/agents/orchestration/program-research/))
...
2. Durable task table and idempotent source polling integration, once the [Task source abstraction](/roadmap/task-source-abstraction/) and [Persistent storage layer](/research/agents/orchestration/memory/persistent-storage-layer/) land.
...
9. Budget gating (`budget_usd_per_day`), blocked on the host-global per-instance usage-storage substrate — see [Token & Cost Telemetry](/research/agents/telemetry/token-cost-telemetry/).

Full design rationale, config sketch, prior-art comparison (multicode), and open questions live in [Autonomous Task Queue: Design Exploration](/research/agents/orchestration/fleet-operations/autonomous-task-queue-design/).
...
- [Autonomous Task Queue: Design Exploration](/research/agents/orchestration/fleet-operations/autonomous-task-queue-design/) — problem framing, multicode comparison, recommended shape, open questions.
...
- [Persistent storage layer](/research/agents/orchestration/memory/persistent-storage-layer/)
...
- <RepoFile path="crates/jackin-usage/src/usage_snapshot_store.rs">crates/jackin-usage/src/usage_snapshot_store.rs</RepoFile> — shipped Capsule-local account quota store; not enough for queue budget gates.
