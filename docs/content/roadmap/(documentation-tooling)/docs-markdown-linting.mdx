...
**Current state**: adoption plan drafted, no implementation committed; no rumdl config, CI step, or `check:md` script exists in the repo yet
...
Research: [Docs markdown linting: rumdl adoption rationale](/research/engineering/documentation/docs-markdown-linting/) covers why rumdl over `markdownlint-cli2`/`remark-lint`/`mado`, and the trade-offs of adopting it.
...
3. **Cleanup pass.** Run `rumdl check --fix` against `docs/content/`, hand-fix what `--fix` cannot resolve, and land the cleanup, the config, and the CI step together — a half-adopted state (linter present but non-blocking) is worse than not adopting yet.
...
- Research: [Docs markdown linting: rumdl adoption rationale](/research/engineering/documentation/docs-markdown-linting/) — why rumdl, alternatives rejected, open design questions.
