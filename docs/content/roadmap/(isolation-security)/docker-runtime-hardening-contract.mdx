...
**Current state**: Docker profile enforcement is wired, `standard` is the compiled default, and rootless DinD is a recommended explicit grant; credential env-only posture remains
...
| Does built-in runtime behavior require full sudo? | No built-in jackin❯ runtime path currently requires in-container `sudo`. Runtime package installs remain a role/operator capability decision and should declare `min_profile = "compat"` or an explicit sudo grant. | <RepoFile path="docs/content/(public)/guides/docker-profiles.mdx" /> |
...
1. **Credential env-only posture.** Under `hardened`/`locked`, prefer env-only or otherwise reduced credential exposure and emit a `credential_posture` decision per agent auth type. Coordinate with [Container credential exposure](/research/platform/security/credential-exposure/container-credential-exposure/).
...
- OTLP Capsule egress is jackin❯-owned infrastructure only in `allowlist` or `open` mode: every normalized per-signal endpoint host is derived and admitted without exposing host exporter headers to the container. `network=none` always wins, leaves Capsule telemetry unavailable, and is reflected in typed Capsule health; observability never weakens an isolation grant.
...
| [Process-level sandboxing](/research/platform/security/process-sandboxing/process-level-sandboxing/) | Per-command isolation inside any Docker profile. |
