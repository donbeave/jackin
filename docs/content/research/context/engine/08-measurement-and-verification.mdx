...
- **Counterfactual repricing**: what this session would have cost uncached / unrouted / unfiltered, reconciled against provider usage before any A/B is trusted. Current representation tools expose useful counterfactual patterns, but jackin❯ validates the exact method independently ([current rising-tools comparison](/research/context/tools/06-combining/)).
...
## The harness ([techniques 16 — Validation Harness: the No-Quality-Loss Proof Protocol](/research/context/techniques/16-validation-harness/), runnable per role)
