...
2. **The cross-tool harness** (the highest-value open deliverable in the <RepoFile path="prompts/research/context-tools.md">current research brief</RepoFile>): Native / engine-P2 / engine-P3 / engine-full / RTK / headroom-MCP / lean-ctx(faithful) / caveman+RTK stack / pxpipe proxy arm (gist-tolerant workloads only, allowlisted models, never stacked with headroom on the history region — [current rising-tools comparison](/research/context/tools/06-combining/)) — first controlled multi-arm numbers, published back into the research hub.
