---
title: "15 — Composed Stacks: Conservative / Aggressive / Unbelievable"
description: "End-to-end conservative, aggressive, and frontier optimization stacks with sequential cost and quality accounting."
---
...
Working profile ([01 — Token Economics and Measurement](/research/context/techniques/01-economics-and-measurement/) §5, $22 variant = 6 × measured session):
...
(07: measured −86.3% input-side vs uncached equivalent — $71.59 paid vs $524.23 on this very
session), MCP deferral by default (06), Edit-tool default (09), 1h-TTL main-loop writes
(07: 320/320 calls observed), prior-turn thinking billed per current docs (12).
...
[16 — Validation Harness: the No-Quality-Loss Proof Protocol](/research/context/techniques/16-validation-harness/) screening run (n=12) once; C1's falsifier is the failed-`old_string`
...
- **A1 × A2 double-press terseness** — effort-medium already produces terse output; adding the
 register can cross the token-complexity cliff on hard tasks (09 §10). Canaries C1–C6 of the
 harness are mandatory after stacking both.
- **A3 × caching** — every clear invalidates the prefix at the clearing point (12); `clear_at_least`
...
- **A4 × caching** — the prompt cache is model-scoped; route only at session/task boundaries
 (10: mid-session switch ≈ 9-turn break-even).
...
effort-sweep experiment (09 §1) before and after A1 — it doubles as the missing
thinking-fraction-by-effort measurement.

### 3. Unbelievable stack — chasing 10x

Adds BUILDABLE frontier items ([14 — Frontier — unrealistic but maybe real](/research/context/techniques/14-frontier-ideas/)) and flips the main loop. Stated honestly:
...
Composed from the aggressive endpoint of $8.63 using the same sequential method (prose-heavy case):
...
**composition's quality at parity is unmeasured (T4)** — and 09 §10's token-complexity cliff plus
10's "agent teams ≈ 7x tokens" warn that cheap-model fleets can pay back their savings in retries
