...
description: "Gateway and self-hosted techniques for semantic caching, prefix reuse, model serving, compression, and fleet control."
---

## Summary

Hosted gateways should preserve provider caching exactly; cache-aware routing and KV engineering become useful only on self-hosted serving stacks.
**TL;DR**
...
- Cheapest real wins today: gateway hygiene checklist (forward `anthropic-beta`/`anthropic-version`, one workspace per traffic lane, `CLAUDE_CODE_ATTRIBUTION_HEADER=0` for body-keyed caches, assert `cache_read_input_tokens > 0`), and deterministic JSON minification of verbose MCP schemas (**−22.9%** on the rewritten segment, local measurement — but deferred tool loading saves more; see [06 — Context architecture — what enters the window at all](/research/context/techniques/06-context-architecture/)).

## Question and scope

Which gateway and self-hosted mechanisms can reduce billed or processed context, and which cannot affect hosted-provider billing?

## Method

The analysis distinguishes hosted billing from self-hosted compute, measures representative encoding transforms with the checked-in token counter, and bounds each mechanism by deployment and quality constraints.
...
All dollar arithmetic below uses the modeled session profile from [01 — Token Economics and Measurement](/research/context/techniques/01-economics-and-measurement/) (local Baseline measurement): per heavy session ~19 API calls, 5.5k uncached input / 85k cache-write / 1.17M cache-read / 27k output; dollar split cache reads 32 / writes 29 / thinking 20 / visible output 17 / uncached input 2 (normalized to 100). Key derived quantity: **full-price-equivalent input = 32/0.1 + 29/1.25 + 2 = 345.2 units**, so the uncached-equivalent session is 345.2 + 37 = 382.2 units vs 100 actual → native caching removes 73.8% (~74%) of uncached-equivalent cost, and a fully broken cache costs 3.82x (~3.8x). ESTIMATE — n=1 session mix; the protective conclusion is robust to mix variation, the exact multipliers move with the cache-read share.

### Local measurements

Method: encode each stated n=1 sample, then run `python3 docs/content/research/context/techniques/tools/count_tokens.py --model MODEL text sample TEXT` against the live `count_tokens` API. The independent samples reproduce the Baseline ordering while retaining their own magnitudes.

| Variant | chars | tokens (claude-fable-5) | vs plain |
|---|---|---|---|
| English prose sample | 642 | 189 | — |
| base64 of same bytes | 856 | 819 | **4.33x MORE** (Baseline: 4.3x) |
| gzip+base64 of same bytes | 528 | 506 | **2.68x MORE** despite 17.8% FEWER chars (Baseline: 2.8x) |
| 5-param tool schema, pretty-printed (indent=2) | 938 | 310 | — |
| Same schema, JSON-minified | 712 | 239 | **−22.9% tokens** (Baseline schema: −25.7%) |
| Prose sample on claude-sonnet-4-6 tokenizer | 642 | 125 | Fable 5 tokenizer +51% on this jargon-dense sample (Baseline prose: +38%; direction confirmed, magnitude sample-dependent) |
...
- **Mechanism:** proxy transforms the body identically on every call (idempotent, byte-stable), so the prefix stays cache-consistent while containing fewer tokens. Local measurement (table above): 5-param schema pretty-printed 310 tokens → minified 239 (−22.9%; Baseline's different schema: −25.7%). Claude Code's built-in tool schemas are already compact; this applies to verbose MCP servers and custom SDK tools.
- **Expected savings:** ~23-26% of the rewritten segment only (n=2 schemas, local). On the modeled profile: the 11 local MCP schemas = 1,420 tokens ≈ 2.3% of the ~61.6k-token per-call prefix; minifying them saves ~0.5% of prompt-side spend ≈ 0.3% of session dollars (ESTIMATE). Deferred tool loading (1,420 → ~60 tokens, local Baseline) saves ~4x more on the same block. On self-host backends WITHOUT prefix caching configured, the full ~23% of the segment recurs every call, so it matters more there.
...
- **Composability:** compatible with prompt caching ONLY if deterministic from the session's first request. Pairs with (and is dominated by) deferred tool loading — see [06 — Context architecture — what enters the window at all](/research/context/techniques/06-context-architecture/). Anti-synergy with any per-request dynamic content injection.
...
For jackin❯, the infrastructure-level (self-hosted / gateway tier) evidence identifies which mechanisms are safe to adopt directly and which still require workload-specific validation.
...
|---|---|---|
| "Compress prompts with gzip/base64 before sending" | KILLED (local) | (table above): base64 = 4.33x MORE tokens; gzip+base64 = 2.68x MORE despite 17.8% fewer chars. Tokenizers have no merges for high-entropy encodings. Any gateway feature marketed this way is negative-value. (Baseline sample agreed: 4.3x / 2.8x.) |
...
| TokenMix "$42K/mo → $2.1K with LLMLingua, zero model change" | KILL (unverifiable marketing) | No methodology or workload disclosure (tokenmix.ai); contradicts the RCT, the code ceiling, and cache arithmetic. Its 20x anchor is GSM8K math CoT (77.33 vs 78.85 EM), not code. |
...
| 7 | vLLM APC prefill-only scope; block-hash design, sha256 default v0.11 | https://docs.vllm.ai/en/stable/features/automatic_prefix_caching/ and `/en/latest/design/prefix_caching/` | |
| 8 | SGLang "up to 6.4x higher throughput"; v2; no venue listed | https://arxiv.org/abs/2312.07104 (refetched) | |
| 9 | "Up to 5x" + baselines vllm v0.2.5 / guidance v0.1.8 / TGI v1.3.0 | https://www.lmsys.org/blog/2024-01-17-sglang/ | |
| 10 | HiCache internal "up to 6x throughput / up to 80% TTFT cut"; Novita Qwen3-Coder-480B 25K-tok ~8-turn: TTFT −56%, throughput 2x, hit 40%→80%; Ant DeepSeek-R1-671B: −84% TTFT; tiers + `--hicache-ratio 2` | https://www.lmsys.org/blog/2025-09-10-sglang-hicache/ (refetched) | |
| 11 | Router "up to 1.9x throughput / 3.8x hit rate"; 82,665→158,596 tok/s; 20%→75%; approximate radix tree, communication-free; workload caveat | https://www.lmsys.org/blog/2024-12-04-sglang-v0-4/ (refetched) | |
| 12 | LMCache "up to 15x improvement in throughput" (multi-round QA, doc analysis) | https://arxiv.org/abs/2510.09665 | |
| 13 | LMCache MoE ~13x TTFT (0.29s vs 3.98s; p99 1.30 vs 13.55); Momento >50% cold-start | vendor blogs blog.lmcache.ai and gomomento.com; vendor-reported | |
| 14 | Semantic cache 61.6-68.8% hit, 92.5-97.3% positive-hit, consumer-QA categories | https://arxiv.org/abs/2411.05276 (search digest; tables not independently read) | |
| 15 | LiteLLM cache features (7 backends, threshold 0.8, TTL, `x-litellm-cache-key`); NO published hit rates | https://docs.litellm.ai/docs/proxy/caching; absence claim | |
| 16 | LLMLingua up to 20x; GSM8K 77.33 vs 78.85; BBH 7x; 1.7-5.7x latency; LLMLingua-2 2-5x / 1.6-2.9x / 3-6x | https://llmlingua.com/llmlingua.html | |
| 17 | ~10% compression ceiling for code generation | ACM TOSEM https://dl.acm.org/doi/10.1145/3735636 (search digest) | |
| 18 | "Adds latency without quality benefits for code generation"; 1.4x on LongBench LCC; >1.3x speedups only on non-optimized stacks; overhead negates gains on vLLM/commercial APIs | https://arxiv.org/html/2604.02985 | |
...
| 20 | Kong AI Prompt Compressor: Enterprise, Gateway >=3.11, LLMLingua-2 sidecar, no published performance numbers | https://developer.konghq.com/plugins/ai-prompt-compressor/; absence claim | |
| 21 | Messages API full parameter list; no logprobs/draft/predicted-outputs/KV params | https://platform.claude.com/docs/en/api/messages | |
| 22 | Consumer Terms §3 no-train clause (commercial wording NOT independently verified — flagged) | https://www.anthropic.com/legal/consumer-terms | |
| 23 | Fable 5 $10/$50, Haiku 4.5 $1/$5 per MTok; batch 50% off | reference pricing confirmed against caching/pricing docs in #1 | |
| 24 | base64 4.33x (189→819 tok); gzip+base64 2.68x (189→506; chars 642→528); minify −22.9% (310→239); Fable-vs-Sonnet tokenizer +51% on sample | Checked-in token counter and the encoding procedure stated in Method | |
| 25 | Session mix 0.44%/6.73%/92.83% prompt-side; dollar split 32/29/20/17/2; ~$22/day; thinking 54.8% of output; MCP schemas 1,420 tok vs ~60 deferred; caveman 58.5%/56.6%/74.5% token cuts | local Baseline measurements (see [01 — Token Economics and Measurement](/research/context/techniques/01-economics-and-measurement/), [02 — Baseline audit of the research environment](/research/context/techniques/02-baseline-audit/)) | |
