---
title: "09 — Output discipline and structured generation"
description: "Methods for reducing visible output and structured-generation overhead without losing decisions, evidence, or errors."
---

## Summary

Diffs, omission of code restatement, and concise wrappers reduce output safely; brevity must not remove reasoning, decisions, evidence, or errors.
**TL;DR**
...
Which output controls remove restatement and formatting waste while preserving reasoning, correctness, and warnings?

## Method

The analysis compares provider output controls, published quality studies, and local counts of equivalent edit, rewrite, summary, and structured-output payloads using the checked-in token counter.

## Findings

### Why this layer matters

On the modeled heavy day (~$22, 6 sessions, ~27k output tok/session — see [01 — Token Economics and Measurement](/research/context/techniques/01-economics-and-measurement/)), output is ~$8.14/day: ~$4.40 thinking + ~$3.74 visible (prose + tool arguments). Three structural facts shape everything below:
...
Method: free `count_tokens` API, model `claude-fable-5`, each payload sent as a single user message; 1-char baseline message = 7 tok envelope, subtracted in % columns. Measurement script reproduced in [16 — Validation Harness: the No-Quality-Loss Proof Protocol](/research/context/techniques/16-validation-harness/) style: simulate the exact JSON a tool call would carry. File under test: <RepoFile path="crates/jackin-config/src/app_config/workspaces.rs">crates/jackin-config/src/app_config/workspaces.rs</RepoFile> (200 lines); change under test: a realistic 5-line method addition with a 5-line unique anchor.
...
These independently reproduce the baseline numbers (89.5% / 302 tok / 15–25 tok est.) within noise of payload phrasing. At $50/MTok: an avoided whole-file rewrite of this file saves ~$0.143; an avoided verbose summary ~$0.018; an avoided JSON wrapper ~$0.0015.
...
- **Mechanism:** Claude Code output styles replace part of the default system prompt (re-sent and cache-stable every turn); CLAUDE.md content rides as a user message after the default system prompt. The caveman plugin implements register-based persona compression as a skill. The current `outputStyle` setting remains available through `/config`; the `/output-style` command is absent (github.com/anthropics/claude-code/issues/10671).
- **Expected savings:** local baseline ground truth: caveman ultra = **58.5%** token cut on visible prose (the plugin's "~75%" claim is character-level folklore); wenyan-full = 80.9% char cut but only **56.6%** token cut (CJK ≈ 1.47 chars/tok vs 3.35 English); wenyan-ultra = 74.5% token cut. Applies ONLY to visible prose ≈ 17% of session dollars: upper bound 0.585 × $3.74 ≈ **$2.19/day** on the modeled profile if every visible token were prose (it isn't — tool args share that class, so real ceiling is lower).
...
For jackin❯, the output discipline and structured generation evidence identifies which mechanisms are safe to adopt directly and which still require workload-specific validation.
...
| Claude Code effort defaults (high on Fable 5/Opus 4.8/4.6/Sonnet 4.6; xhigh on Opus 4.7); max session-only (persists when set via `CLAUDE_CODE_EFFORT_LEVEL`) + "prone to overthinking"; ultracode/ultrathink semantics; per-model calibration; frontmatter `effort` | quotes | code.claude.com/docs/en/model-config |
| "Thinking cannot be turned off on Fable 5… MAX_THINKING_TOKENS=0 have no effect there" | quote | same |
| Edit vs Write payloads (200-line file, 5-line change) | 347 vs 3,197 tok = 89.3% | Local `count_tokens` measurement using the file, anchor, payload construction, and model specified in Method with the [checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes) |
...
| Token-complexity thresholds; universal curve; success prediction | "above 90% for most models and benchmarks" (the stronger 95% figure is not confirmed) | arxiv.org/html/2503.01141v1 |
...
| Prefill `{` mechanics + thinking incompatibility | quotes | platform prefill doc via search snippets (direct fetch redirected — page possibly mid-reorganization); 400s on Opus 4.6: blog.laozhang.ai (T3) |
| Claude Code REPL lacks schema-constrained output | open issue | github.com/anthropics/claude-code/issues/9058 |
| Current output-style behavior and availability | current provider status | code.claude.com/docs/en/output-styles |
| Caveman/wenyan register cuts | 58.5% / 56.6% / 74.5% token; 80.9% char | local Baseline measurement |
| Thinking share of output at max effort | 54.8% (n=1) | local Baseline: usage.output_tokens − count_tokens(visible blocks) |
| Session dollar split (reads 32/writes 29/thinking 20/visible 17/uncached 2) | modeled day ~$22 | local Baseline heavy-session profile — see [01 — Token Economics and Measurement](/research/context/techniques/01-economics-and-measurement/) |
| Root <RepoFile path="AGENTS.md">AGENTS.md</RepoFile> always-on size | 2,738 tok | local Baseline measurement |
