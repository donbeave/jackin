...
description: "Comparison of current token-optimization tools, memory systems, formats, infrastructure, and gaps in published evidence."
---

## Summary

Current tools concentrate on visible prose and memory while leaving cache reads, cache writes, and thinking—the dominant measured cost buckets—largely untreated.
**TL;DR**
...
- The market crowds onto the two smallest cost buckets of a real heavy session — visible prose (17% of dollars) and memory files (slice of the 2% uncached input) — while **nothing ships** for the big three: cache reads (32%), cache writes (29%), thinking (20%) (local measurement; see [01 — Token Economics and Measurement](/research/context/techniques/01-economics-and-measurement/)).
...
- Local kills (fresh, [checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes), Fable-family tokenizer): TOON saves **41.2% vs minified JSON**, not "60%" (34.3% of the headline is just minification); caveman's "~75%" replicates as **58.5%** on tokens; the Fable-vs-Sonnet tokenizer gap is content-specific — prose/ASCII gets the premium, but code/CJK probes were near-neutral.

## Question and scope

Which published tools and techniques reduce end-to-end coding-agent tokens, and which claims collapse under direct measurement?

## Method

### Scope and method

Scanned: the four operator-named tools, Claude Code's official cost levers, the plugin/MCP ecosystem, gateways, and research-grade prompt compression. Every external claim carries a source URL and evidence tier. Local numbers use [checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes) (free `count_tokens` endpoint; the single-user-message wrapper adds a constant 7 tokens — percentages below are conservative by &lt;1 point). Dollar arithmetic uses the modeled heavy-day profile from [01 — Token Economics and Measurement](/research/context/techniques/01-economics-and-measurement/): ~6 sessions/day, ~19 calls/session, 5.5k uncached input, 85k cache-write, 1.17M cache-read, 27k output (54.8% thinking), ~$22/day split 32% cache-read / 29% cache-write / 20% thinking / 17% visible output / 2% uncached input. Validation protocols are designed to run on the harness in [16 — Validation Harness: the No-Quality-Loss Proof Protocol](/research/context/techniques/16-validation-harness/); this machine's own overhead baseline is in [02 — Baseline audit of the research environment](/research/context/techniques/02-baseline-audit/).
...
### Local measurements run for this file ([checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes), claude-fable-5)
...
An independent local measurement with a different dataset of the same shape produced -60.6%/-40.8%/-33.6%, agreeing within about one percentage point.
...
Sonnet 4.6 and Haiku 4.5 return byte-identical counts — shared tokenizer confirmed. Other baseline samples measured +15% for code and +38% for prose; a tighter probe measured prose +35%, code −3%, and CJK −4% — the premium is prose/ASCII-specific, not code-wide. Treat the Rust row above as sample-specific.

Self-cost of the optimizer: the caveman plugin family's always-on skill listing (7 skills, names + descriptions as injected in the measured environment) = **940 tokens** of prefix per session — ~0.5% of the modeled day in cache-read rent before it saves anything. Root <RepoFile path="AGENTS.md">AGENTS.md</RepoFile> measured at **2,744 tokens**.
...
- **Expected savings:** independent trace: same session ~$5.50 on Sonnet vs ~$10+ on Opus (dev.to slima4). **Cross-boundary math:** use **3.33x Fable:Sonnet** on code/CJK-heavy text and **~4.3x** on prose/ASCII-heavy text after the tokenizer premium (ESTIMATE; verification). Downgrades save at least the sticker ratio, and prose-heavy downgrades save more.
...
- **Layer:** input (tool schemas). **Mechanism:** schemas stay out of context (names only) until needed; API Tool Search Tool queries a registry; Claude Code defers MCP definitions by default ("only tool names enter context until Claude uses a specific tool" — code.claude.com/docs/en/costs).
...
- **Evidence tier:** T3 + local reproduction (two independent local measurements agree within about one percentage point).
...
- **Availability:** CLAUDE-CODE-TODAY. **Effort to adopt:** minutes.
- **Composability:** foundation for every validation protocol here; see [16 — Validation Harness: the No-Quality-Loss Proof Protocol](/research/context/techniques/16-validation-harness/).
...
- **Composability:** the denominator for every percentage in this file; this machine's own numbers in [02 — Baseline audit of the research environment](/research/context/techniques/02-baseline-audit/).
...
2. **Cache-economics tooling** — reads+writes = 61% of heavy-session dollars; nothing optimizes breakpoint placement, predicts 5-min-vs-1-h TTL value, schedules compaction to minimize write spikes, or reports write amplification (ccusage shows totals, not causes). See [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/).
...
7. **Independent replication infrastructure** — every vendor self-reports; a public ct.py-style harness re-measuring plugin claims against live tokenizers does not exist (this dossier's method is itself novel prior art; see [16 — Validation Harness: the No-Quality-Loss Proof Protocol](/research/context/techniques/16-validation-harness/)).
...
For jackin❯, the prior art and market scan evidence identifies which mechanisms are safe to adopt directly and which still require workload-specific validation.
...
| K5 | "Model price ratios = cost ratios" | Partly false: code/CJK-heavy work can be close to sticker ratios, but prose/ASCII-heavy text gets the Opus-4.7+ tokenizer premium. Effective Fable:Sonnet input ratio is ~3.33x for code-heavy probes and ~4.3x for prose-heavy probes. Routers do not tokenizer-normalize. |
...
| caveman local: ultra 58.5%; wenyan-full 56.6% tok / 80.9% chars; wenyan-ultra 74.5%; CJK ~1.47 chars/tok; tool deferral 1,420 -> ~60 (96%); prompt mix 0.44/6.73/92.83%; dollar split 32/29/20/17/2; thinking 54.8% of output; day ~$22 | local measurement (baseline); profile detailed in [01 — Token Economics and Measurement](/research/context/techniques/01-economics-and-measurement/) |
...
| Local tokenizer premium +42.7% prose / +39.2% one Rust sample raw (+44.7%/+39.7% net of 7-tok wrapper); Sonnet=Haiku identical counts; prose premium holds; code/CJK can be near-neutral | [checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes) on Fable-family / Sonnet-family models, fixed prose + <RepoFile path="crates/jackin-build-meta/src/lib.rs">crates/jackin-build-meta/src/lib.rs</RepoFile> head |
...
| Local format table 484/318/187 tabular (-34.3%/-61.4%/-41.2%), 272/194 nested (-28.7%); independent local sample -60.6/-40.8/-33.6; log filter 10,108 -> 590 (-94.2%, 3/3 failures kept); <RepoFile path="AGENTS.md">AGENTS.md</RepoFile> 2,744 tok (baseline 2,738); caveman skill listing 940 tok; wrapper constant 7 tok | [checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes) claude-fable-5 on generated fixtures with reproducible generated fixtures, repo <RepoFile path="AGENTS.md">AGENTS.md</RepoFile>, reconstructed skill listing, 1-char probe; grep filter shown above |
