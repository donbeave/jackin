...
description: "Protocols for compact delegation, bounded subagent reports, shared evidence, handoffs, and multi-agent context control."
---

## Summary

Multi-agent savings depend on small dispatches, bounded evidence-bearing returns, and stable shared prefixes; unconstrained fan-out merely multiplies context.
Area H. Siblings: cost model and heavy-day profile in [01 — Token Economics and Measurement](/research/context/techniques/01-economics-and-measurement/) (§5); register mechanics in [04 — Style and Language Compression — Beyond Caveman](/research/context/techniques/04-style-and-language-compression/); tokenizer effects in [05 — Tokenizer arbitrage](/research/context/techniques/05-tokenizer-arbitrage/); cache mechanics in [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/); worker down-routing in [10 — Model routing and tiered delegation](/research/context/techniques/10-model-routing-and-delegation/); self-host-only inference plays in [13 — Infrastructure-level (self-hosted / gateway tier)](/research/context/techniques/13-infrastructure-level/); eval design in [16 — Validation Harness: the No-Quality-Loss Proof Protocol](/research/context/techniques/16-validation-harness/).

Arithmetic in this file uses the **$17/day floor variant** of the modeled profile from [01 — Token Economics and Measurement](/research/context/techniques/01-economics-and-measurement/) unless
explicitly tied to the local 25-agent fleet case study. Multiply floor-profile dollar figures by
~1.28 to compare against the $22/day working variant used in [15 — Composed Stacks: Conservative / Aggressive / Unbelievable](/research/context/techniques/15-composed-stacks/).
...
How should multi-agent work compress coordination and returned evidence without multiplying cold context or losing task boundaries?

## Method

The analysis separates main-thread, spawn, cache, and returned-summary costs in a measured fleet sample, then tests protocol recommendations against provider semantics and lossless handoff requirements.
...
### 2. Case study: 25-agent local fleet sample

The measured sample covers 25 agents, 3.64M subagent tokens, 668 tool uses, and 202 API calls; input was 180k uncached / 1.71M cache-write / 12.95M cache-read / 390k output, approximately $55.6 at the recorded list prices.
...
**Mechanism:** SDK semantics above; Claude Code costs docs: "the verbose output stays in the subagent's context while only a summary returns to your main conversation." Each spawn re-pays project context (CLAUDE.md = 2,744 tok in this repo, local re-count via [checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes)) plus tool defs; subagents cannot nest. Orchestrator-side spec discipline is part of the protocol: "Each subagent needs an objective, an output format, guidance on the tools and sources to use, and clear task boundaries" — Anthropic reports vague specs made subagents "misinterpret the task" and duplicate work.
**Expected savings:** Keeping a 10k-tok log/test payload out of a parent with 30 turns left avoids ~10k×(1.25+2.9)×$10/M ≈ $0.42 input spend per occurrence, against ~$0.11 spawn overhead (local) plus report cost → net ~$0.25-0.30/occurrence. On the [01 — Token Economics and Measurement](/research/context/techniques/01-economics-and-measurement/) §5 profile ($17.00/day), five such delegations/day ≈ $1.3-1.5 ≈ 8-9% (ESTIMATE), plus deferred compaction. Counterweight: agents ≈4x chat, parallel research multi-agent ≈15x — delegation saves only on verbose side-tasks.
...
**Composability:** Substrate for H2-H5; pairs with worker down-routing ([10 — Model routing and tiered delegation](/research/context/techniques/10-model-routing-and-delegation/)); conflicts with nothing.
...
**Composability:** Multiplies with H1 (applies exactly at the re-read interface); stacks with Haiku down-routing (same payload 135 vs 170 tok under the Sonnet/Haiku tokenizer — [05 — Tokenizer arbitrage](/research/context/techniques/05-tokenizer-arbitrage/)); choose one register per interface vs H3.
**Validation protocol:** count_tokens A/B on same-information report pairs (harness per [16 — Validation Harness: the No-Quality-Loss Proof Protocol](/research/context/techniques/16-validation-harness/)); always count with the Anthropic tokenizer, never tiktoken.
...
**Quality risk:** NEGATIVE-COST for code and long reports — byte-exact, immune to broken-telephone re-summarization (H7); only stale-pointer hygiene. Falsification: count stale-path Read failures; if material, namespace under /jackin/run/&lt;task&gt;/. Verdict: NEGATIVE-COST.
...
**Expected savings:** Prefix term at P=20k, n=5: $1.25 vs $0.33 per fan-out wave. Spawn writes in the 25-agent sample totaled ~$2.1-2.7; perfect staggering of the cross-agent-shared portion would recover roughly $1-1.5 of the $55.6 sample (~2-3%, ESTIMATE). Writes of 6.8-8.5k tokens against a 33,746-token base context indicate that Claude Code dispatch already reused ~75-80% of the shared prefix; the full 3.8x applies only to naive simultaneous SDK fan-out.
...
**Composability:** Interacts with everything that fattens the shared prefix — CLAUDE.md (2,744 tok), MCP schemas (1,420 tok loaded vs ~60 deferred, local) — the fatter the prefix, the more staggering pays; see [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/).
...
**Availability:** NOT-USER-ACCESSIBLE (GATEWAY-OR-SELF-HOST at absolute best, same-architecture pairs, custom serving — see [13 — Infrastructure-level (self-hosted / gateway tier)](/research/context/techniques/13-infrastructure-level/)).
...
**Validation protocol:** Track rework rate on compressed vs full task specs; run the 1-2-hop decode eval per [16 — Validation Harness: the No-Quality-Loss Proof Protocol](/research/context/techniques/16-validation-harness/).
...
For jackin❯, the multi-agent protocols and subagent compression evidence identifies which mechanisms are safe to adopt directly and which still require workload-specific validation.
...
| 6 | 25-agent fleet sample: agents/tokens/tool uses/calls; in/cw/cr/out; list cost | 25; 3.64M; 668; 202; 180k/1.71M/12.95M/390k; ≈$55.6 | local fleet self-observation |
...
| 11 | CLAUDE.md token cost per spawn (this repo) | 2,744 tok | `python3 docs/content/research/context/techniques/tools/count_tokens.py --model claude-fable-5 file rules AGENTS.md` |
...
| 14 | caveman-ultra cut on prose; advertised claims | 58.5%; 60-75% claimed | local (Baseline); github.com/JuliusBrussee/caveman README |
...
| 28 | Modeled day profile used for % arithmetic | $17.00/day; $1.70/task | [01 — Token Economics and Measurement](/research/context/techniques/01-economics-and-measurement/) §5 (local model) |
| 29 | Tokenizer divergence (Fable vs Sonnet/Haiku) on identical text | 15-38% sample-dependent; register payload 170 vs 135 tok | local measurement; [05 — Tokenizer arbitrage](/research/context/techniques/05-tokenizer-arbitrage/) |
