---
title: "24 — Fresh literature & market delta (clean-room re-sweep)"
---

# 24 — Fresh literature & market delta (clean-room re-sweep)
...
.** Anthropic made Fable 5 free on Pro/Max/Team/Enterprise "through June 22"
 ...
 dollar figures should be read as **~half** for an Opus-4.8 subscriber (carried to 27).
- **The KV-cache compression family Volume I missed (SnapKV, H2O, PyramidKV, KVQuant) is real,
 ...
 reduce *server-side GPU memory*, not API-billed tokens — they extend Volume I's self-host tier (13),
...
- **A new risk axis: prompt-compression proxies are an attack surface.** CompressionAttack (arXiv
 ...
 path" verdict. The paper was revised on arXiv — title now "When Compression Becomes an Attack Surface: Black-Box Attacks on Prompt-Compressed LLM Agents", attack named COMA, abstract reporting 0.71 average ASR vs 0.21 for the strongest baseline; the risk-axis verdict is unchanged. Two new lossless-compression papers (LoPace at-rest; LTSC meta-tokens) do **not** help
 a hosted-API token bill (one compresses disk bytes, the other needs a fine-tuned model).
- **No post-06-12 change to Anthropic's API cost primitives** (caching multipliers, context-editing beta, memory tool, effort ladder all unchanged) **with one exception: Claude Sonnet 5** (2026-06-30), a new Anthropic billable model — default in Claude Code 2.1.197, native 1M context, 128K output, $2/$10 per MTok promo through 2026-08-31 then $3/$15; the "API primitives did not move" verdict carries this exception, and the Fable-promo-end tiering flip to Opus 4.8 is superseded for workloads Sonnet 5 covers (Claude Code CHANGELOG 2.1.197; https://www.anthropic.com/news/claude-sonnet-5). The other deltas are Claude Code *app* features (`/cd` preserves cache; subagents now nest 5 deep; `/usage` gained a cache-miss line) and the 06-15 billing split ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/research/context/techniques/19-subscription-and-quota-economics/)). Routing research broadened past RouteLLM (LLMRouterBench, OrcaRouter) but none produces a Claude-cache-aware number that overrides Volume I's verdict.

Dollar/quota context inherited from Volume I and [19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/research/context/techniques/19-subscription-and-quota-economics/); tiers per finding.

---

## A. Provider changelog drift

| Change | Date | Tier | Effect on a Volume I number |
|---|---|---|---|
| **Fable 5 free window ends; removed from plans 06-23** | | T1 | Big — model-tiering flips to Opus 4.8 for subscribers (27). Fable 5 was suspended 2026-06-12..06-30 under an export-control directive and redeployed 2026-07-01; Pro/Max/Team/select Enterprise include it up to 50% of weekly usage limits through 2026-07-07, then usage credits at API rates — the Opus-4.8-only subscriber tiering applies only after 07-07 (https://www.anthropic.com/news/redeploying-fable-5). |
| **SDK/`claude -p`/GitHub-Actions usage off the subscription cap** → separate API-rate credit | | T1 | Bifurcates the cost model by surface ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/research/context/techniques/19-subscription-and-quota-economics/) Q7) |
| **Claude Code: subagents nest up to 5 levels deep** (2.1.172) | | T1 | Compounds the ~7× team blowup geometrically (25 governance) |
| **Claude Code `/cd` preserves the prompt cache** across a dir change (2.1.169) | | T1 | New negative-cost cache lever (technique FL2) |
| **`/usage` adds cache-miss + subagent/long-context attribution** (2.1.174) | | T1 | Partially closes Volume I white-space #2 (cache tooling). The 2.1.174 changelog entry is scoped [VSCode] — attribution (cache misses, long context, subagents, per-skill/agent/plugin/MCP, 24h/7d) landed in the VSCode Account & usage dialog, not the CLI `/usage`. |
| **Claude Sonnet 5 ships** (2026-06-30; default model in Claude Code 2.1.197; native 1M context; 128K output; $2/$10 per MTok promo through 2026-08-31 then $3/$15) | 2026-06-30 | T1 | New billable Anthropic model — exception to the "API primitives unchanged" verdict below; supersedes the Opus-4.8 tiering flip for workloads Sonnet 5 covers (anthropic.com/news/claude-sonnet-5 + Claude Code CHANGELOG 2.1.197) |
| OpenAI **GPT-5.5** in API ($5/$30, $0.50 cached = 0.1×; effort none→xhigh; 1.05M ctx; >272K billed 2×/1.5×) | | T1 | Portability (23); no Claude-side change |
| OpenAI **`prompt_cache_retention` defaults to 24h** (non-ZDR) | | T1 | Strengthens OpenAI cross-provider cache case (23) |
| OpenAI **container sessions billed per-minute** (5-min min) | | T1 | Minor; rewards finishing fast (21) |
...
hit the *subscriber* directly and are folded into [19 — Subscription & quota economics](/research/context/techniques/19-subscription-and-quota-economics/) and the [27 — Volume II capstone](/research/context/techniques/27-extension-stacks-and-verdict/). The verdict carries a Sonnet 5 exception — see the Sonnet 5 row above.
...
Volume I's vLLM/SGLang/HiCache section (13) and its K2 (LMCache/CacheGen) — and do **not** give a
...
|---|---|---|
| **LoPace** (2602.13266, Feb 2026) | lossless compression *at rest* (Zstd/BPE+packing), 72.2% disk saving, 100% reconstruction | **Irrelevant to tokens** — compresses bytes on disk, not tokens sent; only helps a memory-store's footprint (Vol I 08). Category error to avoid. |
...
| **CompressionAttack** (2510.22963, Oct 2025) | security: perturbations surviving compression flip agent behavior (≤80% ASR) | **New risk axis** — adds a security reason to Volume I's cost-based "no compressor in the hot path." (Revised on arXiv as "When Compression Becomes an Attack Surface: Black-Box Attacks on Prompt-Compressed LLM Agents"; attack named COMA; 0.71 average ASR vs 0.21 strongest baseline.) |
...
- **LLMRouterBench** (2601.07206, Jan 2026): 400K+ instances, 33 models, 10 baselines; top routers
 +4% accuracy or 31.7% cost reduction at baseline performance. The larger benchmark Volume I (10)
...
- **OrcaRouter** (2605.30736, May 2026): production LinUCB contextual-bandit router, RouterArena #2 at
 submission. Resolves part of [10 — Model routing and tiered delegation](/research/context/techniques/10-model-routing-and-delegation/)'s open "no current router eval" question — still a
...
- **Coverage-delta:** New. CAG is 0-hit in Volume I (confirmed 18-overview); [08 — Retrieval, memory, and state offloading](/research/context/techniques/08-retrieval-memory-and-state/) debates
...
- **Expected savings:** converts per-turn RAG retrieval+injection into a one-time cached-prefix write
 ...
 subscription the cached reads are also ~0.1× against the cap ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/research/context/techniques/19-subscription-and-quota-economics/)). Bounded by the 200K
 subscription context ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/research/context/techniques/19-subscription-and-quota-economics/)) — large corpora still need retrieval.
...
- **Composability:** stacks with prompt caching (07) and the codebook/pointer ideas ([08 — Retrieval, memory, and state offloading](/research/context/techniques/08-retrieval-memory-and-state/)/14);
...
- **Coverage-delta:** New (Claude Code 2.1.169). [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/) enumerates cache-safe vs
...
- **Mechanism:** `/cd` moves a session to a new working directory "without breaking the prompt cache
 ...
 22) and forced a full rewrite at 1.25–2×.
- **Expected savings:** one avoided full-history rewrite per directory change — the per-bust economics
 of [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/) tech 1 / [19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/research/context/techniques/19-subscription-and-quota-economics/) Q2 (≈150K × (1.25–2.0 − 0.1) tokens).
...
- **Composability:** extends [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/)'s cache-safe operation list; pairs with prefix-stability (19 Q2).
...
- **Coverage-delta:** New risk axis. [13 — Infrastructure-level](/research/context/techniques/13-infrastructure-level/) covers the *cost* trap of compression proxies;
...
- **Mechanism:** CompressionAttack (2510.22963) crafts inputs that survive a compression module but
 ...
 (LLMLingua/Selective-Context) and soft (ICAE/AutoCompressors) compressors. The revised paper names the attack COMA and reports 0.71 average ASR vs 0.21 strongest baseline. A compressor in the
...
- **Expected savings:** none — loss-avoidance. Reinforces routing compression to ingestion-only seams
 ([14 — Frontier](/research/context/techniques/14-frontier-ideas/) K15) or avoiding it entirely for code.
...
- **Effort to adopt:** minutes (policy: no compressor in the hot path; see [25 — Meta layer: the cost of optimizing, budget governance, and online quality guarding](/research/context/techniques/25-meta-cost-governance-and-online-quality/)).
- **Composability:** strengthens Volume I's LLMLingua kill; relevant to governance (25) and infra (13).
...
- **Coverage-delta:** New (0-hit in Volume I). Extends [13 — Infrastructure-level](/research/context/techniques/13-infrastructure-level/) (self-host) and 14 K2; the
...
- **Composability:** self-host stack only (with [13 — Infrastructure-level](/research/context/techniques/13-infrastructure-level/)/K2); orthogonal to everything hosted.
...
- **Coverage-delta:** Drift, not in Volume I (the promo window post-dates the freeze framing). Volume
 I's tiering (10) and profile assume Fable 5 as the main model.
...
- **Expected savings:** for a subscriber who was on Fable 5, moving the main loop to Opus 4.8 halves
 ...
 Volume I's Fable-priced dollar figures are ~2× an Opus-4.8 subscriber's (carried to 27).
...
- **Composability:** the tokenizer family is unchanged (Opus 4.8 ≡ Fable 5 tokenizer), so all of
 Volume I's tokenizer-arbitrage and file-20 image findings transfer directly.
 ...
 *billing policy* (Fable promo, 06-15 SDK split) — which is exactly the quota axis (19) Volume I
 under-weighted.
...
| 3 | Claude Code: nested subagents 5-deep (2.1.172); `/cd` preserves cache (2.1.169); `/usage` cache-miss + attribution (2.1.174; entry is scoped [VSCode] — VSCode Account & usage dialog) | code.claude.com/docs/en/changelog |
...
| 8 | CompressionAttack 2510.22963: ≤80% ASR on prompt-compression modules (paper revised — title now "When Compression Becomes an Attack Surface: Black-Box Attacks on Prompt-Compressed LLM Agents"; attack named COMA; abstract reports 0.71 average ASR vs 0.21 strongest baseline; cite the versioned v2 URL) | arxiv.org/html/2510.22963v2 |
