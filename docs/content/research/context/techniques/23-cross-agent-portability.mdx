...
description: "Portability matrix for token-efficiency techniques across agents, providers, APIs, hooks, and local infrastructure."
---
...
- **What does NOT port: the `cache_control` breakpoint API is Anthropic-only.** Everywhere else
 ...
 switch) but cannot *place breakpoints*. So the core dossier's caching *discipline* ports; its caching
...
- **Net for the operator:** the core dossier stack is ~80% portable as discipline and ~60% as feature.
 The Anthropic-specific edge is `cache_control` breakpoints, the 1-hour-TTL-on-subscription behavior
 ([19 — Subscription and quota economics](/research/context/techniques/19-subscription-and-quota-economics/)), and fast mode ([21 — Latency, wall-clock, and human-time as a second cost axis](/research/context/techniques/21-latency-and-time-economics/)). The biggest *gains* from looking outward are reverse-ported:
...
- **Effort tiering — PORTS as a first-class feature everywhere**, with finer granularity than the core dossier documented (OpenAI/OpenCode now expose none→xhigh; Gemini 3 uses `thinking_level` with per-model values: gemini-3-pro-preview low/high; gemini-3-flash-preview low/medium/high; gemini-3.5-flash minimal/low/medium/high; gemini-3.1-pro-preview low/medium/high; `thinkingBudget` was dropped from the thinking page). Thinking-bills-as-output is universal, so "lower effort on routine work"
...
- **Output discipline — PORTS unevenly.** Codex (`model_verbosity`, `model_max_output_tokens`) and
 ...
 the cross-agent form of the core dossier's Edit-over-Write lever. Claude Code relies on hooks/instructions.
- **Register compression (caveman) — DOES NOT PORT as a feature.** No agent ships a register
 ...
 inline" — Cursor/Copilot vendor guidance). Consistent with the core dossier's finding that style
...
- **Quota/subscription specifics — DO NOT PORT.** Each provider's cap structure, cache-vs-storage
 billing, and auth-gating differ (below); [19 — Subscription and quota economics](/research/context/techniques/19-subscription-and-quota-economics/)'s Anthropic quota model is Anthropic-specific.
...
- **Evidence scope:** New synthesis. The core dossier labels availability per technique but never identifies
...
- **Expected savings:** avoids a full prefix re-ingestion across idle gaps on 5-min-TTL paths — the
 same per-bust economics as [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/), realized as a shipped flag.
...
- **Availability:** Aider today; the *pattern* is portable to any SDK harness ([07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/) tech 3).
...
- **Evidence scope:** the core dossier's routing file (10) covers model tiering and advisor escalation; the
...
- **Mechanism:** Aider's architect mode sends planning to a strong reasoner then conversion-to-edits
 ...
 calls. The diff-emitting editor is the cross-agent form of the core dossier's Edit-over-Write lever.
- **Expected savings:** combines the core dossier's model-routing and output-minimization savings: expensive
...
- **Availability:** Aider / OpenCode today; portable to Claude Code as subagent-model pinning +
 Edit-tool discipline ([09 — Output discipline](/research/context/techniques/09-output-discipline/)/10).
...
prefix — a context-selection lever the core dossier under-emphasizes.

- **Evidence scope:** The core dossier covers compaction and observation masking (06) but not
...
- **Composability:** the dedup-before-truncate ordering pairs with the core dossier context editing (06/12)
...
- **Expected savings:** bounds tool-output bloat (a top hidden cost in long sessions) and avoids
 always-loading rules that only matter on some paths — the conditional-context idea [08 — Retrieval, memory, and state offloading](/research/context/techniques/08-retrieval-memory-and-state/)
...
- **Mechanism:** (a) `cache_control` breakpoint placement is Anthropic-only — elsewhere you protect
 ...
 19) and **fast mode** ([21 — Latency, wall-clock, and human-time as a second cost axis](/research/context/techniques/21-latency-and-time-economics/)) are Claude-Code-specific; (c) caveman register compression is a
...
|---|---|---|---|
| Claude Code | sub cap ([19 — Subscription and quota economics](/research/context/techniques/19-subscription-and-quota-economics/)) / API $ | 0.1× (cache_read) | `cache_control`, 1h-TTL on sub, fast mode |
...
as output, and cached input is ~10% of input almost everywhere — so the core dossier's economics (output is
