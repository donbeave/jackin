---
title: "12 — Provider-level features, current and beta"
description: "Provider-native caching, compaction, tool search, programmatic calls, batching, and other token-control features."
---

## Summary

Provider-native caching, deferred tool loading, and context editing offer stronger evidence than external compression, but volatile model and quota claims require one-cutoff verification.
**TL;DR**
...
- One of Anthropic's own live pages still says prior-turn thinking is "automatically stripped": the token-counting page carries the blanket "Thinking blocks from previous assistant turns are ignored and do not count toward your input tokens" note, while the adaptive-thinking, context-editing, and context-windows pages say Opus 4.5+/Sonnet 4.6+/Fable 5/Mythos 5/Mythos Preview keep ALL prior thinking turns in context and bill them as input (earlier Opus/Sonnet and all Haiku models strip them). The contradiction is live and worth real money on long sessions.

Method: every doc claim re-fetched live from platform.claude.com (docs.claude.com redirects there) / claude.com / anthropic.com / github.com; cheap-to-check token claims reproduced locally via the free count_tokens harness ([checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes), OAuth, exact-model counts). Modeled session profile for arithmetic (local Baseline measurement): heavy day ≈ 6 sessions × (19 calls, 5.5k uncached in, 85k cache-write, 1.17M cache-read, 27k out @ 55% thinking) ≈ $22/day at Fable 5 list prices — internally consistent with the live price table to within 1%.

## Question and scope

Which current provider-native controls reduce token use, and what availability, cache, and quality constraints bound them?

## Method

### Local measurements (count_tokens API, method: [checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes))
...
Fable 5 ≡ Opus 4.8 and Sonnet 4.6 ≡ Haiku 4.5 byte-for-byte on every sample — two tokenizer families confirmed. The +27.7% on this Rust sample is above the Baseline ~15% code figure (different sample) but inside the vendor's "up to 35%" band; structured JSON pays the smallest penalty.
...
| Visible output 17% | ~$3.7 | effort (affects ALL response tokens), see [09 — Output discipline and structured generation](/research/context/techniques/09-output-discipline/) |
...
- **Composability:** multipliers stack with Batch 50% and data-residency 1.1x ("These multipliers stack with other pricing modifiers"). Tension: context editing/compaction invalidate cache at the edit point; defer_loading and stable adaptive thinking preserve it. See [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/).
...
- **Effort to adopt:** hours (one edits entry + append full `response.content` so compaction blocks persist; track cost via `usage.iterations[]` — with the compaction beta enabled every response includes `usage.iterations`; the compaction ENTRY appears only on turns that trigger a new compaction, and re-applying an existing compaction block incurs no additional compaction cost).
...
- **Layer:** output.
- **Mechanism:** `output_config: {effort: low|medium|high|xhigh|max}`, GA, no beta header; `high` default ("exactly the same behavior as omitting the parameter"). Affects ALL response tokens: thinking, tool-call count, preamble ("lower effort would mean Claude makes fewer tool calls"). `xhigh` on Fable 5/Mythos 5/Opus 4.8/Opus 4.7/Sonnet 5; `max` on those plus Mythos Preview/Opus 4.6/Sonnet 4.6; Claude Code default effort is `high` on Fable 5/Sonnet 5/Opus 4.8/Opus 4.6/Sonnet 4.6 and `xhigh` on Opus 4.7 (effort page + code.claude.com/docs/en/model-config). On Fable 5 thinking is always-on (`{type:"disabled"}` rejected); `budget_tokens` 400s on Opus 4.8/4.7, deprecated on 4.6/Sonnet 4.6. `max_tokens` is the hard cap (docs: start 64k at xhigh/max). Claude Code: default high (effort page: Opus 4.8 "default is `high` on all surfaces, including the Claude API and Claude Code"); "ultracode" is NOT an API level — it is `xhigh` + standing multi-agent permission injected via mid-conversation system messages. Observability: `usage.output_tokens_details.thinking_tokens` splits billed reasoning from response (strictly better than the Baseline subtraction method). `display: omitted/summarized` never changes billing.
...
- **Validation protocol:** fixed Claude Code task set at low/medium/high/xhigh; read `output_tokens_details.thinking_tokens` per run; report tokens + pass-rate per level. This closes the dossier's biggest missing number and re-bases the 54.8% n=1. See [16 — Validation Harness: the No-Quality-Loss Proof Protocol](/research/context/techniques/16-validation-harness/).

### 5. Prior-turn thinking retention — the hidden input-side cost of reasoning models
- **Layer:** input / context lifecycle.
- **Mechanism:** Adaptive-thinking page (verbatim): "whether the API keeps or strips them depends on the model: Opus 4.5+ and Sonnet 4.6+ keep them in context by default; earlier Opus/Sonnet models and all Haiku models strip them"; pricing section bills "Thinking blocks from prior assistant turns kept in context: ... all turns by default on Opus 4.5+ and Sonnet 4.6+ (input tokens)". Reclaim via `clear_thinking_20251015` (keep: N). The context-windows page agrees: "On Claude Opus 4.5 and later Opus models, Claude Sonnet 4.6 and later Sonnet models, Claude Fable 5, Claude Mythos 5, and Claude Mythos Preview, the API keeps previous thinking blocks by default... On earlier Opus and Sonnet models and all Haiku models, the API automatically strips previous thinking blocks." LIVE CONTRADICTION (one page): the token-counting page says prior-turn thinking "do not count toward your input tokens" (plausibly true for the counting endpoint specifically, while create keeps and bills).
...
- **Availability:** CLAUDE-CODE-TODAY (the measured environment's harness runs 37 deferred tools behind a ToolSearch tool) + SDK.
...
- **Mechanism:** Requires `code_execution_20260120` or later (`code_execution_20260521` — same runtime, discloses a 90s per-cell limit — is accepted and interchangeable in `allowed_callers`); tools opt in via `allowed_callers: ["code_execution_20260120"]` (guidance, "not a security boundary"). Claude writes a script in the sandbox; each in-script tool call pauses execution, client supplies tool_result, script resumes. Verbatim: "Tool results from programmatic invocations do not count toward your input/output token usage. Only the final code execution result and Claude's response count." Platforms: Claude API, Claude Platform on AWS, MS Foundry — NOT Bedrock/Vertex. Exclusions: MCP-connector tools, `strict: true` tools.
...
- **Composability:** see records 2-3 for the quality counterweight; see [06 — Context architecture — what enters the window at all](/research/context/techniques/06-context-architecture/).
...
- **Availability:** SDK (and trivially scriptable from Claude Code, as done here — [checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes)).
...
For jackin❯, the provider-level features, current and beta evidence identifies which mechanisms are safe to adopt directly and which still require workload-specific validation.
...
- KILL (nuanced) "prior-turn thinking is always stripped, so reasoning never costs input" — false on Opus 4.5+/Sonnet 4.6+/Fable 5 (kept, billed as input). One Anthropic page still propagates the dead claim: context-windows is corrected to per-model behavior; only token-counting still carries it.
...
6. **Independent replication** — ALL headline percentages (84/39/29, >85, 38/24, 20-40) are vendor self-reported on internal evals; the τ²-bench negative result raises confidence in honesty, but no third-party replication is available in the cited evidence (T1-vendor, not T3).
7. **Fast mode** as a spend-more lever (Opus 4.8 at $10/$50 — Fable 5 sticker for an Opus model; Opus 4.7 at $30/$150 = 6x, deprecated with removal 2026-07-24; Opus 4.6 fast mode ended 2026-06-29 and runs/bills standard; research preview, no batch, stacks with caching) deserves a quality-per-dollar comparison vs Fable 5 at high effort — only Opus 4.8 remains long-term.
...
| Fast mode: Opus 4.8 $10/$50; Opus 4.7 $30/$150 (deprecated, removed 2026-07-24); Opus 4.6 fast mode ended 2026-06-29 and runs/bills standard; research preview; stacks with caching/data-residency; no batch | same pricing page |
| Tool-use system tax table (290/410, 675/804, 497/589, 496/588, Sonnet 5 354/474; Fable 5 absent); bash +245; text editor +700; computer use 735 & 466-499; web search $10/1k; web fetch $0, ~2,500/~25k/~125k tok examples | same pricing page |
...
| Tool search: ~55k example, ">over 85%" reduction, 30-50 tool degradation, "prefix is untouched, so prompt caching is preserved", 10,000 max, "up to 5 matching tools by default", &lt;10-tools guidance, no beta header; tool-use examples compatible (input_examples expand on discovery); defer_loading tools reject cache_control with 400 | platform.claude.com/docs/en/agents-and-tools/tool-use/tool-search-tool |
...
| Structured outputs: output_config.format, 24h grammar cache, structure-only invalidation, 20 strict tools, deprecated beta header | platform.claude.com/docs/en/build-with-claude/structured-outputs |
| token-efficient-tools / output-128k headers "have no effect" on Claude 4+ | platform.claude.com migration guide |
...
| OpenAI: automatic ≥1,024 tok, free writes, "up to 90%" input cost / "up to 80%" latency, 5-10min→1h + 24h extended retention free, prompt_cache_key | developers.openai.com/api/docs/guides/prompt-caching |
| Gemini: implicit default on 2.5+ (2,048/4,096 min), 0.1x explicit reads ($0.125-$0.20 vs $1.25-$2.00), storage $1.00/$4.50 per MTok/hr, batch 50% | ai.google.dev/gemini-api/docs/caching + /pricing |
| Tokenizer deltas +37.9% prose / +27.7% Rust / +8.9% JSON; Fable 5 ≡ Opus 4.8; Sonnet 4.6 ≡ Haiku 4.5; 2.81 vs 3.88 chars/tok | local measurement, [checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes) vs count_tokens API (samples: repo README/stories.rs/8x get_weather JSON) |
| Tool overhead: 403/19 (Fable 5 & Opus 4.8), 793/24 (4.7), 587/16 (Sonnet 4.6), 586/16 (Haiku 4.5) | local measurement, [checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes) with --tools (exact match to docs' 403 example) |
| count_tokens rejects fabricated thinking signatures (400) | local measurement, [checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes) |
| 54.8% thinking share; 0.44/6.73/92.83% prompt mix; 32/29/20/17/2% dollar split; ~$22/day; 1,420 vs ~60 tok MCP schemas; caveman/wenyan cuts | local Baseline measurements (see [01 — Token Economics and Measurement](/research/context/techniques/01-economics-and-measurement/), [02 — Baseline audit of the research environment](/research/context/techniques/02-baseline-audit/)) |
