---
title: "08 — Retrieval, memory, and state offloading"
description: "Comparison of selective retrieval, repository maps, memory stores, state handles, and context-offloading techniques."
---

## Summary

Selective repository retrieval has stronger evidence than autonomous memory injection; durable state should preserve provenance and require retrieval rather than entering every prompt.
**TL;DR**

- The only vendor-published token numbers in this space: Anthropic's context editing cut token consumption **84%** in a 100-turn web-search eval and improved task performance **29%** alone, **39%** combined with the memory tool (claude.com/blog/context-management). T1, but a search-heavy self-eval — not coding — and the docs themselves warn each clear invalidates cached prefixes.
...
When do retrieval, scoped instructions, handoffs, and durable memory beat replaying full files or transcripts?

## Method

### Local measurements

Repository files and constructed samples use `python3 docs/content/research/context/techniques/tools/count_tokens.py --model claude-fable-5 file sample FILE`.

| Item | Tokens | Note |
|---|---|---|
| <RepoFile path="AGENTS.md">AGENTS.md</RepoFile> (always-on index, 75 lines) | 2,744 | measured with the checked-in counter |
...
| <RepoFile path="docs/AGENTS.md">docs/AGENTS.md</RepoFile> (subtree-scoped) | 5,982 | observed only after reading a file under `docs/` |
...
| Pointer to the same decision (path + gist) | 53 | **77.3% cut**; independent local sample: 251→52 = 79.3% |
...
On the modeled session profile (local baseline measurement: ~6 sessions/day, 19 API calls/session, 5.5k uncached input, 85k cache-write, 1.17M cache-read, 27k output, ~$22/day), retrieval/memory/state techniques act on the input side: cache reads 32% + cache writes 29% + uncached input 2% = **63% of dollars**. The mechanism is always the same: replace re-replay (transcript) and re-derivation (re-exploring the codebase) with recall from a cheap external store — disk files, a memory directory, or a billed summary. Output-side techniques are in [09 — Output discipline and structured generation](/research/context/techniques/09-output-discipline/); cache mechanics in [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/); the measurement harness in [16 — Validation Harness: the No-Quality-Loss Proof Protocol](/research/context/techniques/16-validation-harness/).
...
- **Evidence tier:** T1 (documented harness behavior, code.claude.com/docs/en/memory + /context-window) + local measurement (table above; lazy injection of docs/CLAUDE.md observed in the measured environment).
...
- **Composability:** multiplies with prompt caching (smaller stable prefix, see [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/)); survives /compact (root index re-injected from disk); topic files compress further with caveman-compress (local baseline: 58.5% token cut on prose). Anti-synergy: none known.
...
- **Evidence tier:** T1 (shipped behavior, precisely documented; survival table verified verbatim by live fetch)
...
- **Expected savings:** vendor: qualitative only — no published numbers (T1 as practice, quantitatively T4/local). Local proxies ([checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes)): realistic 21-line handoff file = **387 tokens**; booting from it replaces transcript resumption or re-exploration that costs tens of thousands of input tokens (ESTIMATE: re-reading 5 source files ≈ 10–40k tok; a /resume replays the full prior conversation). Per-item recall is technique 5's 77.3%.
...
- **Validation protocol:** same multi-session feature three ways: (a) handoff file + /clear, (b) /resume transcript replay, (c) ride auto-compact. Sum billed tokens per arm (OTLP per-session), grade task completion. No one — including Anthropic — has published this comparison; see [16 — Validation Harness: the No-Quality-Loss Proof Protocol](/research/context/techniques/16-validation-harness/).
...
- **Expected savings:** local measurement: restatement 233 tok vs pointer 53 tok = **77.3% per recalled item**; an independent local sample measured 79.3%. Kills the "a pointer is ~10 tokens" folklore — useful pointers carry path + gist. Scales with restatement frequency (unmeasured; needs transcript mining). Cache side-benefit: pointers are often byte-identical across turns, restatements never are — preserves prefix stability ([07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/)). Manus frames the payoff via cache economics: cached input 10x cheaper (their $0.30 vs $3.00/MTok matches Anthropic's 0.1x cache-read ratio, cross-checked against reference pricing); "KV-cache hit rate is the single most important metric for a production-stage AI agent"; Manus input:output ≈ 100:1 (vendor figure, no methodology).
...
### 6. Anthropic API memory tool (memory_20250818, GA) — cross-session file memory
...
- **Mechanism:** tool type `memory_20250818` with view/create/str_replace/insert/delete/rename against a directory you host. Enabling it auto-injects a protocol prompt ("ALWAYS VIEW YOUR MEMORY DIRECTORY BEFORE DOING ANYTHING ELSE... ASSUME INTERRUPTION: Your context window might be reset at any moment") — locally measured at **189 tokens** (Fable 5, [checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes)) of standing cost once enabled. Docs prescribe the multi-session pattern (initializer session → progress log → end-of-session update) and position memory as what "preserves the information that must survive summarization" (Using with compaction section). ZDR-eligible. (platform.claude.com/docs/en/agents-and-tools/tool-use/memory-tool.)
- **Expected savings:** vendor: memory + context editing = **+39% task performance** over baseline on an internal agentic-search eval (claude.com/blog/context-management); token savings never isolated for memory alone — the 84% belongs to context editing. Recall-vs-rederive savings unquantified by anyone. Memory writes bill as output (~5x input).
...
- **Availability:** SDK (Developer Platform, Bedrock, Vertex); generally available on the Messages API, no beta header required — tool type `memory_20250818`, all Claude 4+ models; SDK helpers remain in beta namespaces despite GA. NOT-USER-ACCESSIBLE inside Claude Code — auto memory (technique 3) is the product analogue.
...
- **Expected savings:** Mem0 paper (abstract): "26% relative improvements... over OpenAI" (66.9% vs 52.9% LLM-as-Judge on LOCOMO, paper body), "91% lower p95 latency" (1.44s vs 17.12s), "saves more than 90% token cost" (~1.8K vs 26K tokens/conversation = 93.1%). Counter-evidence: Letta — filesystem agent on gpt-4o-mini scores **74.0%** on LoCoMo vs "Mem0's reported 68.5%... top-performing graph variant"; Letta could not reproduce the MemGPT numbers and "Mem0 did not respond to requests for clarification" (letta.com). ConvoMem (arXiv 2511.10523, 75,336 QA pairs): "simple full-context approaches achieve 70-82%" while "Mem0 achieve only 30-45%... under 150 interactions"; long context "excels for the first 30 conversations, remains viable... up to 150".
...
- **Expected savings:** up to 68.8% of calls *on FAQ-shaped traffic only*. For Claude Code-like traffic: ~zero expected — prompts embed file contents, diffs, session state; superficially similar queries ("fix the test") demand different answers. The local baseline mix (92.83% of heavy-session input = prefix cache reads) shows coding-agent repetition lives in the PREFIX, which Anthropic prompt caching already prices at 0.1x — the whole-response repetition this technique needs does not exist in the workload.
...
For jackin❯, the retrieval, memory, and state offloading evidence identifies which mechanisms are safe to adopt directly and which still require workload-specific validation.
...
| "Semantic response caching saves money for coding agents" | KILL | T4, zero evidence; local baseline: repetition is in the prefix (92.83% cache reads), already priced at 0.1x |
...
|---|---|
| 84% token cut, 100-turn web-search eval; +39% memory+editing; +29% editing alone; | https://claude.com/blog/context-management — quotes verified by live fetch |
...
| memory_20250818 commands, injected protocol prompt text, multi-session pattern, "preserves the information that must survive summarization" quote, GA (no beta header), ZDR, no perf numbers on page | https://platform.claude.com/docs/en/agents-and-tools/tool-use/memory-tool — live fetch |
| Injected memory protocol prompt = 189 tok; handoff sample = 387 tok; restatement 233 vs pointer 53 (77.3%) | Checked-in token counter on the quoted and constructed samples described above |
| <RepoFile path="AGENTS.md">AGENTS.md</RepoFile> 2,744; 12 topic files 35,769 (max <RepoFile path="PULL_REQUESTS.md">PULL_REQUESTS.md</RepoFile> 10,939); <RepoFile path="docs/AGENTS.md">docs/AGENTS.md</RepoFile> 5,982; 93.8%/92.9% deferred | local: [checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes) per file, totals summed; measured with the checked-in counter of <RepoFile path="AGENTS.md">AGENTS.md</RepoFile> |
| +$1.27/session, +$7.64/day (+35%) all-eager counterfactual; auto-memory tax $0.02–$0.23/session ($0.12–$1.40/day); worst case ≈7,642 tok (25,600 chars ÷ 3.35 chars/tok) | local arithmetic, ESTIMATE, on modeled profile (19 calls, 6 sessions/day, $22/day) and baseline Fable-5 prose ratio |
| Break-even N* ≈ 11.5 × S / C (examples 4.3 and 138 calls) | local arithmetic, ESTIMATE: cache write $12.5/MTok vs read $1/MTok |
| Heavy-session mix 92.83% cache reads / 0.44% uncached; dollar split 32/29/20/17/2; 11 MCP schemas 1,420 tok vs ~60 deferred; caveman ultra 58.5% | local baseline measurements (treated as ground truth) |
...
| Zep critique exists ("Lies, Damn Lies, Statistics") | https://blog.getzep.com/lies-damn-lies-statistics-is-mem0-really-sota-in-agent-memory/ |
