---
title: "19 — Subscription and quota economics"
description: "How subscription limits, cache weighting, reset windows, routing, and overage change token-optimization priorities."
---

## Summary

Subscription work is constrained by rolling usage windows rather than API dollars until overage begins. This chapter builds the quota-weighted model, reorders the dossier's levers under it, and states what providers do not publish.
...
- **For a flat-rate subscriber the optimization target flips from "$ per task" to "tasks per cap."**
 ...
 cap you spill into **usage credits billed at standard API rates** — so the core dossier's dollar model
...
- **The lever order re-sorts under quota.** Anthropic's own stated top cap-drains are (1) **expensive
 ...
 inverts* the core dossier's dollar-era praise for heavy subagent fan-out (subagents were ~61% of calls in
 one measured account).
- **Current verified subscription facts:** 5-hour limits **doubled and the peak-hours throttle was removed** (announced and effective 2026-05-06); **all consumption counters were reset** — a sub-claim resting only on the morphllm.com secondary source (the announcement page does not mention counter resets); **Opus-specific caps were removed** in Opus 4.5 (Opus now draws the general weekly limit; Max keeps a separate *Sonnet-only* weekly limit); subscription context is **capped at 200K** even though the API exposes 1M; and the announced non-interactive Agent-SDK/headless billing split (separate API-rate credits, off the interactive cap) is **PAUSED** — "For now, nothing has changed: Claude Agent SDK, claude -p, and third-party app usage still draw from your subscription's usage limits" (support article 15036540).
- **What you can measure vs. what you can't:** `/usage` reports session-% and weekly-% of cap
 ...
 the cap**. Local JSONL transcripts give token classes (this account, the measurement: 33.5M cache-read /
...
Dollar arithmetic, where used, follows the core dossier's $22/day profile; the quota model is the governing axis for subscription work.

---

## Question and scope

How do opaque subscription windows change the optimal techniques when accepted tasks per quota, not API dollars, is the binding metric?

## Method

The analysis separates published plan and limit facts from inferred quota weights, combines them with local usage-field measurements, and marks any denominator the provider does not expose as unknown.
...
| Unit you optimize | **% of window per task** | $ per task (the core dossier) |
...
The decisive consequence: **a dossier "saving" that only reduces dollars changes nothing for a
...
1. **Expensive cache *misses* on stale large-context sessions.** A 1M-context session that loses its
 ...
 1M context window, which the core dossier correctly noted has *no dollar premium* (00 graveyard #4), a
...
### How the core dossier's levers re-rank under the quota metric

| Lever (core-dossier view) | Dollar verdict (the core dossier) | Quota verdict (this file) | Change |
|---|---|---|---|
...
- **Evidence scope:** The core dossier's measurement file (01) and `ccusage` coverage (03 record 18)
...
- **Quality risk:** **NEUTRAL.** A transparent proxy that mis-handles `cache_control` could *harm*
 caching — verify it passes breakpoints through ([03 — Prior art and market scan](/research/context/techniques/03-prior-art-and-market-scan/) record 17).
...
- **Evidence scope:** the core dossier covers the eight cache busters (13 tech 1) as a *dollar* lever
 ($2.30–3.80/bust); their **quota** cost (a miss converts a 0.1× read into a 1.25–2× write *against
 the cap*) is new, and Anthropic's own attribution of cap drain to misses is later evidence.
...
- **Composability:** precondition for Q3/Q5; same discipline as [13 — Infrastructure-level](/research/context/techniques/13-infrastructure-level/) tech 1 with a bigger payoff.
...
- **Evidence scope:** Provider-specific environment control; the core dossier discusses
...
The dollar model favors fan-out as cache arbitrage; against a cap, call count is itself a drain.

- **Evidence scope:** **Quota-specific reinterpretation** of [13 — Infrastructure-level](/research/context/techniques/13-infrastructure-level/) tech 4 / 16. The dollar-model break-even is in
 dollars; the quota break-even is different because each spawn writes a prefix and subagents were
 ≈61% of calls in a measured account — a fact later.
...
- **Expected savings:** negative if over-spawned; the lever is *restraint* — spawn when a subagent
 genuinely quarantines a large read it will reuse, not reflexively. Reuse the core dossier's break-even but
...
- **Quality risk:** **NEUTRAL-to-RISKY** — too few subagents re-rots the main context (12); too many
 drains the cap. The quality-safe point is the core dossier's, re-costed.
- **Availability:** CLAUDE-CODE-TODAY.
...
- **Evidence scope:** [13 — Infrastructure-level](/research/context/techniques/13-infrastructure-level/) tech 2 covers the 1h/5m split as a *dollar* lever; the
 quota-specific nuances (overage silently reverts to 5m; the v2.1.90/v2.1.106 bugs; Cherny's "small
 win" framing) are later and reframe it for the cap.
- **Layer:** cache / config.
...
The core dossier's dollar model.

- **Evidence scope:** The core dossier never models the cap edge because it never models the cap.
...
- **Mechanism:** at the 5-hour or weekly limit, you either wait for reset (5-hour = rolling, frees
 ...
 rates** for the spillover — at which point every core-dossier dollar lever (effort, routing, batch,
...
- **Expected savings:** converts an otherwise-blocked session into either zero-dollar wait (time cost,
 see 43) or API-priced work (dollar cost, the core dossier model). The decision is a time-vs-dollar trade.
...
### Q7. Headless and SDK work remains inside the interactive cap

Anthropic paused the announced usage split. The support article says, "For now, nothing has changed: Claude Agent SDK, claude -p, and third-party app usage still draw from your subscription's usage limits." The separate monthly credit is not live behavior; headless usage drains the same interactive cap. https://support.claude.com/en/articles/15036540-use-the-claude-agent-sdk-with-your-claude-plan

- **Evidence scope:** current provider policy for non-interactive subscription usage.
- **Layer:** infra / workload placement.
- **Mechanism:** non-interactive usage through the Agent SDK, headless `claude -p`, Claude Code GitHub Actions, and third-party Agent-SDK apps draws from the subscription's existing usage limits.
- **Expected savings:** none. Moving work to a headless or SDK path does not protect the interactive cap while the split remains paused.
- **Evidence tier:** T1 (support.claude.com agent-SDK-with-your-plan).
- **Quality risk:** **NEUTRAL.**
- **Availability:** no off-cap subscription path is currently available.
- **Effort to adopt:** none until provider policy changes.
- **Composability:** batch and fleet execution still share the same subscription cap.
- **Validation protocol:** confirm current policy in the provider support article before treating any non-interactive path as off-cap.
...
the measurement (vs #24147's 1,310:1 over 30 real-usage days — the ratio is workload- and
...
- The 1M context window — marketed as a capability and shown by the core dossier to carry **no dollar
...
For jackin❯, the subscription and quota economics evidence identifies which mechanisms are safe to adopt directly and which still require workload-specific validation.
...
| 2 | 5-hour rolling session limit; weekly limit(s); Max has all-model + Sonnet-only weekly; fixed weekly reset anchor | support.claude.com what-is-the-max-plan / models-usage-and-limits-in-claude-code (5-hour reset evidenced by support article 12429409 — "session limits continue to reset every five hours" — and weekly structure by article 11049741; article 14552983 was restructured and no longer states window durations) |
| 3 | Opus-specific caps removed (Opus 4.5); Opus draws general weekly limit | anthropic.com/news/claude-opus-4-5 |
| 4 | 5-hour limits doubled + peak-hours throttle removed; counters reset | anthropic.com/news/higher-limits-spacex; morphllm.com/claude-code-usage-limits (doubling + throttle removal announced and effective 2026-05-06; the announcement page does not mention counter resets — that sub-claim rests only on the morphllm.com secondary source) |
...
| 6 | Overage = usage credits at standard API rates; $2,000/day redemption cap; optional monthly cap + auto-reload | support.claude.com manage-extra-usage (the page is titled "Manage usage credits for paid Claude plans", article 12429409) |
...
| 13 | Agent-SDK/headless usage split off the cap into separate API-rate credit | support.claude.com use-the-claude-agent-sdk-with-your-claude-plan (split PAUSED — "For now, nothing has changed: Claude Agent SDK, claude -p, and third-party app usage still draw from your subscription's usage limits"; the separate credit is a suspended plan, not live behavior) |
