---
title: "20 — Multimodal token economics: images, screenshots, PDFs"
description: "Measured token costs and practical routing, resizing, cropping, and format choices for images, screenshots, and PDFs."
---

## Summary

Images, screenshots, and PDFs have distinct token costs and routing options. Every number below comes from a cited primary source or a reproducible local `count_tokens` measurement with the method shown.
...
- **An image costs `⌈width/28⌉ × ⌈height/28⌉` visual tokens** (28-pixel patches), billed at the
 ...
 1,304). The core dossier never stated the formula.
- **The per-image cap differs ~3× across the two tokenizer families, and it is a routing lever.**
 ...
 land around **~4,760 / ~1,520–1,570**, i.e. **~3.0–3.1×**. Haiku 4.5 ≡ Sonnet 4.6 on images (identical counts), extending the core dossier's tokenizer-family
 equality to vision.
...
- **Net new levers for the stack:** route vision work to Sonnet/Haiku (~3.0–3.1× image saving),
 ...
 NEGATIVE-COST-to-NEUTRAL on quality. None appear in the core dossier.

The measurements below use the exact model identifiers recorded in each table. Revalidate tokenizer-family equivalence before transferring counts to another model.

---

## Question and scope

When are images, screenshots, or PDFs cheaper and more faithful than text, and how do model-specific visual caps change routing?
...
- **PNGs** at controlled dimensions via Python `zlib`: a valid RGB PNG with a gradient
 (non-degenerate) so token cost reflects dimensions, not blank-image special cases.
- **PDFs** with byte-offset-correct xref tables via Python `zlib`/`struct`: N text pages
 of Helvetica lines, optionally a final embedded-image page (the scanned-PDF case).
- **Image token counts** via the checked-in <RepoFile path="docs/content/research/context/techniques/tools/image_tokens.py" /> workflow, which constructs controlled PNGs and sends real `image` content blocks.
- **PDF token counts** via the same provider endpoint with `document` content blocks. The fixture procedure constructs a PDF 1.4 byte stream with a catalog, pages tree, Helvetica font, one content stream per page, byte-offset-correct xref table, and either 5 or 50 repeated text lines per page; page counts are 1, 3, 10, and 25. The identical 50-line string is also counted as plain text for the tax comparison.

Counts are non-billable and cache-inert (the core dossier, [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/)). Raw `input_tokens` includes a constant ~6–8-token user-message wrapper (the core dossier
...
- **Evidence scope:** The core dossier's routing file (10) and tokenizer file (05) cover the **text**
  premium but never images; "image"/"vision" is absent from both. The image cap divergence is a distinct,
  larger (~3.0–3.1×) effect.
...
- **Expected savings:** per full-frame screenshot, roughly 4,760 → 1,520–1,570 tokens = **~−67%** on the image
 ...
 large in **quota** ([19 — Subscription and quota economics](/research/context/techniques/19-subscription-and-quota-economics/)) and in window pressure. A 25-page PDF: 78,806 → 68,804 tokens
...
- **Composability:** stacks with the core dossier's tokenizer-arbitrage routing (05/10) and subagent fan-out
 (07 tech 4); the image-handling subagent quarantines the pixels off the main prefix.
...
- **Evidence scope:** No resolution/detail control appears anywhere in the core dossier (0 hits).
...
- **Evidence scope:** Multimodal axis. The core dossier's context-architecture file (06) argues "don't send it" for
...
- **Composability:** the multimodal sibling of the core dossier's preprocessing/CLI-over-MCP (03 record 20)
 and repo-maps (06).
...
- **Evidence scope:** The core dossier's only PDF reference is the "~125k tok/500 kB" page-size estimate
 (03:267, 12:165); the per-page mechanism and the text-vs-PDF tax are unmeasured there.
...
- **Evidence scope:** no cropping or region-selection discussion appears in the earlier chapters.
- **Layer:** input (image token class).
...
- **Evidence scope:** chapter 06 applies lazy loading to tools and skills, not vision inputs.
...
- **Quality risk:** **NEUTRAL-to-NEGATIVE-COST** — fewer stale frames is also less context rot
 (06). RISKY only if a needed visual is skipped. Falsify by tracking tasks that failed for lack of a
 screenshot.
...
- **Composability:** pairs with context editing/observation masking ([06 — Context architecture](/research/context/techniques/06-context-architecture/)/12) applied to image
 blocks specifically.
...
- Haiku 4.5 and Sonnet 4.6 return byte-identical image counts, just as the core dossier found for text —
...
For jackin❯, the multimodal token economics: images, screenshots, pdfs evidence identifies which mechanisms are safe to adopt directly and which still require workload-specific validation.
...
| 4 | Measured image curve (256²=108/110 … 1000²=1,304/1,306 … capped rows around Opus ~4,750–4,792 / Sonnet-Haiku ~1,531–1,574; practical divergence ~3.0–3.1×) | <RepoFile path="docs/content/research/context/techniques/tools/image_tokens.py" /> on claude-opus-4-8 / claude-sonnet-4-6 / claude-haiku-4-5 |
...
| 6 | PDF: 1pg×5ln = 1,742/1,700; 1pg×50ln = 3,182/2,780; 3/10/25 pg = 9,484/31,541/78,806 (Opus, ~3,150 tok/pg); 2txt+1img = 7,886/7,083 | Reproducible PDF 1.4 construction and `document` counting procedure specified in Method |
...
| 9 | Screenful as text: dense Rust (~2 KB) Opus 765 / Sonnet 593; wide markdown (~4.6 KB) Opus 1,951 / Sonnet 1,468 | count_tokens on real repo files (<RepoFile path="crates/jackin-capsule/src/git_context.rs">crates/jackin-capsule/src/git_context.rs</RepoFile> L100-149; [03 — Prior art and market scan](/research/context/techniques/03-prior-art-and-market-scan/) L1-50) |
