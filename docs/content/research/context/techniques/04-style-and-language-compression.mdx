...
description: "Measured effects and quality limits of concise registers, prompt compression, token budgets, and symbolic language."
---

## Summary

Concise output registers can materially reduce visible tokens, but symbolic prompt languages add tokenizer cost and obedience risk while leaving hidden thinking untouched.
**TL;DR**

- Register compression of the model's visible replies is the only style lever worth real money: locally measured **−54% to −67%** tokens on instruction/reply ladders (Fable tokenizer, method below), consistent with baseline's session-level **caveman-ultra = 58.5%** (its "~75%" marketing claim is character-level folklore). On the modeled session profile that is **~10% of session dollars, hard-capped at 17%** even at total muteness.
...
Cross-references: dollar split and pricing in [01 — Token Economics and Measurement](/research/context/techniques/01-economics-and-measurement/); always-on token inventory in [02 — Baseline audit of the research environment](/research/context/techniques/02-baseline-audit/); instruction-side architecture in [06 — Context architecture — what enters the window at all](/research/context/techniques/06-context-architecture/); the output-side sibling levers in [09 — Output discipline and structured generation](/research/context/techniques/09-output-discipline/); experiment harness in [16 — Validation Harness: the No-Quality-Loss Proof Protocol](/research/context/techniques/16-validation-harness/).

## Question and scope

How far can terse language, notation, and output registers reduce tokens before comprehension or fidelity degrades?

## Method

The analysis counts equivalent prose, telegraphic, and notation samples with the checked-in token counter, then compares those token results with published comprehension and fidelity evidence.

## Findings

### Why output register is the only style slice worth real money

Per heavy modeled session (local baseline measurement, scaled): ~19 API calls, 5.5k uncached input, 85k cache-write, 1.17M cache-read, 27k output (55% thinking at max effort); ~$3.67/session, ~$22/heavy day at Fable 5 list prices. Dollar split: cache reads 32% / cache writes 29% / thinking 20% / visible output 17% / uncached input 2%.
...
Method: the free `count_tokens` API via the [checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes). Each prose, telegraphic, and notation sample preserves the same facts; the tables record the compared text and labels. Per-text tokens equal raw count minus the 7-token message overhead, calibrated with `hello` = 8 raw and `hello world` = 9 raw; per-symbol or word costs divide the overhead-corrected count of 10 space-prefixed repetitions by 10. Absolute counts vary by about one token; relative percentages are robust.
...
**Notation shootout** (same content): glyph DSL "↹ logs.csv ⊕ filter(level≥ERROR) ⇒ Σ summary ∴ top3 causes" = **42 tok**; plain English = **35 tok**; telegraphic ASCII "Read logs.csv. Filter level>=ERROR. Summarize. Top 3 causes." = **31 tok**. The DSL loses to plain prose by +20% and to telegraphic by +35%. A second sample measured DSL 36 / prose 40 / telegraphic 26: the DSL narrowly beat prose but lost to telegraphic by +38%. Across both samples, telegraphic ASCII used about 26–28% fewer tokens than the DSL.
...
**Wenyan / classical Chinese**: the original classical-Chinese rendering of "if all tests pass then commit" measured **9 tokens** versus **8 tokens** for the English phrase (net negative; original non-English text omitted). A longer rule measured 32 tokens in wenyan, 30 in plain English, and 25 in telegraphic English. Classical CJK ran **0.89–0.91 chars/token** here (at least one token per character). Characters are not tokens, and wenyan's measured savings are registral (extreme word-dropping), not tokenizer efficiency.
...
**Cross-tokenizer transfer** (same texts on `claude-sonnet-4-6`): polite-full 72 tok Fable vs 59 Sonnet (**Fable +22.0%**); telegraphic 29 vs 21 (**+38.1%**) — corroborates baseline's +15–38%. The relative ladder cut transfers: −59.7% (Fable) vs −64.4% (Sonnet 4.6). Any published per-token claim measured on another tokenizer does not transfer in absolute terms.
...
- **Expected savings:** 50–67% of visible-output tokens (local ladder, table above; baseline session-level caveman-ultra 58.5%). Modeled profile: 0.60 × 17% visible-output dollar share ≈ **10.2% of session dollars** ≈ $0.37/session, $2.2/heavy day (ESTIMATE, arithmetic shown).
- **Evidence tier:** T1 (method above) + baseline session measurement + live product docs for the mechanism.
...
- **Evidence tier:** T2 (peer-reviewed venue FLLM 2024, verified on the arXiv page). Staleness caveat: Jan-2024 GPT-3.5/4 era, pre-reasoning models.
...
- **Mechanism:** TALE-EP pre-estimates a per-problem budget with a cheap zero-shot estimator call, then injects it ("use fewer than N tokens" ≈ 17 tok locally). Exact figures (arXiv 2412.18547, v5): ~67% token reduction with &lt;3% accuracy decrease (avg 81.03% vs 83.75%); GSM8K accuracy RISES 81.35% → 84.46% while output falls 318.10 → 77.26 tok (−75.7%). Token elasticity: a 10-token budget produced 157 actual tokens vs 86 at a 50-token budget — under-budgeting nearly doubles real cost.
...
- **Evidence tier:** T2 (ACL Findings 2025 publication; linked implementation repository is no longer public).
...
- **Validation protocol:** test budgets `25, 50, 100, 200, none` over 100 fixed problems; plot actual `usage.output_tokens` and accuracy per budget to locate your elasticity knee before deploying any budget phrasing; re-run quarterly (model updates move the knee).
...
- **Validation protocol:** count the same content in DSL, prose, and telegraphic forms with the [checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes). To test comprehension, present 30 matched instructions in glyph notation and English, then score execution fidelity blind; require English-equivalent fidelity. MetaGlyph's 26%-fidelity result needs replication on Fable 5 before being treated as stronger than T4.
...
The most aggressive register measured (wenyan-ultra −74.5% tokens, baseline) marks the practical ceiling — bought with maximal unmeasured comprehension risk, and short phrases can cost MORE than English.

- **Layer:** output (extreme register rung).
- **Mechanism:** baseline local: wenyan-full = 80.9% character cut but only **56.6% token cut**; wenyan-ultra = **74.5% token cut**. This dossier's spot checks: 9 vs 8 tok against plain English on a short phrase (net NEGATIVE), 32 vs 25 against telegraphic English on a longer rule, and classical chars at 0.89–0.91 chars/token — the compression is extreme word-dropping in wenyan grammar, not cheap characters.
- **Expected savings:** 56.6–74.5% of visible-prose tokens (baseline); ceiling arithmetic: 0.745 × 17% ≈ **12.7% of session dollars**, i.e. +2.8pp over caveman-ultra's 9.9% — the entire wenyan increment over caveman is worth ~$0.62/heavy day (ESTIMATE).
...
- **Mechanism:** local n=1 max-effort session: thinking = 54.8% of output tokens (consistent with the 20%-thinking/17%-visible dollar split: 54.8/45.2 ≈ 20/17). Live platform docs (extended-thinking page): "You're charged for the full thinking tokens generated by the original request, not the summary tokens"; on Fable 5 "extended thinking is always enabled and cannot be disabled... `thinking: {type: "disabled"}` returns an error"; `budget_tokens` deprecated in favor of effort. Live effort page: "Effort is the primary control for trading off intelligence, latency, and cost on Claude Fable 5"; effort "affects **all tokens** in the response", including "Extended thinking"; lower effort documentedly performs register compression as a side effect — "Proceed directly to action without preamble", "Use terse confirmation messages after completion", "Combine multiple operations into fewer tool calls". **Current model-config evidence:** "Thinking cannot be turned off on Fable 5. The session toggle, `alwaysThinkingEnabled`, and `MAX_THINKING_TOKENS=0` have no effect there"; non-zero values apply only in the legacy fixed-budget mode on Opus 4.6/Sonnet 4.6; `ultrathink` merely "adds an in-context instruction. The effort level sent to the API is unchanged". **Prompt-control boundary:** "If you want Claude to think more or less often than the current level produces, you can say so directly in your prompt or in CLAUDE.md; the model responds to that guidance within its effort setting" — documented prompt-level control of thinking FREQUENCY (not per-block verbosity), within effort.
...
- **Mechanism:** prompt side of a heavy session is 92.83% cache-read (baseline); always-on text bills at $1/MTok after the first write. Live docs concur: "Adding instructions to the system prompt increases input tokens, though prompt caching reduces this cost after the first request in a session" (output-styles page). The `/caveman-compress` skill automates the rewrite (keeps `.original.md` backup).
...
- **Composability:** orthogonal to output-side techniques; **dominated by deferred tool loading on the same surface** (baseline: 11 MCP schemas = 1,420 tok loaded vs ~60 tok as deferred names — a bigger instruction-side win with zero comprehension risk; see [06 — Context architecture — what enters the window at all](/research/context/techniques/06-context-architecture/)).
...
For jackin❯, the style and language compression — beyond caveman evidence identifies which mechanisms are safe to adopt directly and which still require workload-specific validation.
...
|---|---|
| "Caveman mode cuts ~75% of tokens" | KILLED. Baseline: ultra = 58.5% token cut on visible prose; "~75%" is character-level. Use 55–63%, ≈10% of session dollars. |
...
| "Write output in (classical) Chinese for 75–80% savings" | KILLED (nuance). 80.9% character cut = 56.6% token cut (baseline); this dossier measured classical chars at ~0.9 chars/token and two samples where wenyan LOSES to plain/telegraphic English (9 vs 8; 32 vs 25 tok). |
...
6. **Cross-tokenizer transfer** of relative register savings held in two local pairs (−59.7% Fable vs −64.4% Sonnet 4.6; 51.4% vs 52.8%) but absolute counts differ +22–38%; no systematic study. The research field optimizes prompt-side compression while cached agent workloads make output-side register the 50x-leveraged target.
7. The baseline figures leaned on throughout (thinking 54.8%, caveman-ultra 58.5%, dollar split) are **n=1 session-level measurements** pending replication (see [16 — Validation Harness: the No-Quality-Loss Proof Protocol](/research/context/techniques/16-validation-harness/)).

## Sources

| Number | Source / method |
|---|---|
| Ladder −54.2 / −59.7 / −66.7%; reply ladder −60.0% | Matched samples described in Method, counted with the [checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes), with the 7-token envelope subtracted |
...
| CoD "as little as only 7.6% of the tokens" (v2) | https://arxiv.org/abs/2502.18600; per-task table (190.0→39.8 etc.) from https://arxiv.org/html/2502.18600v2 |
| CCoT 48.70% / 27.69% / 22.67%; venue FLLM 2024 pp. 476–483 | https://arxiv.org/abs/2401.05618 |
| TALE 67% / &lt;3%; GSM8K 318.10→77.26, 81.35→84.46%; elasticity 157 vs 86 tok (v5) | https://arxiv.org/html/2412.18547v5; publication record at https://aclanthology.org/2025.findings-acl.1274/ |
...
| MTOB 44.7/45.8 chrF vs human 51.6/57.0 | https://arxiv.org/abs/2309.16575 |
| NEO-BENCH "nearly halved... single neologism" | https://aclanthology.org/2024.acl-long.749/ |
| Distillation 147→56 tok; math non-distillability quote | https://arxiv.org/html/2407.06023v2 |
| Neologism learning = trained embeddings (not in-context) | https://arxiv.org/pdf/2512.18551 |
