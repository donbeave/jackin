...
description: "Model, effort, and delegation strategies that trade token cost, reasoning quality, latency, and coordination overhead."
---

## Summary

Explicit task-boundary routing and subagent pins can reduce cost without cache loss; mid-session switches and opaque gateway routing cannot.
**TL;DR**
...
Which tasks can move to cheaper models or delegated sessions without losing acceptance quality or paying more in retries and context?

## Method

The analysis counts fixed prose, code, rules, and tool-schema samples across model families, then combines price, cold-context, retry, and quality effects under a paired-task acceptance gate.
...
Method: deterministic samples use `python3 docs/content/research/context/techniques/tools/count_tokens.py --model MODEL file sample PATH`, one user message per call. Samples: first 2,000 B of <RepoFile path="crates/jackin-core/src/agent.rs">crates/jackin-core/src/agent.rs</RepoFile>; first 4,096 B and full 6,366 B of <RepoFile path="AGENTS.md">AGENTS.md</RepoFile>; a fixed 528 B English paragraph; and a 558 B JSON tool schema. Counts include a few tokens of message-wrapper overhead, so ratios are marginally understated.
...
- Baseline's root <RepoFile path="AGENTS.md">AGENTS.md</RepoFile> measurement (2,738 tok) reproduces as 2,744 on Fable (within 0.2%; wrapper overhead/file drift). The same always-on file is only 1,919 tokens on Sonnet/Haiku — repo instructions are 43% more expensive in tokens on Fable-tier models before the price ratio even applies.
...
- **Expected savings:** ESTIMATE on a 5-worker exploration fan-out, each 40K in / 3K out in Fable tokens: all-Fable = 5×(40K×$10 + 3K×$50)/1M = **$2.75**. Haiku with no tokenizer premium = **$0.275** (−90.0%); Haiku at the official/prose 1.3x divergence = $0.21 (−92.3%); repo-markdown-like 1.43x = **$0.19 (−93.0%)**. On the modeled heavy day ($22, Fable): two such fan-outs previously done inline at parent rates ≈ $5.50 → $0.38–0.55, ~22–23% off the day IF that work was being done at all (the context-isolation win compounds it; see [06 — Context architecture — what enters the window at all](/research/context/techniques/06-context-architecture/)).
...
- **Composability:** stacks with `effort: low` frontmatter, parent prompt caching (untouched), and caveman-style output compression ([09 — Output discipline and structured generation](/research/context/techniques/09-output-discipline/); the cavecrew agents in this repo compose both). Anti-synergy: fork mode (inherits parent model); `CLAUDE_CODE_SUBAGENT_MODEL` is global, so it also downtiers subagents you wanted strong.
...
- **Mechanism:** Explore is documented as "**Model**: Haiku (fast, low-latency)", read-only tools (Write/Edit denied). "Explore and Plan skip your CLAUDE.md files and the parent session's git status to keep research fast and inexpensive" — every other subagent loads both. Plan also skips CLAUDE.md but inherits the main model. Claude picks a thoroughness level per invocation (quick/medium/very thorough) — effort-like tiering inside the cheap tier. Separately, background functionality (`--resume` summarization etc.) runs on the `ANTHROPIC_DEFAULT_HAIKU_MODEL` alias and costs "typically under $0.04 per session" (T1, costs page).
- **Expected savings:** unquantified by Anthropic. The lever: exploration corpora bill at $1/$5 instead of $10/$50 and never enter the main context. On the modeled profile, prompt-side tokens are 92.8% cache reads (local baseline); Explore keeps new exploration off that meter entirely and onto Haiku's.
...
- **Composability:** explicitly cache-safe, so composes with all caching techniques ([07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/)). Complements technique 1: advisor = escalate UP at decision points; subagents = delegate DOWN for volume. Anti-synergy: Fable-main sessions; very long transcripts (consult tax grows linearly, uncached).
...
Before changing models, change effort: the same model at lower effort thinks less, preambles less, and makes fewer tool calls — and thinking is 54.8% of output tokens at max effort (local baseline), billed at 5x input price.
...
- **Evidence tier:** T1 mechanics; T4/ESTIMATE magnitudes; thinking share is local measurement (baseline).
...
- **Validation protocol:** 15 fixed tasks × `high, medium, low` on Sonnet 4.6; record output tokens, tool-call count, tests-pass rate. Publish the per-level % — it would be the first public number (see [16 — Validation Harness: the No-Quality-Loss Proof Protocol](/research/context/techniques/16-validation-harness/)).
...
- **Effort to adopt:** none for billing; minutes to re-derive your own multipliers with [checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes).
...
- **Evidence tier:** T1 (7x figure, guardrails, $13/day, $150–250/month, &lt;$30/day for 90% of users — costs page); orchestrator quote T1 (Haiku 4.5 announcement).
...
- **Composability:** combines techniques 1 + 5 (effort: low workers) + compressed worker output ([09 — Output discipline and structured generation](/research/context/techniques/09-output-discipline/)). Anti-synergy: parallel identical-prefix workers all pay full cache-write price unless the first request is staggered (API parallel-cache trap; see [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/)).
...
- **Composability:** precondition for every other technique's math; see [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/).
...
For jackin❯, the model routing and tiered delegation evidence identifies which mechanisms are safe to adopt directly and which still require workload-specific validation.
...
4. Tokenizer measurements are n=5 here (n=1 per content type in baseline); prose samples exceed the official 1.35x ceiling — a corpus-level sweep would tighten every multiplier.
5. The effort-invalidates-cache claim rests on the advisor documentation and its linked cache guidance; the evidence does not include an independent prompt-caching reproduction.
...
| Tokenizer table: 830/830/629/629 … 183/183/126/126; ratios 1.320–1.452x | Checked-in counter with explicit `--model`; samples described in Method |
| <RepoFile path="AGENTS.md">AGENTS.md</RepoFile> = 2,744 Fable tok (baseline: 2,738) / 1,919 Sonnet-Haiku tok | local [checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes) on repo <RepoFile path="AGENTS.md">AGENTS.md</RepoFile> (6,366 B) |
| Baseline: thinking 54.8% of output; prompt mix 0.44/6.73/92.83%; dollar split 32/29/20/17/2; ~$22/heavy day | local baseline measurement (see [01 — Token Economics and Measurement](/research/context/techniques/01-economics-and-measurement/)) |
...
| RouteLLM "up to 85%… like MT Bench"; 85/45/35 per-benchmark; 26%/14% GPT-4-call fractions; trained on gpt-4-1106-preview vs mixtral-8x7b; ICLR 2025 | https://github.com/lm-sys/RouteLLM/blob/main/README.md + https://www.lmsys.org/blog/2024-07-01-routellm/ |
...
| Martian "20% to 97%" | https://docs.withmartian.com/ (vendor claim, no methodology) |
