---
title: "02 — Baseline audit of the research environment"
description: "Measured baseline for agent prompts, tools, context, output, cache behavior, and tokenizer effects in the research environment."
---
...
- **Thinking is the majority of output spend here: ≈55% of output tokens** in the measured workload
...
- **Measured end-to-end value of style compression in the measured workload: ≈10% of dollars.** Visible
...
What does the measured local workload reveal about token distribution, standing context, and the reachable optimization ceiling?
...
`s1 / wenyan-full`: Classical-Chinese rendering of the same diagnosis; original non-English text omitted. (103 chars → 69 net tokens)

`s1 / wenyan-ultra`: More compressed classical-Chinese rendering; original non-English text omitted. (57 chars → 40 net tokens)
...
In the measured environment these tools are **deferred** (ToolSearch): the context carries only a name list
...
[05 — Tokenizer arbitrage](/research/context/techniques/05-tokenizer-arbitrage/) and [10 — Model routing and tiered delegation](/research/context/techniques/10-model-routing-and-delegation/) with more samples.
...
value rise; the effort sweep lives in [09 — Output discipline and structured generation](/research/context/techniques/09-output-discipline/).)
...
The aggregate was measured from the complete local transcript sample described in [01 — Token Economics and Measurement](/research/context/techniques/01-economics-and-measurement/).

### 7. Measured token and dollar distribution

Usage totals (deduplicated) priced at Fable 5 list rates ($10 in / $50 out / $12.50 cache-write
5-min / $1 cache-read per MTok — verified against live docs, see [01 — Token Economics and Measurement](/research/context/techniques/01-economics-and-measurement/)):
...
of call count, which is why input-architecture levers ([06 — Context architecture — what enters the window at all](/research/context/techniques/06-context-architecture/)) outrank style levers despite the
0.1× price.

### 8. Measured value of the current caveman setup (this environment)
...
- Visible-output spend: $0.61 of $3.63 (17%). Caveman-ultra cuts ~58.5% of *that*: **~$0.36, or
 ...
 also shrink subsequent cache writes/reads of those turns; quantified in [15 — Composed Stacks: Conservative / Aggressive / Unbelievable](/research/context/techniques/15-composed-stacks/)).
- The "~75%" face-value claim, applied naively to all output, would have promised ~$1.0 (28%).
 **The measured number is ~2.8× smaller than the folklore number.** This is the brief's §5
...
For jackin❯, the baseline audit of the research environment evidence identifies which mechanisms are safe to adopt directly and which still require workload-specific validation.
