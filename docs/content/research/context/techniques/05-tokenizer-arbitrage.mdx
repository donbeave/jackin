...
description: "Measured token effects of data formats, minification, model tokenizer families, identifier patterns, and language choices."
---

## Summary

Lossless serialization choices produce the safest tokenizer savings: compact tabular formats beat pretty JSON or XML, while exotic glyphs and language switching do not generalize.
**TL;DR**

- Claude bills BPE tokens, not characters. Re-encoding the *same* information changes cost with zero information loss. Biggest measured lever: serialization format — one 10-record/7-field dataset spans **986 tokens (pretty XML) to 402 (CSV)** on `claude-fable-5`, a **2.45x spread**; an independent dataset measured a 3.1x spread. Minified JSON → CSV is **-34%**, pretty JSON → minified is **-29% (Fable) / -41% (Sonnet)**.
...
Which lossless representation choices reduce tokens across current Claude tokenizer families, and which character-level shortcuts fail?

## Method

All local numbers use the live `count_tokens` API through the [checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes). The serialization samples use the 10-record, 7-field schema stated below and encode identical values in each format; the identifier and character probes are written inline. Delta method: `tokens(payload) = count(payload as single user message) − overhead`, with overhead calibrated per model as `count("a") − 1` (6 on `claude-fable-5` and `claude-opus-4-8`; 7 on `claude-sonnet-4-6` and `claude-haiku-4-5`). Provider docs warn that counts are estimates and may differ slightly from billing. Each payload is an n=1 sample; a second dataset with the same shape preserved the ordering but shifted magnitudes, so ranges are shown where relevant. Isolated 1–12 digit probes on Sonnet read a constant +1 over the `ceil(n/3)` law because of the single-message boundary; the law holds in-context for 20 eight-digit integers plus 19 newlines (79 Fable tokens = 20×3+19).

Dollar arithmetic uses the modeled heavy-day profile from [01 — Token Economics and Measurement](/research/context/techniques/01-economics-and-measurement/): ~$22/day at Fable 5 list prices, split 32% cache reads / 29% cache writes / 20% thinking / 17% visible output / 2% uncached input (local measurement).
...
T1 local measurements. The second dataset used a different field mix and shorter values, making key overhead more dominant: XML 945, pretty 779, minified JSON 548, TOON 317, CSV 301. Minified JSON→CSV therefore ranged **-34% to -45%** across the two samples. Savings scale with the key:value length ratio of *your* data; measure yours (see the final technique).
...
- **Mechanism:** JSON repeats every key plus quotes/braces per record; BPE charges for all of it. Column-once formats pay for keys a single time. Local: minified JSON 611 → CSV 402 Fable (**-34.2%**; Sonnet -30.5%; second sample -45%); pretty JSON → CSV **-53.4%** (second sample -61%). Delimiter is a pure no-op: CSV=TSV=PSV exactly (402/402/402 Fable, 317/317/317 Sonnet). TOON costs **+4.2% over CSV** locally (second sample +5.3%; TOON repo's own figure +5.9% on o200k_base, 67,778 vs 63,997 tokens) while adding `[N]` length and `{fields}` schema guardrails.
...
- **Composability:** multiplies through caching (smaller static content = cheaper cache write AND 0.1x reads thereafter; see [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/)); stacks with numeric/ID normalization below (those dominate per-row cost); orthogonal to model routing.
...
- **Composability:** rescales every other technique's denominator — minification saves 41% on Sonnet but 29% on Fable; never port format-ROI numbers across the 4.7 boundary. Pairs with subagent delegation patterns in [06 — Context architecture — what enters the window at all](/research/context/techniques/06-context-architecture/).
...
- **Effort to adopt:** minutes.
- **Composability:** stacks inside tabular re-serialization; pairs with output-side rules in [09 — Output discipline and structured generation](/research/context/techniques/09-output-discipline/) (told-to-emit numbers ride the ~5x output price).
...
- **Composability:** output-side wins ride the ~5x output price multiplier (see [09 — Output discipline and structured generation](/research/context/techniques/09-output-discipline/)); input-side stacks with everything.
...
- **Evidence tier:** T1 (two independent + Baseline) + T3 (ai.rs replication) + T2 (Petrov et al., arXiv:2305.15425).
...
- **Availability:** CLAUDE-CODE-TODAY (trivially — by *not* doing it; use English-register compression instead, covered in [03 — Prior art and market scan](/research/context/techniques/03-prior-art-and-market-scan/) and [09 — Output discipline and structured generation](/research/context/techniques/09-output-discipline/)).
- **Effort to adopt:** n/a.
...
- **Composability:** applies to MCP schema design (see [02 — Baseline audit of the research environment](/research/context/techniques/02-baseline-audit/) tool-schema overhead) and any codegen target.
...
- **Availability:** CLAUDE-CODE-TODAY (API key or Claude Code OAuth token; the [checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes) is the working reference).
- **Effort to adopt:** minutes.
- **Composability:** prerequisite for all of the above; feeds the regression harness in [16 — Validation Harness: the No-Quality-Loss Proof Protocol](/research/context/techniques/16-validation-harness/).
...
For jackin❯, the tokenizer arbitrage evidence identifies which mechanisms are safe to adopt directly and which still require workload-specific validation.
...
| "Wenyan compresses 75–80%" | Char illusion: Baseline 80.9% chars → 56.6% tokens; ai.rs tiktoken test: the 28% claim was char-counted; wenyan +6.4% *worse* than English shorthand on cl100k. |
...
| "Caveman mode cuts ~75%" | Baseline: 58.5% measured token cut at ultra register. Good technique, honest number. |
...
1. All cross-content premiums are n=1 payloads; prose premium varies +38% (Baseline corpus) to +59% (this battery) around the official "roughly 30%" — a 1K-file corpus sample via the free endpoint is the cheap next step. Likewise ZH chars/token read 0.93 here vs ~1.47 in Baseline (different text mixes; unresolved).
...
3. Output-side format arbitrage untested: output bills ~5x input and thinking is 54.8% of output (Baseline); whether instructing Claude to *emit* CSV/minified JSON harms generation correctness is the highest-value open experiment here (the 862→611→402 input ratios should roughly apply to emitted data).
...
5. Cache interplay shown only as arithmetic (smaller content → cheaper write and 0.1x reads immediately); not yet demonstrated end-to-end in a live session — see [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/).
...
|---|---|
| Format table (986/862/611/628/544/440/437/419/402 Fable; 901/777/456/543/448/348/327/333/317 Sonnet); 2.45x spread; -34.2% min→CSV; -29.1%/-41.3% pretty→min; TOON +4.2% vs CSV; fields+rows -28.5%; CSV=TSV=PSV | Ten-record, seven-field samples described in Method, measured with the [checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes) and the stated delta method |
...
| Baseline ground truth: thinking 54.8% of output; wenyan-full 80.9% chars→56.6% tokens; wenyan-ultra 74.5%; caveman-ultra 58.5%; Fable premium ~+15% code / ~+38% prose corpus; day split 32/29/20/17/2 | Local Baseline measurement (see [01 — Token Economics and Measurement](/research/context/techniques/01-economics-and-measurement/)) |
