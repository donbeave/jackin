...
do **not** duplicate Volume I's sixteen ([20 — Multimodal token economics](/research/context/techniques/20-multimodal-token-economics/) K1–K16). Each is worked mechanism → savings math →
...
profile (note: Fable-priced; ~half for an Opus-4.8 subscriber once Fable 5 is removed, [24 — Fresh literature & market delta (clean-room re-sweep)](/research/context/techniques/24-fresh-literature-and-market-delta/)); quota figures
use [19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/research/context/techniques/19-subscription-and-quota-economics/)'s model.
...
**Mechanism:** the subscription cap is a rolling 5-hour window plus a fixed weekly anchor ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/research/context/techniques/19-subscription-and-quota-economics/)).
...
**Coverage-delta:** New. Directly attacks [19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/research/context/techniques/19-subscription-and-quota-economics/)'s bounded INCOMPLETE (the unpublished cap
...
0.1× ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/research/context/techniques/19-subscription-and-quota-economics/)).

**Savings math:** no direct saving; it converts [19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/research/context/techniques/19-subscription-and-quota-economics/)'s "tasks-per-cap is unquantifiable" into a
...
*if* the proxy preserves `cache_control` (a careless proxy busts the cache — [19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/research/context/techniques/19-subscription-and-quota-economics/) Q1). **Effort:**
medium.
...
the 3.05× per-image cap divergence ([20 — Multimodal token economics: images, screenshots, PDFs](/research/context/techniques/20-multimodal-token-economics/)), which Volume I never measured.
...
modest in dollars (image tokens at input price) but real in quota ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/research/context/techniques/19-subscription-and-quota-economics/)) and window pressure, and
...
**Coverage-delta:** New. Volume I has zero multimodal; this operationalizes [20 — Multimodal token economics: images, screenshots, PDFs](/research/context/techniques/20-multimodal-token-economics/)'s "text beats
...
**Coverage-delta:** New. Volume I never models latency; this automates [21 — Latency, wall-clock, and human-time as a second cost axis](/research/context/techniques/21-latency-and-time-economics/)'s v·t·s > Δ$
inequality.
...
**Coverage-delta:** New synthesis of [24 — Fresh literature & market delta (clean-room re-sweep)](/research/context/techniques/24-fresh-literature-and-market-delta/) FL1 (CAG-via-caching) + [22 — Fleet, team & multi-tenant cache economics (hosted)](/research/context/techniques/22-fleet-and-multitenant-cache/) (fleet workspace cache) +
...
exploration tokens. Bounded by the 200K subscription context ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/research/context/techniques/19-subscription-and-quota-economics/)) — the *core*, not the whole
repo, fits.

**Feasibility verdict:** BUILDABLE — jackin❯'s launcher is the natural home (it already owns the
insertion points, Vol I K16 / [22 — Fleet, team & multi-tenant cache economics (hosted)](/research/context/techniques/22-fleet-and-multitenant-cache/) F6). The hard part is curating "the stable core" and keeping it
...
**Coverage-delta:** New. Connects [25 — Meta layer: the cost of optimizing, budget governance, and online quality guarding](/research/context/techniques/25-meta-cost-governance-and-online-quality/)'s online judge (blind spot 8) to compression; Volume I's
...
**Coverage-delta:** New. Operationalizes [23 — Cross-agent / cross-provider portability matrix](/research/context/techniques/23-cross-agent-portability/)'s portability matrix; Volume I is single-agent.
...
Volume I left them — blocked behind the hosted API (soft-prompts, KV export: K1/K2/[24 — Fresh literature & market delta (clean-room re-sweep)](/research/context/techniques/24-fresh-literature-and-market-delta/)) — and the
