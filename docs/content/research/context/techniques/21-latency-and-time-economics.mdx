...
# 21 — Latency, wall-clock, and human-time as a second cost axis

Volume II area file for blind spot 3. Volume I optimizes dollars
only; latency appears in scattered mentions (`11:110` "dollars-for-wallclock", `12:132` batch
latency, `13:147` self-host TTFT, `01:52` "speed not savings") but is **never built into a decision
...
credits (real dollars) but **do not count against the cap** ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/research/context/techniques/19-subscription-and-quota-economics/)) — so on a quota-bound Max seat,
...
- **Coverage-delta:** New. Volume I's measurement file (01) and validation harness (16) never cost
...
- **Coverage-delta:** Volume I's multi-agent file (11) covers parallel-vs-serial *token* economics;
...
- **Coverage-delta:** Volume I's caching file (07) is entirely dollar/quota; the **TTFT and
...
- **Coverage-delta:** Volume I (12) states the 50%/24 h facts; weighing them on the *time axis* (when
...
- **Expected savings:** 50% dollars on all offline-able work; on a subscription, headless/SDK batch
 also draws the separate credit pool, off the interactive cap (19 Q7).
...
At the subscription cap, the only remaining lever *is* a time-vs-dollar trade (bridges [19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/research/context/techniques/19-subscription-and-quota-economics/) Q6).
...
- Caching is the only lever that is **negative-cost on dollars, quota, *and* time** simultaneously —
 which is why prefix hygiene ([07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/), [19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/research/context/techniques/19-subscription-and-quota-economics/) Q2) is the highest-leverage habit on every axis.
