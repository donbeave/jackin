---
title: "06 — Context architecture — what enters the window at all"
description: "Techniques that prevent unnecessary instructions, tools, files, and history from entering an agent context window."
---

## Summary

Preventing irrelevant tools, files, and repeated instructions from entering context is safer and usually more valuable than compressing them after admission.
**TL;DR**
...
- Prefix economics compound through caching: every 10K of always-on prefix ≈ $0.01/turn in cache reads ≈ **$1.89/day** on the modeled profile (114 reads + 6 writes/day). Cache reads are 32% of heavy-session dollars (local Baseline; see [01 — Token Economics and Measurement](/research/context/techniques/01-economics-and-measurement/)).

## Question and scope

Which context should load eagerly, load on demand, move behind retrieval, or stay outside the model window?

## Method

The analysis inventories standing and on-demand context, measures representative repository files and prefix components, and compares those local results with provider rules for tools, memory, and scoped instructions.
...
|---|---|---|---|
| Never-load | Does it enter the prefix at all? | MCP deferral (06.1), prompt audit (06.2), skills (06.3), path-scoped rules (06.4) | 85-96% of the artifact, forever |
| Load-less | Can a cheaper representation do the job? | Repo maps/outlines (06.5), LSP navigation (06.6), hook preprocessing (06.7) | 85-92% per file/result |
| Evict | When does it leave? | Observation masking (06.8), /clear-/compact discipline (06.9), subagent isolation (06.10), external memory (06.11), learned pruning (06.12) | 84-88% of stale history |

### Local measurements (this repo, Fable 5 count_tokens via [checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes))

Method: `python3 docs/content/research/context/techniques/tools/count_tokens.py --model claude-fable-5 file sample FILE` (free `count_tokens` API). Outlines: `grep -nE '^\s*(pub(\([a-z: ]+\))?\s+)?(async\s+)?(unsafe\s+)?(fn|struct|enum|trait|impl|mod|type|const|static)\b' FILE`.

| Artifact | Lines | Tokens | Note |
|---|---|---|---|
| <RepoFile path="AGENTS.md">AGENTS.md</RepoFile> (root, always-on) | 75 | 2,744 | under the official 200-line rule; matches Baseline's 2,738 net of harness overhead |
...
### 06.2 System-prompt and CLAUDE.md mass audit — re-run every release
...
- **Effort to adopt:** minutes — `grep -o '"cache_creation_input_tokens":[0-9]*' SESSION.jsonl | head -1`; pipe CLAUDE.md through [checked-in token counter](/research/context/techniques/tools/#count_tokenspy-modes).
- **Composability:** the meta-technique that prioritizes all others; prefix cuts compound via the 0.1x cache-read multiplier on every turn (cache reads = 32% of heavy-session dollars; see [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/)).
...
### 06.3 Skills as lazy loading (progressive disclosure)
...
- **Composability:** official escape valve for the 200-line rule (06.2); `skills:` preloads fully into subagents (06.10) instead of your window.
...
### 06.4 Path-scoped rules (.claude/rules paths frontmatter, per-directory agent rules)
...
- **Composability:** the middle rung of the instruction ladder — CLAUDE.md (always) → rules (on path match) → skills (on invoke) (06.2, 06.3).
- **Validation protocol:** count_tokens each scoped file; run sessions inside and outside the scoped paths verifying (a) `/context` shows the rule only when expected, (b) a seeded rule-violation prompt is caught inside the path. Confirm no always-on regression by re-reading first-cache size (06.2).

### 06.5 Repo maps and outlines instead of file dumps
...
- **Composability:** feeds subagent isolation (06.10: Explore ≈ self-assembling repo map) and LSP navigation (06.6).
...
### 06.6 Symbol-graph / LSP navigation instead of file reads
...
- **Layer:** input — load-less (file content)
- **Mechanism:** LSP tools (go-to-definition, find-references, find_symbol) return bodies or locations directly. Official costs documentation states that one go-to-definition call can replace a grep plus multiple candidate reads; installed language servers also report type errors after edits. Serena MCP is the self-hosted form, while Claude Code exposes first-party code-intelligence plugins and a deferred LSP tool.
- **Expected savings:** no published measurements anywhere (Serena README is qualitative — "much more token-efficient"; checked). Magnitude bound from 06.5: not-reading-the-rest-of-the-file is worth 85-92% per file touched (local). Counterweight: an LSP MCP server adds its own schemas — mitigated by default deferral (06.1).
...
- **Composability:** pairs with grep-first (grep finds candidates, LSP picks one) and outlines (06.5); diagnostics-after-edit replaces compile-run cycles (separate channel).
...
### 06.7 Hook-based preprocessing / tool-output filtering
...
- **Composability:** composes with eviction (06.8) — masking can only evict what entered; hooks stop it entering. Cousin of skills' script pattern (06.3: script code never enters context, only output).
...
### 06.8 Observation masking / tool-result eviction (context editing, microcompact)
...
- **Composability:** complements deferral (06.1: prefix vs history); interacts with caching — naive client-side deletion invalidates the cached prefix; Claude Code's cache-edit approach exists precisely for this (see [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/)).
...
### 06.9 Compaction discipline: /clear over /compact, steered compaction
...
- **Composability:** last-resort layer after 06.1/06.7/06.8 (Claude Code itself orders it: clear tool outputs first, summarize second). CLAUDE.md is the durable instruction channel across compactions (06.11).
...
### 06.10 Subagent context isolation (context firewall)
...
- **Mechanism:** subagents get their own fresh context; the main thread pays the spawn prompt + returned summary. Built-in Explore/Plan omit CLAUDE.md and git status from startup; subagents run a slimmer system prompt than the interactive host. Since Claude Code 2.1.198 the built-in Explore agent inherits the main session's model (capped at Opus) instead of running on Haiku — Explore is no longer cheap-model-routed by default; pin a cheaper model explicitly. Subagents and compaction inherit the session's extended-thinking config, and subagents run in the background by default (code.claude.com/docs/en/changelog).
- **Expected savings:** main-window: 20 files × ~2K tok lands as a summary "often 1,000-2,000 tokens" instead of ~40K (eng blog). Total-dollar: often negative — the subagent pays its own, frequently cold-cache, bill; official: "Agent teams use approximately 7x more tokens than standard sessions when teammates run in plan mode" (costs doc — plan-mode agent teams specifically; do not generalize to single subagents). Cross-ref: the caveman cavecrew plugin claims ~60% smaller re-injected subagent output (plugin description; Baseline measured the underlying ultra register at 58.5% on visible prose).
- **Evidence tier:** T1 for isolation semantics (official docs); the 1-2K summary is a description, not a guarantee.
- **Quality risk:** NEUTRAL on cost, quality-protective for the main thread (stays under the rot knee, 06.0). RISKY for tight back-and-forth tasks — the summary boundary loses detail.
...
- **Composability:** multiplies with repo maps (06.5) and cheap-model routing (`model: haiku` for simple subagents — official tip); the GitHub-MCP mitigation (tool-search subagent, main-thread 51,000 → 8,500) is this pattern.
...
### 06.11 Memory outside the window (memory tool, auto memory, note files)
...
- **Composability:** the enabling pair of 06.8 (Anthropic ships and evaluates them together); what makes /clear safe mid-project (06.9).
...
### 06.12 Learned context pruning (SWE-Pruner) — research frontier
...
- **Composability:** generalizes hook-greps (06.7: learned filter vs regex); could stack under masking (prune on entry, mask on age).
...
For jackin❯, the context architecture — what enters the window at all evidence identifies which mechanisms are safe to adopt directly and which still require workload-specific validation.
...
7. **"Caveman-style compression cuts ~75% of tokens"** — Baseline killed it: ultra register = 58.5% measured TOKEN cut; ~75% is character-level folklore. Applies to any "compress your CLAUDE.md by X%" claim measured in characters (wenyan-full: 80.9% chars, only 56.6% tokens).
...
2. LSP/Serena navigation has zero published quantitative benchmarks (06.6's protocol is cheap, unclaimed territory).
...
4. The cache-economics interaction is unpublished: eviction rewrites cached prefix (1.25-2x write) while prefix-shrinking saves 0.1x reads forever; with reads at 32% and writes at 29% of heavy-day dollars, optimal eviction FREQUENCY is an open optimization problem (see [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/)).
...
| <RepoFile path="AGENTS.md">AGENTS.md</RepoFile> 2,744 tok/75 lines; scoped agent rule files 10,694 + 5,982 + 2,821 + 1,053 + 381 = 20,931 (88.4% scoped) | `python3 docs/content/research/context/techniques/tools/count_tokens.py --model claude-fable-5 file sample FILE` |
...
| 11 MCP schemas 1,420 vs ~60 deferred; dollar split 32/29/20/17/2; session profile (19 calls, 1.17M cache-read, 27K out); caveman 58.5%/56.6%/80.9% | local Baseline measurements (ground truth per dossier brief) |
| Fable 5 $10/$50/MTok; cache read 0.1x, write 1.25x (5min); all $/day arithmetic | dossier reference pricing (see [01 — Token Economics and Measurement](/research/context/techniques/01-economics-and-measurement/)) + arithmetic shown inline, labeled ESTIMATE |
