---
title: "22 — Fleet, team & multi-tenant cache economics (hosted)"
description: "Fleet and multi-tenant cache economics, prefix-sharing rules, isolation boundaries, and hosted deployment techniques."
---

## Summary

Hosted fleets share cache only when provider scope and byte-identical prefixes align. This chapter models cross-container prefix reuse, isolation boundaries, launcher-controlled dynamic sections, and the interaction between fleet size and quota.
...
- **The server prompt cache is workspace-scoped, not machine-scoped.** N
 ...
 rules the core dossier cited ([07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/) tech 7) describe Claude Code's **local file cache** (GitHub #17531), a
 different layer.
- **A ~111-token dynamic block silently un-shares a ~28–34k-token prefix.** The Agent SDK preset puts
 ...
 so the fleet shares one entry — the **primary hosted-fleet lever**, partially closing the core dossier's
 unmeasured-size gap.
...
- **Multi-tenant boundary: org isolation is absolute; workspace is the sharing unit.** Tenant-per-org
 ...
 one org → the shared system prefix pools. On a **subscription**, the whole fleet shares **one
 pooled cap** ([19 — Subscription and quota economics](/research/context/techniques/19-subscription-and-quota-economics/)); headless and SDK work do not currently provide an off-cap lane.

Cache multipliers and the $22/day profile are inherited from the core dossier; fleet dollar figures are
...
Under which provider scope and prefix conditions can separate agents share prompt-cache entries safely?

## Method

The analysis applies documented cache-scope and prefix rules to a parameterized fleet model, with local counts for dynamic-prefix fields and explicit cold-start, isolation, and quota assumptions.

## Findings

### Hosted and local cache layers
...
The caching chapter ([07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/) tech 7 + surprising-findings: "your git state is in the cache key… worktrees never
...
Claude Code's *observed* reuse blends both layers. This correction has now been applied to [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/);
the practical upshot is favorable — hosted
fleets *can* share across machines/dirs, which the core dossier's framing implied they could not.
...
3. **Same model** (and same effort/fast-mode — the core dossier's cache-key facts hold).
...
5. **Prefix ≥ the model minimum** (Opus 4.8 = 1,024; Fable 5 = 512; Haiku 4.5 = 4,096 — live values
 match [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/) tech 11; sub-minimum prefixes silently cache nothing).
...
- **Evidence scope:** [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/) tech 7 covers the SDK flag; the **workspace-scope rule** (the
 isolation change) as the gating condition for hosted fleet sharing is not in the core dossier.
...
- **Expected savings:** turns each extra container's ~28–34k system prefix from a 1.25–2× write into a
 ...
 ~$0.94 (1 write + 24 reads) per TTL window (ESTIMATE on the core dossier multipliers).
...
- **Evidence scope:** [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/) tech 7 names the flag; **the enumerated six fields, the measured
 ~111-token size, the version gating, and the mixed-cohort trap** are new (and partially close
 the core dossier Gaps #6).
...
- **Evidence scope:** [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/) tech 8 covers the concurrency rule for subagent waves; applying it
...
- **Quality risk:** **NEUTRAL** (adds one TTFT of latency to the wave start — [21 — Latency, wall-clock, and human-time as a second cost axis](/research/context/techniques/21-latency-and-time-economics/)).
...
- **Evidence scope:** GitHub issue #29966 reports a potentially version- or path-dependent conflict. The earlier measurement found cavecrew/Claude Code
 subagents *writing* 5m cache (07 tech 2); this reports Agent-tool subagents with caching **off** —
 a possible version/path-dependent conflict, flagged in 27.
...
- **Evidence tier:** T3 (one community-measured session; issue open, not Anthropic-confirmed). **Verify
 in your own version before acting** — the core dossier's own measurement showed subagents writing cache, so
...
- **Composability:** interacts with [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/) tech 4 (subagent fan-out economics assume caching);
 if caching is off, fan-out is far more expensive than the core dossier modeled.
...
- **Evidence scope:** The core dossier is single-operator; org/workspace multi-tenant cache boundaries
 are not analyzed.
- **Layer:** infra / tenancy.
...
and the version floor so the whole fleet inherits cache sharing without per-user
discipline.

- **Evidence scope:** the core dossier K16 in [14 — Frontier](/research/context/techniques/14-frontier-ideas/) proposes baking the optimization pack into jackin❯; the
 **specific hosted-fleet items** (workspace auth, excludeDynamicSections, cold-start pre-warm,
 and the SDK-version floor) are new and concrete.
...
- **Mechanism:** jackin❯'s launch env assembly is the chokepoint: pin every container to one workspace
 ...
 once before fanning out the wave (F3), and enforce the SDK version floor (avoid F2's cohort split / F4).
- **Expected savings:** the de-duplicated fleet sum: one cached prefix instead of N and F3's ~10× cold-start cut, delivered as infrastructure rather than discipline.
...
- **Availability:** BUILDABLE (jackin❯ roadmap; insertion points mapped in [14 — Frontier](/research/context/techniques/14-frontier-ideas/) K16 / 17).
...
- **Validation protocol:** per-fleet before/after: cache-read ratio on container 2..N's first call and cold-start write total.

---

### Surprising findings

- The hosted server cache is **more** shareable than the core dossier implied: no machine/worktree key means a
...
- On a subscription, the fleet's binding constraint is the **single pooled cap** (19), not dollars; non-interactive containers currently consume that same cap.

## Implications for jackin❯

### Techniques

For jackin❯, the fleet, team & multi-tenant cache economics (hosted) evidence identifies which mechanisms are safe to adopt directly and which still require workload-specific validation.
...
| 4 | Dynamic block ≈111 tok (6 fields) / ≈201 with short git status; blast radius = full ~28–34k prefix | local count_tokens on a representative block + the core dossier prefix size (02/07) |
...
| 7 | Min cacheable prefix Opus 4.8 = 1,024; Fable 5 = 512; Haiku 4.5 = 4,096 (matches [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/)) | platform.claude.com prompt-caching |
...
| 10 | Subscription fleet shares one pooled cap; headless and SDK work remain within it while the announced split is paused | support.claude.com ([19 — Subscription and quota economics](/research/context/techniques/19-subscription-and-quota-economics/) ledger) |
