---
title: "08 — Codex follow-ups"
description: Resolves Codex follow-up questions about plan labels, account methods, credential storage, and quota provenance.
---

## Summary

First-party Codex evidence resolves account methods and credential storage, while marketing multiplier labels remain external mappings.

## Question and scope

(Q1) How does first-party openai/codex render `plan_type` values (`pro`, `prolite`) to user-facing plan names — is `prolite` "Pro 5x" and `pro` "Pro 20x", or plain "Pro"? Can chapter 02's "Pro 5x vs Pro 20x not distinguishable from `plan_type`" and jackin❯ `pro`→"Pro 20x" / `prolite`→"Pro 5x" mapping both be true? (Q2) In `codex-rs/app-server-protocol/src/protocol/` (common.rs and v2/), what are the `account/*` JSON-RPC methods — does `account/read` (called by jackin❯) exist, what does `account/usage/read` (documented in chapter 02) return, and is jackin❯ missing a method or did chapter 02 misname one? (Q3) What is the default value of `cli_auth_credentials_store` and the semantics of `auto` — on a default macOS install, does the token live in `~/.codex/auth.json` or the OS keyring?
...
- `account/usage/read` returns non-limit telemetry rather than quota windows — `codex-rs/app-server-protocol/src/protocol/v2/account.rs` lines 392–395 and 435–449 (confidence: HIGH).
- **Settlement.** Neither a missing method nor a misname. jackin❯ calls `account/rateLimits/read` + `account/read` in <RepoFile path="crates/jackin-usage/src/usage/codex.rs" /> (lines 767–785); `account/usage/read` is a distinct, out-of-scope method that jackin❯ correctly does not call (confidence: HIGH).
- Cross-ref divergence observed while verifying: jackin❯ `account/read` decoder `CodexRpcAccountDetails` in <RepoFile path="crates/jackin-usage/src/usage/codex.rs" /> (lines 428–438) accepts tags `"apikey"` and `"chatgpt"` only, while upstream serializes `"apiKey"` (camelCase) and also emits `"amazonBedrock"`. Serde external-tag matching is exact, so `account/read` decode fails for API-key and Bedrock accounts; and although the comment at lines 775–776 says the account label is non-essential, the decode error at lines 786–792 propagates via `?` and fails the whole RPC usage result. Finding only, recorded for the jackin-desktop consumer (confidence: HIGH for the tag mismatch; HIGH for the propagation path as written)
...
Separate first-party plan and window identifiers from marketing multipliers, and reuse current app-server contracts without implying undocumented API support.
