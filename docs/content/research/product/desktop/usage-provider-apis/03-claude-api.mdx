...
- A CodexBar Claude documentation page appeared in search results; not fetched (clean-room constraint) — nothing from it is used here


## Implications for jackin❯

Represent Claude’s observed subscription windows and reset metadata without inventing token totals, refill promises, or plan labels the payload does not prove.

## Limitations and unknowns

The `oauth/profile` endpoint exists (scope-gated `any_of(user:profile, user:office)`, returns account email/uuid + organization uuid); `claude setup-token` tokens are rejected by `oauth/usage` (403, `user:inference` only). Payload fields (`weekly_scoped`, `seven_day_cowork`, `amber_ladder`, `seven_day_oauth_apps`) have independent public corroboration, and macOS stores credentials in Keychain by default under `Claude Code-credentials` — see [chapter 09](/research/product/desktop/usage-provider-apis/09-claude-followups/).
...
- Whether `oauth/profile` returns a plan-label field remains unknown; its scope-gated email/UUID fields are verified in [chapter 09](/research/product/desktop/usage-provider-apis/09-claude-followups/).
...
- [Claude follow-ups](/research/product/desktop/usage-provider-apis/09-claude-followups/)
