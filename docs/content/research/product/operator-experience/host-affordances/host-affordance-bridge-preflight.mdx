...
description: Validates host-bridge feasibility across clipboard, image, URL, file, diagnostics, and export interactions.
---
...
Clipboard, image, URL, file, diagnostics, and export affordances fit one typed host bridge when each action has explicit policy.

The preflight evidence covers clipboard, image, URL, file, diagnostics, and export paths; canonical docs and roadmap pages own current feature status.

## Purpose

This page records the evidence required for default behavior in the [Host Affordance Bridge](/roadmap/clipboard-image-bridge/). The bridge roadmap owns product status and architecture. The evidence checklist covers terminal events, OrbStack transport, clipboard readers, Amp `xclip` behavior, binary transfer shape, and GitHub CI URL lookup.

## Required Outputs

Default behavior requires the following evidence artifacts; the verification cutoff and remaining live-validation gaps are recorded below.
...
- A clipboard-backend report for macOS, Linux X11, and Linux Wayland. Evidence through the cutoff covers macOS; Linux Wayland/X11 remain live-validation gaps.
...
**Verification cutoff:** 2026-06-11. The probes establish the current clipboard and terminal boundaries. Reproduce them with the commands and inputs documented below; scratch output is not evidence and is not part of the product.
...
- The opt-in host path owns raw mode, stdin/stdout relay, SIGWINCH resize, focus/mouse bytes, cleanup, direct-socket transport, `attach-proxy` transport, and shared OSC 10/11 terminal-color probing. Default behavior remains gated on live validation.
...
- The command palette now has `Stage image from clipboard path`. This explicit action handles the measured plain-text-path result without changing ordinary text paste: the host attach client reads host clipboard text only for this action, requires an absolute readable image path or `file://` image URL, validates image magic bytes, sends the same image frames as direct paste on success, and returns `ClipboardImageError` for visible failure feedback on rejection.
...
- Default behavior remains gated on the live-validation evidence below, including Linux host backends and broader affordance boundaries.
...
The default-behavior evidence gate requires this flow to be proven manually or with throwaway prototype code:
...
## Default-Behavior Evidence Gate

Every required output above must be one of the following before the host affordance bridge becomes default behavior:
...
Do not replace missing measurements with assumptions. The bridge exists to remove host/container ambiguity; default behavior must follow measured boundaries.
