---
title: "Operator Handler System: Design Exploration"
description: Defines a typed operator-handler system for routing agent requests to visible, controllable host actions.
---
...
A typed handler registry should route agent requests only to visible, policy-controlled host actions.

This exploration informs [Operator handler system](/roadmap/operator-handler-system/) in the [Agent Orchestration Program](/research/agents/orchestration/program-research/).
...
- Multiple downstream items in this program need it: [GitHub link tracking](/roadmap/github-link-tracking/) opens issue/PR URLs, [agent tag protocol](/roadmap/agent-tag-protocol/) opens repo paths, [custom operator tools](/roadmap/custom-operator-tools/) expand exec templates that eventually want the same launcher resolution.
...
- Config — [`config.toml` `[handler]` block](https://github.com/graemerocher/multicode/blob/main/config.toml)
