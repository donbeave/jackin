---
title: "Custom Operator Tools (`[[tool]]`): Design Exploration"
description: Designs manifest-defined operator tools with explicit inputs, host actions, timeouts, and safe presentation.
---
...
Operator tools should be manifest-defined, typed, time-bounded, and visibly authorized before invoking host actions.

This exploration informs [Custom operator tools](/roadmap/custom-operator-tools/) in the [Agent Orchestration Program](/research/agents/orchestration/program-research/).

## Research question

jackin❯ already has pieces that custom tools should reuse: in-container `jackin-exec`, the credential picker, and MCP execution plumbing. Missing is the operator-configured `[[tool]]` surface: reserved-key validation, substitutions from workspace/session/tag state, console rendering, output capture, and clear host-vs-container execution semantics.
...
- It pairs cleanly with [agent runtime status](/roadmap/agent-runtime-status/) and [tag protocol](/roadmap/agent-tag-protocol/): the captured tags become substitution arguments for the tool command, so a tool can be as simple as "open the issue URL the agent declared".
- It uses the same handler-resolution machinery as the [operator handler system](/roadmap/operator-handler-system/) — different config surface, same primitives.

## Inspiration in multicode

**Sources**:
- No dedicated README section
- Config — [`config.toml` `[[tool]]` array](https://github.com/graemerocher/multicode/blob/main/config.toml) (the example only shows one tool — `bash` on `b` — but the schema is the same)
- Source — [`tui/src/app.rs`](https://github.com/graemerocher/multicode/blob/main/tui/src/app.rs) (tool firing, reserved-key validation)
...
jackin❯ existing reserved keystrokes (the navigation set in the console, plus quit/exile/eject) are off-limits. Tool config validation errors at config load if a custom tool collides with a reserved key.
...
- `{repo}`, `{issue}`, `{pr}`, `{link}` — most-recent value from the [tag protocol](/roadmap/agent-tag-protocol/) index
...
- Tool firing routes through the [operator handler system](/roadmap/operator-handler-system/) for command resolution and output rendering.

## Out of scope

- Per-workspace tool overrides; tools remain operator-config-level.
- Streaming output for long-running tools; output is captured and displayed on completion.
- Tool authoring helpers such as scaffolding configuration from a command.
- Tool emission of `<jackin:*>` tags to update the per-instance tag index.
- Tool composability (one tool invokes another). Out of scope.

## Limitations and open questions

- **Where does the tool's stdout go?** A modal that overlays the console matches multicode but interrupts flow. A scrolling log panel is friendlier but requires the [console resource panel](/roadmap/console-resource-panel/) infrastructure. Recommended default: *modal*.
- **Do tools work outside the console?** A `jackin tool run <key>` CLI command would be useful for scripting, but the current design is console-only.
...
- <RepoFile path="crates/jackin/src/cli/config.rs">crates/jackin/src/cli/config.rs</RepoFile> — `[[tool]]` parsing
- <RepoFile path="crates/jackin/src/console/adapter.rs">crates/jackin/src/console/adapter.rs</RepoFile> — keybinding registration and modal rendering
