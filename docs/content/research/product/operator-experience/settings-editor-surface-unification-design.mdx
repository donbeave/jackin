...
description: Maps duplication between settings and editor configuration surfaces and defines their shared ownership model.
---
...
Settings and workspace editor surfaces should share one configuration model, validation path, and component system.

This page preserves the rationale for shared input ownership, primitives, validation, and parity; current rules live in [TUI architecture](/reference/tui/architecture/).
...
Generic state structs live once in `jackin-console`; concrete instantiations live in the `jackin` app crate (<RepoFile path="crates/jackin/src/console/adapter.rs" />). The structural asymmetry at the root: the editor uses **one** unified modal enum `ConsoleModal` (<RepoFile path="crates/jackin-console/src/tui/model/modal.rs" />); settings splits the same modal taxonomy across **three** enums (`GlobalMountModal`, `SettingsEnvModal`, `SettingsAuthModal`) plus a screen-level error-popup field (<RepoFile path="crates/jackin-console/src/tui/screens/settings/model.rs" />).
