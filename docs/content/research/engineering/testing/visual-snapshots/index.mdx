...
description: Designs deterministic, reviewable visual snapshots for CLI and TUI surfaces beyond text-only assertions.
---
...
CLI and TUI visual regressions need deterministic SVG goldens with one review workflow and explicit rendering controls.

Text snapshots and the component SVG lookbook provide a baseline; styled full-surface, PR-diffable coverage remains unproven.
...
This page is the canonical home for visual / styled-snapshot regression testing of rendered output. It absorbs the former *Snapshot tests for TUI render* item; shared deterministic fixtures live in the [`jackin-test-support` crate](/reference/crates/jackin-test-support/), the CI wiring and coverage are owned by [Rust CI tooling & dependency hygiene](/research/engineering/ci/rust-tooling/), and live PTY read/wait/send automation is owned by [Terminal observation and automation](/research/agents/orchestration/terminal-observation/terminal-observation-automation/). The boundaries between those items and this one are spelled out under *Related work* below.
...
| [`cellshot`](https://github.com/kitlangton/cellshot) | Rust, MIT | PTY capture with a structured cell-frame model; emits text/JSON/ANSI/SVG/PNG | Already the research trigger for [Terminal observation and automation](/research/agents/orchestration/terminal-observation/terminal-observation-automation/); its frame model and block-element SVG rendering are worth borrowing, but it is a PTY daemon, not a render-regression library. |
...
- The shared encoder must preserve every styled-cell attribute and have intent tests proving that colour or modifier loss changes the golden.
- One deterministic harness owns fixed sizes, resolved themes, clocks, redaction, and cell geometry for console, Capsule, launch TUI, and CLI artifacts.
- Composite goldens cover representative full-screen layouts at fixed dimensions and both supported theme variants; functional CLI behavior remains in `assert_cmd` tests.
- Live-session fixtures consume `GridSnapshot`; optional replay artifacts remain documentation aids rather than the regression authority.
- [Launch progress TUI](/roadmap/launch-progress-tui/) and the relevant component roadmaps own delivery order and remaining implementation work.
...
- **Determinism is load-bearing.** Any unpinned size, theme, clock, or path makes a golden flaky. Broad golden coverage requires the shared deterministic harness.
...
These unresolved harness contracts must be settled before broad golden adoption:

- **Deterministic cell geometry.** The encoder emits per-cell `x` / `y` coordinates, so glyph advance width must come from a fixed source — a bundled monospace metrics constant, or a vendored font parsed with `ttf-parser` / `swash` / `fontdue` — never a system-font lookup, or goldens diverge across CI hosts.
...
- [Rust CI tooling & dependency hygiene](/research/engineering/ci/rust-tooling/) — owns CI wiring, coverage, and the `insta` dev-dependency; the golden-comparison steps slot into its aggregators.
- [Terminal observation and automation](/research/agents/orchestration/terminal-observation/terminal-observation-automation/) — owns live PTY read/wait/send automation of running sessions; complementary, not overlapping: that item captures live sessions for orchestration, this one asserts deterministic render regressions. The two should share the styled-cell frame schema.
