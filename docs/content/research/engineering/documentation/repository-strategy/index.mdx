...
description: Evaluates separating documentation into its own repository across ownership, freshness, previews, and contributor workflow.
---
...
Documentation should remain colocated while code links, preview builds, and release-coupled updates depend on repository atomicity.

The ownership trade-offs remain relevant, but cross-repository freshness guarantees need current evidence before reconsideration.

## Research question

The Fumadocs documentation site currently lives inside this
repo under `docs/` (entry point: <RepoFile path="docs/source.config.ts">docs/source.config.ts</RepoFile>, with sidebar metadata in `docs/content/**/meta.json`).
...
- A behaviour change in `src/cli/**` ships with the matching update to
  `docs/content/(public)/commands/<cmd>.mdx` in the same commit.
...
3. PR review across two repos costs no more than one extra click for
   the reviewer.
4. <RepoFile path="PROJECT_STRUCTURE.md">PROJECT_STRUCTURE.md</RepoFile> and the cross-reference table are updated to
...
- [Developer Reference](/reference/developer-reference/) — also a docs-discoverability surface, since both depend on docs being easy to find and edit.
