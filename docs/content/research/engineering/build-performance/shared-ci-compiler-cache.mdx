...
description: Compares shared compiler-cache backends, deployment topologies, trust boundaries, and acceptance criteria for CI reuse.
---
...
Shared compiler caches can reduce redundant CI work only when cache keys, trust boundaries, and deployment ownership remain explicit.

The architecture comparison is complete, but the backend bake-off remains unmeasured.
...
The audit measured 57 active GitHub Actions cache entries totaling
10,570,355,881 bytes (9.844 GiB), effectively exhausting the repository's
10 GiB budget. Large target archives and duplicate pull-request/head Rust
toolchain entries can evict compact input-identical archive sets.
...
- [CI/CD speed roadmap](/research/engineering/ci/performance/ci-speed-roadmap/)
  contains the evidence behind the current cache and runner choices.
