...
description: Ranks CI speed opportunities from measured job and step timing, cache behavior, and dependency-work evidence.
---
...
Measured job timing points first to dependency and cache work, with lane changes justified only by wall-clock evidence.

The ranking uses June 2026 job and step evidence; compare the [CI/CD speed baseline](/research/engineering/ci/performance/ci-speed-roadmap/) and remeasure before prioritizing work.
...
The [CI/CD speed roadmap](/research/engineering/ci/performance/ci-speed-roadmap/) already shipped the baseline (per-lane `cargo-registry-warmup`, semantic per-shard `rust-cache` keys, the reusable nextest workflow with a path-routed Docker E2E lane, the dual-runner GitHub/Velnor parity proof, and timing/cache summaries). It also already recorded outcomes that constrain the recommendations below — this item does not re-propose them naively:
...
This item also intersects [Rust CI tooling and dependency hygiene](/research/engineering/ci/rust-tooling/) (which is adopting `cargo-llvm-cov`, the prerequisite for dynamic test selection) and lists `cargo-hakari` and `cargo-chef` as open.
...
| 7 | Static test-impact analysis: skip untouched crate shards; skip the archive build on non-Rust PRs | Extends the existing `changes` / `paths-filter` job to per-shard granularity with safe fallbacks | **−1 full compile per skipped shard**; because #3 already covers most shard skipping, the distinct value is avoiding archive builds on non-Rust PRs | Med | Med |
...
In <RepoFile path=".github/workflows/ci.yml">.github/workflows/ci.yml</RepoFile> the single `test` job depended on `construct-e2e-image` and called the reusable workflow `.github/workflows/rust-nextest.yml` (since deleted as an uncalled orphan; the per-crate fan-out moves into the generated ci.yml under the fleet restructuring plan), which contained six jobs: five pure-Rust shards (`jackin`, `jackin-capsule`, `jackin-runtime`, `jackin-tui`, `small-crates`) and one `docker-e2e`. Only `docker-e2e` downloads the construct image, but GitHub Actions treats the reusable-workflow call as a single dependency-graph node, so all five package shards inherit the caller's `needs: construct-e2e-image` gate.
...
None on the operator's machine. All work is CI configuration plus root tool/config files such as <RepoFile path=".github/workflows/ci.yml">.github/workflows/ci.yml</RepoFile> and <RepoFile path=".cargo/config.toml">.cargo/config.toml</RepoFile>. Any remote `sccache` backend or third-party runner is an opt-in infrastructure decision with its own credentials.
