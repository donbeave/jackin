...
Docs run `29583135163` populated a runner-neutral, 90-day lychee response
artifact so GitHub cache eviction no longer forces thousands of repeat network
requests. The complete unsharded link job took 111 seconds because a new xtask
contract required a 28-second artifact wait and the changed site contract
required a 53-second 1,080-route build. Dependency installation took one
second, the existing Actions response cache restored in one second, and the
full link check took three seconds with no repeated downloads or compilation.
The job then published the same response state as a durable artifact keyed by
the lychee semantic contract; later runners can restore it when the bounded
Actions cache misses.

Preview run `29586027432` proves the input-identical archive fast path across
both runner lanes. The workflow completed in 54 seconds; its complete package
jobs took 19 seconds and 20 seconds on Velnor and 13 seconds and nine seconds
on GitHub. Each job performed an artifact-existence probe, then skipped the
toolchain, Cargo registry, target cache, complete multi-target build,
attestation, diagnostics, artifact download, and artifact upload steps. The
full log contained zero Cargo or Rustup dependency downloads and zero Rust
compile/check markers. GitHub still prefetched declared action implementations
during job setup, including the skipped attestation action, but no package
archive crossed the network. The cold population run remains one complete
multi-target archive job per package; the warm path does not introduce target
parts, batches, or test shards.

Hygiene run `29587713698` restored one 106.7 MB contract-keyed bundle of
stripped Cargo utilities in every Linux consumer instead of installing the
same tools independently. Restore took one to four seconds. Mise then took two
seconds in the complete scheduled-hygiene job, down from 131 seconds in main
run `29580885810`; coverage, mutation, Hakari, Dylint, and DinD entered their
semantic work within 28 to 35 seconds of job start. Dylint no longer spends
minutes installing its driver from source. Its first measured analysis still
downloaded and compiled the standalone lint workspace because the shared
registry contract omitted <RepoFile path="crates/jackin-lints/Cargo.lock" />, then failed on a
nightly compiler API change before its target cache could save. CI run
`29588123914` corrected that ownership: the single registry-warmup job restored
the prior registry, fetched the missing lint dependencies once, and saved the
new 204 MB immutable contract in 35 seconds. Dylint also enables failure-path
target persistence, so nightly incompatibility cannot force the next run to
repeat an otherwise valid dependency build.

CI run `29588439870` then proved the exact registry contract is reusable: the
single warmup owner restored it and skipped every fetch step. The complete,
unsharded `jackin-term` crate job finished in 56 seconds with no dependency
downloads or external-crate compilation. Its separately attributable reverse
dependent, `jackin-capsule`, took 157 seconds. The raw step ledger located 17
seconds in restoration of a non-canonical target seed, 29 seconds in the
feature powerset, and 33 seconds compressing a 472 MB replacement seed at Zstd
level 10. Cache-only target archives now use Zstd level 3 and a new archive
contract; this preserves every crate check while removing avoidable
high-compression CPU from the critical job. The remaining repeated
`jackin-capsule` compilations are distinct feature combinations, not dependency
cache misses, and require a separate semantic optimization rather than shards.
Docker E2E now follows the same Rust-owned execution contract on GitHub and
Velnor: xtask resolves the capsule path, carries the optional nextest filter,
and maps the runner-provided Docker temporary directory into the child process.
The shared per-crate workflow no longer uses shell logic or a Velnor-specific
execution branch to mutate `JACKIN_CAPSULE_BIN`; runner differences are inputs
to one implementation.

The measured Hygiene build-time ratchet also moved into the prepared xtask.
The command cleans only each measured workspace package, preserves dependency
outputs, resolves library sources from Cargo metadata, performs both builds
offline, and writes the ratchet JSON and step summary directly. This removes
the shell loop, the redundant network-enabled fetch, and the final
`cargo run -p jackin-xtask` that spent 10 seconds recompiling the enforcement
tool in run `29587713698`.

The same Hygiene evidence shows why the remaining multi-minute jobs cannot be
classified as cache setup. Miri `jackin-term` runs one complete crate job, but
main run `29499861725` spent 9,432 seconds interpreting 43 conformance tests;
`sanity_high_volume_plain_text` alone took about 46 minutes and
`vt_decawm_autowrap_off` about 61 minutes. The unit-test binary spent another
1,162 seconds, dominated by bounded hyperlink-map and resize-equivalence
stress cases. Any Miri optimization must preserve one attributable crate job
and the memory-semantics coverage while scaling interpreter-hostile stress
inputs or reusing an input-identical result; it must not shard the crate or
silently drop the conformance suite.
...
- [/research/engineering/ci/rust-tooling/rust-ci-tooling/](/research/engineering/ci/rust-tooling/rust-ci-tooling/) -- dependency hygiene, Codebook, coverage, and release-time tooling.
