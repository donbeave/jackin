...
description: Evaluates Rust CI tooling for dependency hygiene, spell checking, coverage, and snapshot verification.
---
...
Rust CI should add focused dependency, spelling, coverage, and snapshot gates only where each tool has a clear ownership boundary.

Dependency hygiene and Codebook checks are established; coverage and snapshot evidence remain incomplete.
...
This item records the current CI ownership boundaries plus release-tooling evidence that becomes relevant when tagged-release requirements exist. It also records tools rejected for this project profile: solo-maintainer, pre-release, `Apache-2.0`, and a binary CLI workspace shipping static binaries through a Homebrew tap and `cargo-zigbuild`.
...
## Current dependency and quality gates
...
`cargo-shear` is the adopted dependency-hygiene scanner. It runs on stable, is fast enough for every PR, and covers unused dependencies, misplaced dependencies, and unlinked Rust source files. The unlinked-source check guards against orphaned migration-residue source trees that normal compiler lints cannot see.
...
The substantive design for *what* is snapshotted — styled SVG goldens for TUI screens and CLI output, the PR before/after review experience, and the Rust-only tooling decision — is owned by [Visual snapshot testing (CLI & TUI)](/research/engineering/testing/visual-snapshots/). This item provides the dependency and the CI hook; that item defines the artifact format and assertions.
...
Both jobs run `codebook-lsp --root . lint --unique ...` over tracked files on the current branch and use the project dictionary in <RepoFile path=".codebook.toml">.codebook.toml</RepoFile>. The dictionary is the baseline of accepted jackin❯ vocabulary, tool names, British spellings used in docs, terminal/protocol acronyms, and synthetic fixture words; new vocabulary is checked against it.

## Release-tooling evidence

These tools match the distribution shape, but adopting either changes the release authority. [Release roadmap and contributor rules](/roadmap/) must own any migration decision once tagged-release requirements are concrete.
...
| `taplo`, `cocogitto` / `committed`, `cargo-chef`, `cargo-about` | **Open** *(unverified)* | TOML lint, conventional-commit lint, Docker build-cache, and license-attribution generation. Evaluate individually if a concrete need appears (e.g. `cargo-chef` if Rust Docker-layer build times become a problem). |
| `cargo-hakari` | **No-adopt** | Hygiene run `29397057453` measured clean, locked workspace builds at **152s before** and **152s after** generating the workspace-hack and managing every workspace dependency: a 0s / 0% delta. That does not justify the generated crate and dependency-maintenance cost. The exit-checked lane remains as reproducible evidence; see plan 012. |
| `miri` | **Adopt (advisory)** | Hygiene job `miri` runs four independent pure-crate invocations (`jackin-core`, `jackin-config`, `jackin-manifest`, `jackin-term`) with per-crate PASS/FAIL summary lines — no `\|\|` fallback masking. Advisory; not a merge gate. |
...
CI tools are pinned in <RepoFile path="mise.toml">mise.toml</RepoFile> alongside `cargo:cargo-nextest` and `cargo:cargo-zigbuild`:
...
- Keep RustSec, license, source, and unused-dependency checks independently attributable even where their evidence overlaps.
- Keep coverage informational; snapshot assertions belong to normal tests while snapshot-review tooling stays local.
- Install CI tools through the repository's pinned `mise` authority.
- Treat `dist` or `git-cliff` adoption as a release-system migration owned by roadmap and release documentation, not as unfinished work in this research page.
...
The `cargo-deny` config schema, license model, CI-integration shape, and tool facts in this item were verified against upstream sources across two June 2026 multi-source research passes. Confirmed against primary sources:
