...
This page records the design rationale for the console-native, recipe-driven layer of workspace-native PR verification that [Workspace-Native PR Verification](/roadmap/pr-verification/) defers past its shipped `jackin-dev pr sync` slice: the review-mode sidebar, the repo-committed verification recipe, isolation profiles, companion-role pinning, and documentation-preview automation, plus the prior-art survey that motivated building this instead of adopting an existing tool.
...
The shipped `jackin-dev pr sync --config copy|blank` flag (see [Workspace-Native PR Verification](/roadmap/pr-verification/)) already implements the copy-vs-blank choice as a one-shot sync-time flag; this section's design is the review-mode setting that exposes the same choice interactively, per-run, from inside the console.
...
## Per-pull-request bundle metadata (future)
...
The shipped `jackin-dev pr status <PR_NUMBER>` command already reports local/remote head SHAs and freshness by asking git and GitHub directly at call time (see <RepoFile path="crates/jackin-dev/src/main.rs" />); it does not yet persist its own metadata file, and `jackin-dev pr clean` does not yet report the disk freed. Both are open follow-ups, not blocking gaps, for this design.
...
The near-term novel piece is a shipped developer binary that prepares a host-side PR checkout with isolated jackin❯ configuration and state so jackin❯ can verify itself safely (shipped as `jackin-dev pr sync`, see [Workspace-Native PR Verification](/roadmap/pr-verification/)). The later console-native flow described on this page adds instance anchoring, recipe execution, documentation preview, and companion-role-branch pinning.
