---
title: "Console-native PR verification: design and prior art"
description: Defines a console-native, recipe-driven workflow for isolated pull-request verification inside workspaces.
---

## Summary

Pull-request verification should run from repository recipes inside isolated workspaces and report results through the console. The [Workspace-Native PR Verification roadmap item](/roadmap/pr-verification/) owns delivery status; this page preserves the review-mode, recipe, isolation, role-pinning, and documentation-preview rationale.
...
Three things make the console the right home for the richer flow, anchored to the instance the operator is already working in:

- **An instance already knows its pull request.** When the operator is working on a PR inside an instance, jackin-capsule — the in-container daemon — already tracks the working directory's branch and resolves it to the matching open pull request, with title and check status (see <RepoFile path="crates/jackin-capsule/src/pull_request.rs" /> and <RepoFile path="crates/jackin-capsule/src/pr_context.rs" />). The console can show those details the moment the instance is selected, so "verify this" is a natural next step from where the operator already is.
...
serve = { ready = "http://localhost:3000/", route_from = { dir = "docs/content", strip_suffix = ".mdx", index = "index", base = "/" } }
```
...
`jackin-dev pr sync --config copy|blank` provides the command-line copy-versus-blank boundary. Console review mode should expose the same safety choice interactively and per run; the [roadmap item](/roadmap/pr-verification/) owns that surface's delivery state.

## Companion role pinning design
...
## Documentation preview automation design
...
## Per-pull-request bundle metadata
...
`jackin-dev pr status <PR_NUMBER>` reports local and remote head SHAs by querying git and GitHub at call time (see <RepoFile path="crates/jackin-dev/src/main.rs" />). Persisted metadata and reclaimed-disk reporting are separate delivery questions owned by the [roadmap item](/roadmap/pr-verification/); the durable requirement here is isolation and reclaimability.
...
The distinguishing boundary is host-side verification with companion-role selection and isolated jackin❯ state, anchored to the instance under review. `jackin-dev pr sync` provides the checkout and isolation substrate; [Workspace-Native PR Verification](/roadmap/pr-verification/) owns all delivery facts for console integration.

## Decision implications

- PR identity belongs in the selected instance detail; recipe execution belongs in a reversible review mode rather than a separate application surface.
- Changed-file relevance orders and suggests commands but never silently skips verification.
- Every recipe command, interactive shell, and freshly built jackin❯ process shares the same isolated checkout, configuration, state, and companion-role selection.
- Documentation preview is a recipe-declared server lifecycle with readiness and process-group cleanup, not docs-engine-specific product code.
- The [roadmap item](/roadmap/pr-verification/) owns implementation sequencing and remaining delivery work.
