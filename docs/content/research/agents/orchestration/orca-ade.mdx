...
description: "Compares Orca's worktree-native agent environment with jackin❯ to identify reusable fleet and desktop interaction patterns."
---
...
Orca validates worktree-native fleet and desktop interaction patterns, but its weaker isolation boundary makes those UX ideas references rather than an architecture to copy.
...
Every borrowed idea must survive the values filter from the [Agent Orchestration Program](/research/agents/orchestration/program-research/#jackin-values-used-for-evaluation).
...
| Mobile companion (monitor, manage) | Yes | Lower priority; relevant to [jackin❯ desktop Agent Hub](/research/product/desktop/agent-hub/) |
...
| Mobile companion | iOS + Android | Not yet; see [jackin❯ desktop Agent Hub](/research/product/desktop/agent-hub/) |
...
The iOS/Android companion for remote monitoring and task management is an idea for the [jackin❯ desktop Agent Hub](/research/product/desktop/agent-hub/) roadmap item, which uses the daemon as its state/event backend.
...
5. **Self-hosted relay.** Orca's mobile-companion relay is a separate infrastructure concern. Does the jackin❯ daemon design (see [jackin❯ daemon](/roadmap/jackin-daemon/)) make self-hosted relay a natural later capability or an unrelated problem?

## Relationship to the research program

This item extends [Agent Orchestration Program](/research/agents/orchestration/program-research/) with a UX-first desktop tool. Orca does not fit neatly into Track A (fleet ops, multicode-style) or Track B (containment, Hazmat/Docker Sandboxes-style). It is a third reference point: what the user-facing layer looks like when the isolation concern is handed off to git instead of containers.
...
| Mobile companion | [jackin❯ desktop Agent Hub](/research/product/desktop/agent-hub/) |
