...
**Research state**: Open — research and design proposal (Phase 4, [Agent Orchestration Program](/research/agents/orchestration/program-research/))
...
| Context and memory | Resolve a workflow-scoped memory brief from the [persistent storage and workspace memory layer](/research/agents/orchestration/memory/persistent-storage-layer/) before each agent session. Do not depend on one runtime's private memory as the shared source of truth. |
...
| [Helmor](https://github.com/dohooo/helmor) | Open-source local workbench at `dohooo/helmor`; published macOS builds (v0.25 in 2026 added Codex 1.0 and Claude Code 2.0). | Desktop workbench for planning, running, reviewing, testing, merging, and comparing coding-agent work across worktrees and terminals. | Strong visual-workbench reference; optimizes for human review and choosing between parallel attempts. | Review/merge support appears central; full GitHub lifecycle depth needs hands-on validation. | **UX benchmark, not dependency.** Treat its visual comparison/review model as evidence against building only GitHub comments plus CLI status. |
...
| Container-only isolation may be weaker than VM/forkable sandbox competitors. | Keep [selectable sandbox backends](/research/platform/security/sandbox-backends/selectable-sandbox-backends/) compatible with the run model. Do not bake "Docker container" into workflow state. |
...
`MemoryBrief` is the compact context package resolved for one phase/session. It should include only the memories relevant to the current run, workspace, role, workflow, and agent: operator preferences, workspace facts, workflow rules, recent run summaries, and explicit operator notes. It is built from the [persistent storage and workspace memory layer](/research/agents/orchestration/memory/persistent-storage-layer/) and delivered through runtime adapters; it is not copied from Claude Code's or Codex's private memory store.
...
| [Persistent storage layer](/research/agents/orchestration/memory/persistent-storage-layer/) | Durable run records, event logs, GitHub links, review results, and retry/cancel state. |
...
The current `status` and `snapshot` control calls are enough for display, but not enough for reliable workflow orchestration. The runner needs a typed control surface that can manipulate sessions without pretending terminal text is an API. [Terminal observation and automation](/research/agents/orchestration/terminal-observation/terminal-observation-automation/) owns the detailed read/wait/send/capture/tab/session API shape; this section names the subset the workflow runner depends on.
...
[ACP](https://agentclientprotocol.com/get-started/introduction) is the right fit for "a provider or orchestrator wants to talk to an agent process with structured JSON-RPC semantics." It is an open JSON-RPC standard for connecting code editors to AI coding agents, originally championed by Zed and adopted by JetBrains and 25+ agents through 2026. MCO's ACP support is the best current signal that review/fan-out tools may converge there. jackin❯ should watch ACP and avoid inventing an incompatible custom transport for agent-provider adapters.
...
- [Agent Orchestration Program](/research/agents/orchestration/program-research/) — parent research program.
...
- [Persistent storage layer](/research/agents/orchestration/memory/persistent-storage-layer/) — durable run records and event logs.
...
- [Native APM support](/research/agents/runtimes/package-manager-support/native-apm-role-support/) — package/install layer for skills, prompts, and MCP clients used by workflow profiles.
