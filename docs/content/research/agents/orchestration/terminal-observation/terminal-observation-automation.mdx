...
description: "Evaluates terminal capture and semantic observation as foundations for agent status, automation, and operator tooling."
---
...
Programmatic terminal automation needs structured observation, controlled input, wait conditions, and artifact capture behind the Capsule protocol rather than ad hoc `docker exec` scraping.
...
This is useful for role smoke tests, docs demos, and reproducing agent runtime issues. It should not decide Git branches, PR lifecycle, merge policy, or task queues; that belongs to [agent workflow orchestration](/research/agents/orchestration/workflow-systems/agent-workflow-orchestration/).
...
Delivery sequencing belongs to the [jackin❯ Capsule control-plane roadmap](/roadmap/jackin-capsule/) and its consumer roadmap items. This dossier retains the protocol, observation, input, artifact, and automation constraints.
...
- [Agent workflow orchestration](/research/agents/orchestration/workflow-systems/agent-workflow-orchestration/) — future workflow runner that needs session read/wait/send primitives but should not own terminal parsing.
...
- [TUI Design Decisions](/reference/tui/) and [visual snapshot testing (CLI & TUI)](/research/engineering/testing/visual-snapshots/) — deterministic renderer/golden-test work that should converge with this terminal-frame artifact model on a shared styled-cell schema; that item asserts render regressions, this one captures live sessions for orchestration.
- [Persistent storage layer](/research/agents/orchestration/memory/) — durable home for recordings, traces, workflow events, and artifact indexes.
