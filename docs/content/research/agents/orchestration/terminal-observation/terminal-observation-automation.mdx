...
- [TUI Design Decisions](/reference/tui/) and [visual snapshot testing (CLI & TUI)](/research/engineering/testing/visual-snapshots/visual-snapshot-testing/) — deterministic renderer/golden-test work that should converge with this terminal-frame artifact model on a shared styled-cell schema; that item asserts render regressions, this one captures live sessions for orchestration.
- [Persistent storage layer](/research/agents/orchestration/memory/persistent-storage-layer/) — durable home for recordings, traces, workflow events, and artifact indexes.
