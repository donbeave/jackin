...
description: "Evaluates a cross-agent archive that captures, normalizes, searches, and resumes conversations across supported runtimes."
---
...
Déjà Vu should normalize provider-native histories into a host-owned, purge-surviving archive; provider, storage, privacy, and resume boundaries remain unresolved.
...
- **No durable raw log.** Raw bytes are dropped after parsing, so there is no lossless replay source unless [terminal observation and automation](/research/agents/orchestration/terminal-observation/terminal-observation-automation/) adds opt-in recording first.
- **~10k-line horizon.** Anything older than the retained scrollback is gone.
- **Secrets.** Rendered terminal output can contain tokens, paths, and file contents; screen-scrape capture must be opt-in and redaction-aware, consistent with the recording stance in [terminal observation and automation](/research/agents/orchestration/terminal-observation/terminal-observation-automation/).
...
Keying by workspace (then instance/date, then agent) matches the browse flow directly: workspace → instance/date → conversation. Whether the index is Déjà Vu-private or a table inside the persistent storage layer's DB is an [open question](#limitations-and-open-questions).
...
- **TUI (`jackin deja-vu`, console-style).** The headline deliverable. A `jackin console`-shaped browser: pick a workspace, narrow by instance or date, pick a conversation, read the rendered thread top to bottom. Follows the [TUI design decisions](/reference/tui/) (W3C Tabs navigation, scrollable blocks via TermRock scroll/`Viewport`, semantic Theme roles for neutral palette).
...
Delivery sequencing belongs to the [Agent Orchestration Program](/research/agents/orchestration/program-research/) and a dedicated roadmap item. This dossier retains the provider, archive, browse, and fallback capability boundaries.
...
- **Semantic / vector search.** V1 is index + keyword. Semantic retrieval waits until there are enough captured conversations to evaluate quality, aligned with the [persistent storage layer](/research/agents/orchestration/memory/) memory direction.
...
- **Orchestration.** Déjà Vu provides the read surface a future orchestrator consumes (see [Agent workflow orchestration](/research/agents/orchestration/workflow-systems/agent-workflow-orchestration/)); it does not schedule or drive agents.
...
| Déjà Vu-private index vs a table in the persistent storage layer? | Prefer reusing the [persistent storage layer](/research/agents/orchestration/memory/) once it lands; ship a small private SQLite index first if Déjà Vu moves earlier. |
...
| Do we capture tool calls/diffs/thinking in V1? | Capture them when the native source provides them for free; never block the must-have (prompts + visible answers) on them. Thinking and the model-facing system prompt need vector D and remain outside the current evidence layer. |
...
- [Persistent storage and workspace memory layer](/research/agents/orchestration/memory/) — the index/DB home Déjà Vu should reuse, and the cross-agent memory direction it complements.
- [Terminal observation and automation](/research/agents/orchestration/terminal-observation/terminal-observation-automation/) — owns the PTY/`DamageGrid` read APIs and opt-in recording the screen-scrape fallback would build on.
...
- [Agent workflow orchestration](/research/agents/orchestration/workflow-systems/agent-workflow-orchestration/) — future consumer of the captured-conversation read surface.
