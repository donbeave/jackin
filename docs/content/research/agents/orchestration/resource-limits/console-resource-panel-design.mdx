...
description: "Designs a console panel for inspecting per-agent CPU, memory, process, storage, and limit state."
---
...
The console should expose per-agent consumption, configured ceilings, and host headroom from governed resource data before it offers any control actions.
...
- Time-series graphs (sparklines, rolling 5-minute window) are outside this design; [usage surfaces](/reference/usage/) expose current provider limits rather than local usage trends.
...
- **Sampling backend abstraction.** Sample-via-Docker today; sample-via-cgroup-files when [selectable backends](/research/platform/security/sandbox-backends/) lands. Which module owns the abstraction? Candidate: the runtime backend trait owns it, mirroring how each backend already owns mount translation.
- **Always-on vs on-demand sampling.** multicode samples constantly even when the panel isn't visible, because the data feeds usage aggregation. jackin❯ V1 doesn't need that yet — pause-when-closed looks like the right call. Revisit when [Token & Cost Telemetry](/research/agents/telemetry/token-cost-telemetry/) needs a continuous series.
...
- <RepoFile path="crates/jackin/src/console/adapter.rs" /> — keybinding wiring
