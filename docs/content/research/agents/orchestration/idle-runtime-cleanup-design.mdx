...
description: "Defines safe idle hooks for reclaiming runtime resources and preparing warm agents for repeated autonomous work."
---
...
Idle cleanup should run bounded, role-declared hooks only after a verified idle transition, with host policy retaining authority over what may execute.
...
This matters most for the [autonomous task queue](/roadmap/autonomous-task-queue/), which keeps containers warm waiting for the next task. Without cleanup, those warm containers accumulate state every cycle. It's a small feature with disproportionate impact for long-running fleets, and it generalizes naturally to anything a role wants done while idle (commit checkpoint snapshots, push WIP branches, evict caches).
