...
description: "Designs a durable task queue with isolated dispatch, concurrency limits, recovery, budget gates, and operator approvals."
---
...
An autonomous queue needs durable host-owned task state, isolated dispatch, bounded concurrency and budgets, deterministic recovery, and explicit approval gates.
...
This schema is not implemented. It depends on the [Task source abstraction](/roadmap/task-source-abstraction/) adding source config and workspace source assignment, and on the [Persistent storage layer](/research/agents/orchestration/memory/) adding durable task state.
