...
description: "Designs operator-controlled session retention and faithful resume across exits, mounts, worktrees, and runtime state."
---
...
Faithful resume requires explicit retention for clean and dirty exits, persisted launch inputs, verified mounts and sidecars, and purge semantics that operators can predict.
...
Terminal cleanup already has one authoritative mechanism: <RepoFile path="crates/jackin-runtime/src/runtime/cleanup.rs">crates/jackin-runtime/src/runtime/cleanup.rs</RepoFile> routes terminal statuses through `prune_instances`, `purge_container_filesystem`, socket-dir deletion, sibling-lock deletion, `InstanceIndex::remove_many`, orphan-lock reaping, and stale-`active` reconciliation. Exit and launch must use that same lifecycle and distinguish "kept for resume" from "terminal, gone" consistently.

An involuntary runtime loss exposes the second gap. If the host VM or Docker daemon disappears, live role containers can exit `255` while their sidecars exit `0`; surviving homes, conversations, data directories, and project mounts are then mislabeled `failed_setup`. Reconnect refuses a stopped container, while a bare container restart fails when `/jackin/run/agent.toml` must be rematerialized. This reproducible failure establishes the required behavior: reconcile host-caused exits honestly and restore from surviving state through the normal console flow.
...
The current architecture provides these reusable primitives:
...
The exit/restore path and rich-dialog implementation contain the following defects, verified against source at the evidence cutoff.
...
| F5 | Medium | Verification surfaces the actual per-file diff (status, path, before/after content) through the `I`-key Inspect surface. Missing: the unpushed branch name / ahead-count is never shown (only a generic "unpushed commits" tag), and Inspect is optional rather than a required acknowledge step before Keep/Discard. |
...
| F9 | Medium | Terminal-status pruning removes state directories and index rows through `prune_instances` + `purge_container_filesystem` + `InstanceIndex::remove_many`; the live exit path must reach the same lifecycle without requiring manual prune. |
| F10 | Medium | Explicit prune removes terminal rows from `instances.json`; launch and exit require reconciliation before operator choices. |
| F11 | Low | `purge_container_filesystem` removes sibling container locks and `prune_instances` reaps orphan name locks; any additional repo-lock sweep must be justified by current evidence. |
| F12 | Medium | `prune_instances` reconciles a stale `active` row with no container to `crashed`, but launch and console flows do not run that reconciliation before restore decisions. |
| F13 | Low | `start_container` is already shared by hardline reconnect and the launch pipeline's stopped-container restore resolution. |
| F14 | Medium | The manifest stores role and image identity, but not the complete mount plan, environment-source references, and runtime shape required for faithful replay. |
| F15 | High | Stopped-container reconnect does not rematerialize the socket directory and `agent.toml` or establish sidecar health before starting the role container. |
| F16 | Medium | **A daemon/VM mass-kill mislabels healthy sessions.** When the Docker daemon dies under N live `active` sessions (OrbStack upgrade, host power-off, `docker stop` on shutdown), the role containers exit `255` and the next inspect stamps each one `failed_setup` — a status reserved for "the setup hook failed", not "the host went away". It is a restore candidate by luck (`is_restore_candidate()` includes `FailedSetup`), but the label is wrong and the index is never reconciled against the true cause. No distinct `InstanceStatus` variant for "killed by host" exists at the evidence cutoff. This is the involuntary sibling of F12 (stale `active` rows) and wants the same launch-time reconciliation, plus an honest terminal status for "killed by host" distinct from "failed setup". |
...
<Aside type="tip">
`prune_instances` is the single cleanup authority: it combines `purge_container_filesystem` with `InstanceIndex::remove_many`. Any exit-path cleanup must route through that authority rather than creating a parallel teardown path. Roadmap owns the delivery change.
</Aside>
...
Delivery sequencing, migration work, and current status belong to [Session keep and resume](/roadmap/session-keep-and-resume/). This dossier retains the audited lifecycle facts, restore model, TUI flow, and storage constraints.
...
- In-container session continuity is tier-dependent: Tier 0/1 reconnect to the live agent session for free (the container never died), while Tiers 2–3 restore the bind-mounted home/conversation but start a fresh agent process. Should Tiers 2–3 also attempt to resume the agent's own session log (e.g. `claude --resume`), or is restoring the home enough? Scope this against session reconciliation in [Console agent session control](/roadmap/console-agent-session-control/).
- Does extending `dirty_exit_policy` to cover clean exits require a new config schema version, or can the existing field's semantics change without a migration since the on-disk representation (`ask` / `keep` / `discard` strings) does not itself change shape? The answer determines whether the change needs the full migration ceremony.
...
- [Déjà Vu](/research/agents/orchestration/conversation-capture/) — the backstop for "never lose valuable context": even when a session is unrecoverable, the cross-agent conversation archive survives instance teardown.
