...
**Research state**: Open — phased implementation program (fleet-operations track plus containment/recovery track)
...
This page is the task plan for the orchestration program. It keeps only the decision inputs needed to explain the roadmap shape; full external-product dossiers live under [/research/](/research/).
...
- [cellshot](https://github.com/kitlangton/cellshot) and adjacent terminal recorder/test tools are the strongest reference for **agentic terminal observation and automation**: structured terminal frames, visible-text waits, input injection, artifact bundles, and replayable traces. They directly inform [Terminal observation and automation](/research/agents/orchestration/terminal-observation/terminal-observation-automation/), the Phase 2 work that makes Capsule sessions script-addressable — Playwright-like control for jackin❯ instances, tabs, prompts, waits, and captures — without treating Docker logs or screenshots as the API.
...
That wider scope is why this page is a program roadmap, not a multicode-specific task.
...
| Run Docker/Compose without trusting the host daemon | Docker Sandboxes private Docker Engine; Hazmat Tier 3 | Keep host socket mounting out of scope; make private-daemon backends the Docker-capable path under [selectable sandbox backends](/research/platform/security/sandbox-backends/selectable-sandbox-backends/) |
...
| Docker tier decision | Treat host Docker daemon access as a policy boundary, not a convenience default | [Selectable sandbox backends](/research/platform/security/sandbox-backends/selectable-sandbox-backends/), [network egress policy](/roadmap/network-egress-policy/) |
...
|---|---|---|
| Agent isolation | `bwrap` + `systemd-run`; Apple `container` | Have — Docker + DinD; planned — [selectable backends](/research/platform/security/sandbox-backends/selectable-sandbox-backends/) |
...
| Per-agent token / cost / OOM tracking | Yes — usage aggregation service | *Gap — see [Token & Cost Telemetry](/research/agents/telemetry/token-cost-telemetry/)* |
...
| Persistent storage backend | Per-workspace SQLite + per-workspace JSON | *Gap — see [Persistent storage layer](/research/agents/orchestration/memory/persistent-storage-layer/)* |
...
| Strong local isolation | macOS user + seatbelt + `pf`; VM tier for hardest cases | Per-sandbox microVM | [Selectable sandbox backends](/research/platform/security/sandbox-backends/selectable-sandbox-backends/) |
...
| [Terminal observation and automation](/research/agents/orchestration/terminal-observation/terminal-observation-automation/) | cellshot-style visible waits, terminal frames, input injection, artifact bundles, and replay traces | jackin❯ Capsule control plane; agent runtime status for semantic waits |
| [Déjà Vu — cross-agent conversation capture](/research/agents/orchestration/conversation-capture/deja-vu/) | SpecStory-style capture of every operator prompt and agent answer, normalized and archived for browse/replay | Multi-runtime adapter seam; persistent storage; jackin❯ Capsule control plane |
...
| [Public attribution and project growth](/research/product/growth/attribution/public-attribution-and-growth/) | Orca-style Git commit trailers and PR metadata as discoverability surfaces | Operator handler system; local repo policy |
...
| Item | Inspiration in multicode | Depends on |
|---|---|---|
| [Persistent storage layer](/research/agents/orchestration/memory/persistent-storage-layer/) | Per-workspace SQLite | — |
| [Token & Cost Telemetry](/research/agents/telemetry/token-cost-telemetry/) | Usage aggregation | Persistent storage; agent runtime status |
...
| [Agent workflow orchestration](/research/agents/orchestration/workflow-systems/agent-workflow-orchestration/) | Multi-agent task pipeline with parallel review fan-out and deterministic phase routing | Task source; persistent storage; agent runtime status; Capsule session control |
...
| [Container credential exposure](/research/platform/security/credential-exposure/container-credential-exposure/) | Docker Sandboxes credential proxy; Hazmat secret store | Host bridge; credential source pattern |
...
| Item | Primary inspiration | Depends on |
|---|---|---|
| [Selectable sandbox backends](/research/platform/security/sandbox-backends/selectable-sandbox-backends/) | Docker Sandboxes microVM + private daemon; Hazmat Tier 3 decision | Runtime backend refactor |
...
| [Devcontainer parity](/research/platform/runtime/devcontainer-parity/) | Claude/devcontainer firewall and reproducibility pattern | Network egress policy |
...
| [Architecture decision records](/reference/adrs/) | Hazmat design assumptions and verification boundary | Structural foundation |
| [runtime/launch Behavioral Spec](/reference/developer-reference/specs/runtime-launch/) | Hazmat TLA+ setup/launch ordering discipline | Structural foundation |
...
- [Selectable sandbox backends](/research/platform/security/sandbox-backends/selectable-sandbox-backends/) —
...
Research snapshots: May 9, 2026; agent-status addendum May 22, 2026. The cross-project index for repeated prior-art checks now lives in the [Research Watchlist](/research/watchlist/); this page keeps only the program-specific synthesis and source snapshots.
...
- [Terminal observation and automation](/research/agents/orchestration/terminal-observation/terminal-observation-automation/) — jackin❯ roadmap item for `cellshot`-style terminal frames, visible waits, input injection, artifact bundles, recording/replay, and automation scripts on top of Capsule sessions.
