...
description: "Designs host notifications for agents that finish, block, or await input without disrupting operator focus."
---
...
Attention prompts should originate from semantic runtime-state transitions and use focus-aware, operator-controlled delivery; the signal and platform contracts remain open.
...
- [jackin❯ desktop Agent Hub](/research/product/desktop/agent-hub/) — native macOS surface that should display attention/ready-for-review state and would own the first Ghostty click-to-focus UX.
...
- [Agent Orchestration Program → Live agent observability](/research/agents/orchestration/program-research/) — adjacent work on surfacing what the agent is doing in real time; attention prompts would close the loop the other direction.
