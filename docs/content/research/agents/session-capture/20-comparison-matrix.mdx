...
description: "SpecStory is the strongest reusable baseline, but its two-of-six roster overlap and lack of a host-side purge-surviving archive leave the core Déjà Vu gap open."
---

## Summary

Ranks session-capture options by agent coverage, capture robustness, content fidelity, search, sharing, health, and license.
**Verification cutoff:** 2026-07-03. Tools come from [01](/research/agents/session-capture/01-specstory-product/)/[02](/research/agents/session-capture/02-specstory-cli-internals/) (SpecStory), [11](/research/agents/session-capture/11-open-source-exporters/) (OSS), and [12](/research/agents/session-capture/12-platforms-and-observability/) (platforms). Tier value means usefulness to a jackin❯ session-capture capability: roster coverage × mechanism robustness × output/search/share quality × maintenance health × license reusability. It does not rank personal developer products.
...
jackin❯ runs Claude Code, Codex, Amp, Kimi, OpenCode, Grok ([30](/research/agents/session-capture/30-jackin-fit/)). Coverage of that roster by the cross-agent tools:
...
Two consequences: (a) no off-the-shelf tool solves jackin❯'s roster — best case 3/6; (b) Kimi and Grok providers would be first-in-market, and both turn out to have *good* native surfaces (Kimi versioned `wire.jsonl` + ACP; Grok documented session dirs + ACP `updates.jsonl` — [10](/research/agents/session-capture/10-agent-session-stores/)).
...
- [SpecStory issue 146](https://github.com/specstoryai/getspecstory/issues/146)
