...
description: "jackin❯ has host-side foundations for Déjà Vu but lacks provider adapters, Amp stream capture, and a purge-surviving archive."
---

## Summary

Maps session-capture evidence to jackin❯ foundations, provider gaps, implementation options, and unresolved Déjà Vu questions.
**Verification cutoff:** 2026-07-03. This chapter maps evidence onto jackin❯ without making design decisions; feature shape belongs to the [Déjà Vu conversation-capture design](/research/agents/orchestration/conversation-capture/), which this session-capture dossier underpins.


## Question and scope

How does the session-capture evidence map onto current jackin❯ foundations, gaps, and design choices?

## Method

The fit analysis combines repository touchpoints with the agent-store, SpecStory, and comparison findings; it records design implications rather than delivery status.

## Findings
### Where this dossier sits relative to Déjà Vu

Déjà Vu already proposes the capture-store-browse layer: per-agent providers → one normalized record → protobuf archive + SQLite index under `~/.jackin/data/deja-vu/` → TUI/CLI/MCP surfaces. This dossier is the market- and implementation-evidence layer under that design. The evidence confirms **Déjà Vu's core bets and identifies four additional inputs.**

### Findings supported by the evidence

- **Native-file reading is the winning vector.** SpecStory's entire 58k-LOC implementation ([02](/research/agents/session-capture/02-specstory-cli-internals/)) and effectively every OSS tool in [11](/research/agents/session-capture/11-open-source-exporters/) read the agents' own stores; nobody PTY-scrapes; hooks and OTel appear only as liveness/metrics side channels. Déjà Vu's vector ranking (A first, F last) matches the market unanimously.
...
| Agent | Native store (details: [10](/research/agents/session-capture/10-agent-session-stores/)) | Existing ecosystem to lean on | Difficulty signal |
|---|---|---|---|
...
| Grok | see [10](/research/agents/session-capture/10-agent-session-stores/) | Nothing found | Verify live |
...
### Open questions for Déjà Vu

| Question | Why it's now live |
|---|---|
| Verify Grok and Kimi store layouts inside real jackin❯ containers | Only roster members with no third-party evidence base; [10](/research/agents/session-capture/10-agent-session-stores/) records the best external knowledge |
...
Preserve host ownership and purge-independent retention while adding provider adapters, explicit completeness signals, and a separate searchable archive.

## Limitations and unknowns

The mapping does not replace provider-format verification or settle unresolved retention, privacy, and resume semantics.
