...
description: "SpecStory’s CLI proves that provider adapters over native stores can normalize, index, render, and reconstruct sessions without PTY scraping."
---

## Summary

Explains how SpecStory watches native agent stores, normalizes sessions, renders Markdown, indexes history, and supports resume.
**Verification cutoff:** 2026-07-03, using [specstoryai/getspecstory](https://github.com/specstoryai/getspecstory) CLI v2.0.0 (2026-06-29). Everything in this chapter is **tier A** — read from source — unless marked otherwise. Paths are relative to the repo root.


## Question and scope

Which SpecStory mechanisms are durable prior art for cross-agent capture, and where are their limits?

## Method

The findings come from a source audit of SpecStory CLI v2.0.0 at the verification cutoff, with repository paths recorded beside each mechanism.
...
This SPI shape — locate, parse, normalize, watch, reconstruct, per provider — is the direct prior art for [Déjà Vu](/research/agents/orchestration/conversation-capture/)'s per-agent provider layer.
...
The editor-side stores (Cursor IDE `state.vscdb` `cursorDiskKV` keys, VS Code Copilot `workspaceStorage/<hash>/chatSessions/*.json`) are read by the **closed** extension — mechanism confirmed only via FAQ statements and community reverse engineering; see [01](/research/agents/session-capture/01-specstory-product/) (tier B/C).
...
5. **Watching costs CPU when done naively** — the busy-wait and polling complaints in [01](/research/agents/session-capture/01-specstory-product/) show even the no-PTY design needs debounce discipline (SpecStory debounces cloud sync at 10 s but not the file-watch layer itself).

## Implications for jackin❯

Adopt a provider SPI, disposable index, idempotent renderer, and explicit unsupported-capability floor; keep native stores as evidence rather than archive authority.

## Limitations and unknowns

The audit is version-bound and does not independently exercise every provider store or unreleased provenance path.

## Sources

- [SpecStory source](https://github.com/specstoryai/getspecstory)
