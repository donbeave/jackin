...
description: "Compares reliable Claude authentication strategies across host, container, token refresh, and multi-account workflows."
---
...
A restored, host-owned Claude home remains the simplest authentication baseline, but token refresh and multi-account isolation must be revalidated before this page guides current work.
...
- <RepoFile path="crates/jackin-config/src/app_config/roles.rs">crates/jackin-config/src/app_config/roles.rs</RepoFile> — auth mode resolution (`resolve_auth_forward_mode`, `set_agent_auth_forward`)
- <RepoFile path="crates/jackin-config/src/app_config.rs">crates/jackin-config/src/app_config.rs</RepoFile> — `AuthForwardMode` enum, persistence types
...
- <RepoFile path="crates/jackin-instance/src/auth.rs">crates/jackin-instance/src/auth.rs</RepoFile> — auth provisioning semantics for the new mode
...
- [Auth reliability and convenience program](/roadmap/auth-reliability-program/) — umbrella program that places this design in context; its Phases 1–4 address the concurrent-session concerns documented here.

## Related work

- <RepoFile path="crates/jackin-config/src/app_config.rs">crates/jackin-config/src/app_config.rs</RepoFile> — `AuthForwardMode` enum, persistence types
- <RepoFile path="crates/jackin-config/src/app_config/roles.rs">crates/jackin-config/src/app_config/roles.rs</RepoFile> — auth mode resolution
...
- <RepoFile path="crates/jackin-instance/src/auth.rs">crates/jackin-instance/src/auth.rs</RepoFile> — credential forwarding and state preparation
