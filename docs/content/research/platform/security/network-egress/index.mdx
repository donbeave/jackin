...
description: Defines honest enforcement labels and a phased backend strategy for restricting capsule network egress.
---

## Summary

Capsule egress controls must label actual enforcement strength and use backend-specific mechanisms behind one policy model.
...
The upstream Claude Code devcontainer pattern is a third reference for a container-local allowlist. It is pragmatic, but enforcement inside the guest is weaker than host-side mediation. See [Devcontainer parity](/research/platform/runtime/devcontainer-parity/).
...
One candidate shape beyond today's `DockerGrants.network` tri-state is:
