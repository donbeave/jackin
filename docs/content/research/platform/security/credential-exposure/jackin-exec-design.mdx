...
**Research state**: Implemented — this is the design record for the shipped `jackin-exec` on-demand credential mechanism; see [jackin-exec: Secure Execution Wrapper](/roadmap/jackin-exec/) for current roadmap status and remaining work.
...
Works on top of any jackin❯ backend. Does not require the Apple Container backend to land first. See [Agent isolation architecture](/research/platform/security/isolation-architecture/agent-isolation-architecture/) for the complete picture.
