...
description: Compares process-level sandboxing mechanisms, enforcement limits, and integration paths across supported platforms.
---
...
Process-level sandboxing can harden capsules now, but each supported platform needs explicit enforcement limits and integration paths.

Revalidate platform mechanisms and upstream tool capabilities before selecting an implementation path.
...
The [selectable sandbox backends](/research/platform/security/sandbox-backends/) roadmap item covers the container-to-microVM step. This item covers the process sandbox tier — the innermost ring that wraps individual shell commands, build steps, and AI-generated code invocations.
...
|---|---|
| [Selectable sandbox backends](/research/platform/security/sandbox-backends/) | Process sandbox is the innermost tier; microVM is the outermost. Both are needed for the strongest isolation model. This item fills the per-operation gap that VM boundaries cannot close. |
...
- [Selectable sandbox backends](/research/platform/security/sandbox-backends/) — the VM/container isolation tier this item complements
