---
title: "jackin❯ daemon — design proposal"
description: "Design rationale for a long-running, per-operator-user host daemon: what it owns, what it deliberately excludes, and the open lifecycle/install/protocol/security questions that need answers before implementation starts."
---

This is the original design-proposal writeup for the [jackin❯ daemon roadmap item](/roadmap/jackin-daemon/), kept for the design-history record. Nothing described below is implemented; the roadmap item tracks current status.

## Spike resolution (2026-07-04)

Plan 042 settled the daemon's foundation enough to split implementation work
without shipping a production daemon in the spike itself. The selected shape is:
...
- **Control socket:** the daemon listens only on `~/.jackin/run/jackin-daemon.sock` under a `0700` directory. The wire format is newline-delimited JSON with `{ id, protocol_version, type, ... }`, one request and one response per connection for v1. Long-lived event streams can be added as a later explicit request type.
...
- **Secrets and logs:** the daemon must install the same redaction discipline as launch diagnostics before any adapter can carry credentials. Tokens are not persisted in daemon-owned storage; feature stores own their at-rest format. Production daemon startup must disable core dumps where the platform supports it before live-auth-sync or host-bridge land.
...
control message shape with a one-connection Unix listener and the first narrow
...
`live-auth-sync` and `host-bridge` remain deferred until the lifecycle/socket
foundation is production-grade. They are the first adapters that can expose
credentials or host actions, so they must not be used to prove the daemon's base
shape.
...
This proposes introducing **the jackin❯ daemon** as a first-class concept the rest of the project can compose on. It is deliberately written before any one of the dependent features picks an implementation path, so the daemon's lifecycle, install method, security posture, and CLI surface are designed *once* and reused.

The daemon draft PR remains the right place to adjust the base contract while Desktop is being designed. The [jackin❯ desktop Agent Hub](/research/product/desktop/agent-hub/jackin-desktop-agent-hub/) roadmap depends on this daemon, so daemon API gaps discovered there should be fixed here instead of papered over in the macOS app.
...
## Design questions resolved by the spike

The original open questions are kept below as design history, with the Plan 042
resolution now recorded above.
...
- **Secret-redacted logs.** Daemon log output must redact tokens the same way `tracing::debug!` already does for `GithubAuthContext` and `GithubProvisionOutcome` (manual `Debug` impls; see <RepoFile path="crates/jackin-instance/src/lib.rs">crates/jackin-instance/src/lib.rs</RepoFile>).
...
- **Keep-awake `caffeinate`.** Today's per-command reconciler (see <RepoFile path="crates/jackin-host/src/caffeinate.rs">crates/jackin-host/src/caffeinate.rs</RepoFile>) becomes a daemon-owned watcher: the daemon subscribes to "any role from a keep-awake workspace running" via Docker events and holds the caffeinate lock. CLI commands no longer reconcile.
...
1. **Phase 0 — design.** Answer every "Open design questions" subsection. Pick lifecycle, install, control protocol, security posture. Land an updated version of the roadmap item with answers.
...
- [jackin❯ desktop Agent Hub](/research/product/desktop/agent-hub/jackin-desktop-agent-hub/) — native macOS status bar and desktop companion that consumes daemon state for workspaces, sessions, GitHub PRs, account status, notifications, and approved host actions.
- <RepoFile path="crates/jackin-host/src/caffeinate.rs">crates/jackin-host/src/caffeinate.rs</RepoFile> — today's per-command keep-awake reconciler; first migration target in Phase 3.
