...
description: Defines `tmpfs` and ephemeral per-mount isolation modes that express storage lifetime independently of Git semantics.
---

## Summary

Storage lifetime should be independent of Git isolation, with `tmpfs` and ephemeral modes representing distinct disposable-mount behavior.
Design proposal for two new [per-mount isolation](/guides/workspaces/#per-mount-isolation) modes — `tmpfs` and `ephemeral` — that cover **storage kind** rather than **git semantics**. Tracked as an open roadmap item: [Ephemeral mount modes](/roadmap/ephemeral-mount-modes/).
...
(plus `clone`, already shipped). Validation:
...
- **Apple container** (when [selectable
  backends](/research/platform/security/sandbox-backends/) ships):
...
- **Is this its own leaf or an extension to mount isolation?**
  This is treated as a standalone item because it ships
...
- [Ephemeral mount modes (roadmap item)](/roadmap/ephemeral-mount-modes/)
...
- [Per-mount isolation](/guides/workspaces/#per-mount-isolation) —
  the existing modes; this proposal extends the enum vocabulary
