...
**Research state**: Deferred — shared-kernel architecture does not address the kernel isolation gap for the current macOS ARM audience; see [smolvm backend research](/research/platform/isolation/smolvm-backend/) for the active VM isolation track
...
Backend operations should emit `debug_log!("orbstack", …)` (the existing macro in <RepoFile path="crates/jackin/src/console/adapter.rs">crates/jackin/src/console/adapter.rs</RepoFile>) so the firehose under `JACKIN_DEBUG=1` reconstructs the full machine lifecycle. Minimum required lines:
...
All three are additive. The instance manifest at <RepoFile path="crates/jackin-instance/src/manifest.rs">crates/jackin-instance/src/manifest.rs</RepoFile> also needs a `backend_kind`/`provider_kind` axis split, which is per-instance metadata (not a versioned schema in the AGENTS.md sense) but still needs a forward-compatible default for existing Docker-only instances.
...
- <RepoFile path="crates/jackin-instance/src/manifest.rs">crates/jackin-instance/src/manifest.rs</RepoFile> - instance metadata that needs backend/provider fields.
