...
samples land in the
[persistent storage layer](/research/agents/orchestration/memory/persistent-storage-layer/)'s
`usage_samples` table.
...
These read the Capsule daemon cache and `/jackin/state/usage/snapshots.db`.
...
- <RepoFile path="crates/jackin/src/console/adapter.rs">crates/jackin/src/console/adapter.rs</RepoFile> — column rendering
...
- [jackin❯ desktop Agent Hub](/research/product/desktop/agent-hub/jackin-desktop-agent-hub/) — desktop account/quota rendering home
