...
**Research state**: Design exploration for [Console resource panel](/roadmap/console-resource-panel/) (Phase 2, [Agent Orchestration Program](/research/agents/orchestration/program-research/)) — not yet implemented.
...
- Historical graphs (sparklines, rolling 5-minute window). Defer until [persistent storage layer](/research/agents/orchestration/memory/persistent-storage-layer/) ships and jackin❯ can keep a rolling sample buffer.
...
- **Sampling backend abstraction.** Sample-via-Docker today; sample-via-cgroup-files when [selectable backends](/research/platform/security/sandbox-backends/selectable-sandbox-backends/) lands. Which module owns the abstraction? Candidate: the runtime backend trait owns it, mirroring how each backend already owns mount translation.
