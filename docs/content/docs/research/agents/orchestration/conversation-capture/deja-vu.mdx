...
**Research state**: Open — research and design proposal (Phase 2, [Agent Orchestration Program](/research/agents/orchestration/program-research/))
...
- **Semantic / vector search.** V1 is index + keyword. Semantic retrieval waits until there are enough captured conversations to evaluate quality, aligned with the [persistent storage layer](/research/agents/orchestration/memory/persistent-storage-layer/) memory direction.
...
| Déjà Vu-private index vs a table in the persistent storage layer? | Prefer reusing the [persistent storage layer](/research/agents/orchestration/memory/persistent-storage-layer/) once it lands; ship a small private SQLite index first if Déjà Vu moves earlier. |
...
- [Persistent storage and workspace memory layer](/research/agents/orchestration/memory/persistent-storage-layer/) — the index/DB home Déjà Vu should reuse, and the cross-agent memory direction it complements.
