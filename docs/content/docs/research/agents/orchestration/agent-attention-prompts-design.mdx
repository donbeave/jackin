...
**Research state**: Design exploration for [Agent attention prompts](/roadmap/agent-attention-prompts/) (Phase 2, Reactive Daemon Program) — not yet implemented.
...
- **Click-to-focus adapter.** The notification would carry an action that, when activated, runs a host-side command to focus the right terminal tab. The first Desktop slice should implement Ghostty only, matching the [jackin❯ Desktop Agent Hub](/research/product/desktop/agent-hub/jackin-desktop-agent-hub/) v1 launch contract. Other terminal emulators get a notification without click focus until their adapters are deliberately added.
...
- [jackin❯ Desktop Agent Hub](/research/product/desktop/agent-hub/jackin-desktop-agent-hub/) — native macOS surface that should display attention/ready-for-review state and would own the first Ghostty click-to-focus UX.
