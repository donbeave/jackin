...
Research dossier on **AI coding-session capture**: how SpecStory works internally, what the rest of the market ships, where every agent in jackin❯'s roster stores its conversations, and what it all means for the [Déjà Vu](/research/agents/orchestration/conversation-capture/deja-vu/) design proposal. Every external claim carries a source URL; every local number carries its method. **Research conducted: 2026-07-03.** Specification: <RepoFile path="prompts/research/session-capture.md">research brief</RepoFile>.
