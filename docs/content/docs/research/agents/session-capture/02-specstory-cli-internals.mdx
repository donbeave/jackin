...
This SPI shape — locate, parse, normalize, watch, reconstruct, per provider — is the direct prior art for [Déjà Vu](/research/agents/orchestration/conversation-capture/deja-vu/)'s per-agent provider layer.
