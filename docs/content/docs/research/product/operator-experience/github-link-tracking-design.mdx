...
**Research state**: Design exploration for [GitHub link tracking & live status](/roadmap/github-link-tracking/) (Phase 2, [Agent Orchestration Program](/research/agents/orchestration/program-research/)) — not yet implemented
...
Reuse multicode's polling cadence model verbatim — they did the work to calibrate it against real GitHub usage. Use Octocrab as the client. Persist via the [persistent storage layer](/research/agents/orchestration/memory/persistent-storage-layer/).
...
- Per-instance SQLite cache (depends on [persistent storage layer](/research/agents/orchestration/memory/persistent-storage-layer/)).
...
- [Persistent storage layer](/research/agents/orchestration/memory/persistent-storage-layer/) — cache home
