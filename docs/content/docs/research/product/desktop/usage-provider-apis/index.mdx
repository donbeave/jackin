...
description: Vetted provider quota APIs, semantics, provenance, and jackin❯ coverage gaps
---
...
1. **Every provider in scope has a workable availability source, and jackin❯ already
   ...
   [01](01-current-coverage/). Per provider:
   - **Codex**: `GET {base}/wham/usage` (+ `/wham/rate-limit-reset-credits`, consume
     ...
     first-party-sourced from openai/codex — [02](02-codex-api/),
     [08](08-codex-followups/).
   - **Claude**: de-facto `GET api.anthropic.com/api/oauth/usage` (OAuth Bearer +
     ...
     fields multi-source corroborated — [03](03-claude-api/),
     [09](09-claude-followups/).
   - **Amp**: single RPC `POST ampcode.com/api/internal?userDisplayBalanceInfo`
     ...
     balances, all client-parsed — [04](04-amp-api/),
     [11](11-amp-daily-followup/).
   ...
   server/official client — [05](05-grok-api/).
...
2. **The pace/projection phrase family is CodexBar's UI vocabulary, not API data.**
   ...
   client-computed — [10](10-provenance-and-misc/),
   [07](07-runout-projection/).
...
3. **jackin❯ already ships the pace math; the run-out family has a consumer but no
   ...
   latest-only — [07](07-runout-projection/). Window anchoring is
   fixed-slot (not rolling) for Claude weekly, Grok weekly, Codex (client-observable);
   anchor on `resets_at` — [09](09-claude-followups/) Q3,
   [10](10-provenance-and-misc/) Q4.
...
4. **macOS credential blocker for Claude (Desktop-critical):** default macOS Claude
   ...
   (default store = `~/.codex/auth.json` file) — [09](09-claude-followups/) Q1,
   [08](08-codex-followups/) Q3. `claude setup-token` tokens are rejected by the
...
5. **Concrete jackin❯ gaps found** (question c):
   ...
   - Amp current parser: jackin❯ still expects the retired
     ...
     `amp usage` capture in the same `displayText` — [11](11-amp-daily-followup/).
   - Amp "Amp Free" hardcoded plan label stale: Megawatt/Gigawatt subscriptions
     ...
     [04](04-amp-api/), [10](10-provenance-and-misc/) Q3.
   - Amp reset model resolved for Amp Free: the current server format changed
     ...
     jackin❯ replenishment-derived reset is obsolete. Paid subscription
     monthly text remains unobserved — [11](11-amp-daily-followup/).
   - Grok: plan label heuristic wrong-by-design (`auth_mode=="oidc"` → "SuperGrok"
     ...
     `None` though the RPC path has period bounds — [05](05-grok-api/),
     [01](01-current-coverage/) round-2 addenda.
   - Codex `account/read` decoder tag mismatch: expects `"apikey"`, upstream sends
     ...
     [08](08-codex-followups/) Q2.
   - z.ai: observed plan field `data.level` not in jackin❯ alias list; auth header
     form (Bearer vs raw) unsettled — [06](06-zai-minimax-kimi-apis/),
     [10](10-provenance-and-misc/) Q2.
   - MiniMax: officially documented host `www.minimax.io` absent from jackin❯
     URL fan-out — [01](01-current-coverage/) round-2 addenda.
...
6. **Plan labels are mostly client-side.** Server-provided: Grok
   ...
   Amp (nothing), Kimi (web surface only) — [08](08-codex-followups/) Q1,
   [03](03-claude-api/), [09](09-claude-followups/).
...
- **Run-out producer**: Variant A (linear-from-window-start; zero new data; jumpy
  ...
  already exists) — [07](07-runout-projection/).
- **Claude on macOS**: add a Keychain reader (macOS consent UI; "Always
  ...
  (credential-export feature request) — [09](09-claude-followups/).
...
- Any official/public REST usage API for Claude subscription windows (closed "not
  ...
  Grok consumer windows (docs.x.ai is developer-API only) — [02](02-codex-api/),
  [03](03-claude-api/), [05](05-grok-api/).
- Codex per-turn `x-codex-*` headers / websocket events as a passive polling source
  (fire only on real turns) — [02](02-codex-api/).
- `account/usage/read` (historical token stats) — exactly the trend payload the
  limits-only rule forbids — [08](08-codex-followups/).
...
- Amp structured-key fallbacks (`ampFreeRemaining` etc.) and the retired
  ...
  `displayText`, whose Amp Free line is daily percentage — [04](04-amp-api/),
  [11](11-amp-daily-followup/).
- OpenUsage's spend donuts/histories as any kind of reference — forbidden surface
  (limits-only rule), noted in its screenshot — [10](10-provenance-and-misc/).
...
- Amp: post-subscription `displayText` under Megawatt/Gigawatt/linked-subscription
  ...
  now resolved by the public live capture in [11](11-amp-daily-followup/).
...
providers of this topic — [01](01-current-coverage/)).
