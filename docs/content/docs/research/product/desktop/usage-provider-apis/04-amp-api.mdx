...
description: Amp quota endpoints, credit balances, authentication, and plan semantics
---
...
Chapter [11](/research/product/desktop/usage-provider-apis/11-amp-daily-followup/) is the current authority for Amp
...
- Task premise said the Amp Free window "resets daily" — now resolved by
  ...
  jackin❯ replenishment-derived `"Resets in …"` path is obsolete for the
