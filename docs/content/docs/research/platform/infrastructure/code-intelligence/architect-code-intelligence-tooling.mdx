...
**Research state**: Open — pilot ready to implement (the-architect role only, opt-in, no jackin-core code change). Pilot stack decided: `ast-grep` + `rust-analyzer` + cargo verifiers + `fff` + `codedb`. `fff` and `codedb` ship together as complementary, first-class tools — lexical search vs structural intelligence, settled on architectural grounds, not an A/B (see [Design](#design)); `ast-index` was evaluated and deferred. See [Tool evaluation and comparison](#tool-evaluation-and-comparison) for the reasoning and the verified findings behind these choices.
...
This item also records the **evaluation** of the candidate tools (`ast-grep`, `codedb`, `fff`, `ast-index`) that produced this stack, so the choice is auditable and the deferred candidates have a documented home. The broad market sweep and the token-economics A/B protocol live in the research dossier [current retrieval-tools comparison](/research/context/tools/06-combining/); this roadmap item is the **decision record** and the implementation plan, not a duplicate of that evidence.
...
|---|---|---|---|
| [`ast-grep`](https://astgrep.com/) | Structural search / rewrite | tree-sitter AST patterns, deterministic, CLI | **Pilot — default** for syntax-shape queries and codemods |
...
### Current implementation gate (2026-08-12)

Current stable releases are fff v0.10.3 and codedb v0.2.5838. Release metadata is verified in the [current register](/research/context/tools/08-current-release-register/); installer, telemetry, hook, schema, and runtime behavior must be re-audited at those exact tags before the pilot. Use pinned artifacts and manual role-scoped registration; do not run user-scope installers inside the implementation workflow.
...
The current retrieval shortlist, exact releases, and neutral evaluation protocol live in the [current retrieval-tools comparison](/research/context/tools/06-combining/). This roadmap item remains the local decision record; current tool outcomes are not inferred from its older pilot notes.
...
**Current architecture premise (revalidate at fff v0.10.3 and codedb v0.2.5838).** `fff` is selected for lexical path/content discovery; `codedb` is selected for structural symbols, outlines, callers, dependencies, and task context. The roles are complementary by interface, but current installer behavior, schemas, memory, and task benefit remain implementation gates.
...
**Open (next).** Adopting both means two resident MCP servers, standing schema rent, and two indexes. Measure the exact current schemas and runtime state before choosing tool-search deferral or accepting the standing surface.
...
**Current evidence boundary.** Caveman v2.0.0 now includes a BSL-1.1 shrink/MCP path, so older package-shape assumptions are not current evidence. The role does not adopt that wrapper without an exact-license and current-behavior audit. Native schema deferral remains the preferred hypothesis, subject to current runtime verification.
...
- [current retrieval-tools comparison](/research/context/tools/06-combining/) — current releases, mechanisms, and evaluation boundary.
- [ast-grep](https://astgrep.com/) — structural search/rewrite CLI (no Debian apt package; installed via mise). Official Claude skill: [ast-grep/agent-skill](https://github.com/ast-grep/agent-skill) (Claude-only).
...
- [codedb](https://github.com/justrach/codedb) — current code-intelligence server; audit exact v0.2.5838 installer and telemetry behavior before use.
