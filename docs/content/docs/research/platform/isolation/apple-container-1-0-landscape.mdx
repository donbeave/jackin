...
description: Evidence on Apple Container, Sandboxy, OrbStack, smolvm, and the macOS isolation direction
---

**Date**: 2026-06-10 (mid-WWDC26, June 8–12). **Scope**: deep analysis of the Apple Container backend foundation against the upstream state of apple/container as of its 1.0.0 release (June 9, 2026), the new `container machine` feature, Apple's experimental "Sandboxy" agent sandbox, and competing runtimes (Docker Desktop, Docker Sandboxes, OrbStack, and smolvm). All upstream claims below were verified against primary sources on 2026-06-10; a three-vote adversarial pass confirmed 24 of 25 extracted claims. Items marked *(analysis)* are this document's reasoning, not verified upstream statements.
...
The [agent isolation architecture](/research/platform/security/isolation-architecture/agent-isolation-architecture/) records that every jackin❯ session currently runs inside one shared OrbStack Linux VM kernel. The 2025–2026 CVE record plus the jackin❯ `--privileged` DinD sidecar make the kernel the single shared attack surface: one compromised session can reach every other session. The four-layer answer:
...
|---|---|---|---|---|---|---|
| Cold start | VM already running; container ~instant | microVM per sandbox (sub-second class, unbenchmarked) | Machine create seconds; container instant | **Sub-second VM** (Apple, WWDC26) — PR Phase 0 budget under 5s to capsule-ready | Sub-second class + first-boot init | **Under 200 ms claimed** (self-reported) |
...
- **P2 — Warm-start path**: Phase 0 measures cold `run`→capsule-ready (under 5 s budget); add stopped→`start`→ready measurement — reconnect latency is the operator-felt number. `container machine` persistence (V2) and, longer-term, ASIF/DiskImageKit images are upstream accelerants to watch.
- **P3 — Track smolvm fork/CoW as the benchmark**: "new session from warm template in under 1 s" is the bar; if apple/container grows snapshot/clone (Sandboxy already uses rootfs `clonefile`), adopt immediately for role-image first-boot amortization.
...
- **smolvm**: fastest-moving independent; watch for (a) third-party benchmarks of its claimed sub-200 ms start and elastic memory, (b) whether VM fork + OCI-index turn into an agent-template product. Trigger for re-evaluation as primary: apple/container stagnating on memory reclaim or snapshot/clone for more than two release cycles while smolvm holds 1.x stability.
