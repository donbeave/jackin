...
**Wenyan / classical Chinese**: the original classical-Chinese rendering of "if all tests pass then commit" measured **9 tokens** versus **8 tokens** for the English phrase (net negative; original non-English text omitted). Longer rule: wenyan 32 tokens vs plain English 30 vs telegraphic English 25. Classical CJK ran **0.89–0.91 chars/token** here (at least one token per character) — FLAG: phase 0 reported "CJK ~1.47 chars/token"; that figure evidently included the ASCII/code mix of real session output, because pure classical characters are at or above one token per character on the Fable tokenizer. Either way both measurements agree characters are not tokens, and wenyan's measured savings are registral (extreme word-dropping), not tokenizer magic.
...
- **Composability:** orthogonal to output-side techniques; **dominated by deferred tool loading on the same surface** (phase-0: 11 MCP schemas = 1,420 tok loaded vs ~60 tok as deferred names — a bigger instruction-side win with zero comprehension risk; see [06 — Context architecture — what enters the window at all](/research/context/techniques/06-context-architecture/)).
...
7. The phase-0 figures leaned on throughout (thinking 54.8%, caveman-ultra 58.5%, dollar split) are **n=1 session-level measurements** pending replication (see [16 — Validation Harness: the No-Quality-Loss Proof Protocol](/research/context/techniques/16-validation-harness/)).
