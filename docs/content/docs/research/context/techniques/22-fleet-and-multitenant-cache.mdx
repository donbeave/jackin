...
# 22 — Fleet, team & multi-tenant cache economics (hosted)

Volume II area file for blind spot 4. Volume I covered the
**self-hosted** fleet tier fully in [13 — Infrastructure-level](/research/context/techniques/13-infrastructure-level/) (HiCache/LMCache across replicas) and the
excludeDynamicSections *mechanism* in [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/), but left the **hosted** (Claude
API / subscription) cross-container story thin and explicitly flagged the dynamic-section size as
unmeasured in [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/). This file builds the hosted-fleet model: when N containers share one
...
- **The server prompt cache is workspace-scoped , not machine-scoped.** N
 ...
 rules Volume I cited ([07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/) tech 7) describe Claude Code's **local file cache** (GitHub #17531), a
 different layer.
...
- **Multi-tenant boundary: org isolation is absolute; workspace is the sharing unit.** Tenant-per-org
 ...
 pooled cap** ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/research/context/techniques/19-subscription-and-quota-economics/)); a headless/SDK fleet can draw the separate API-rate credit, off
...
The original Volume I text ([07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/) tech 7 + surprising-findings: "your git state is in the cache key… worktrees never
...
- **Coverage-delta:** New. [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/) tech 7 covers the SDK flag; the **workspace-scope rule** (the
...
- **Coverage-delta:** [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/) tech 7 names the flag; **the enumerated six fields, the measured
...
- **Coverage-delta:** [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/) tech 8 covers the concurrency rule for subagent waves; applying it
...
- **Coverage-delta:** Volume I K16 in [14 — Frontier](/research/context/techniques/14-frontier-ideas/) proposes baking the optimization pack into jackin❯; the
...
- **Mechanism:** jackin❯'s launch env assembly is the chokepoint: pin every container to one workspace
 ...
 does not starve the operator's interactive cap (19).
...
- On a subscription, the fleet's binding constraint is the **single pooled cap** (19), not dollars —
...
| 4 | Dynamic block ≈111 tok (6 fields) / ≈201 with short git status; blast radius = full ~28–34k prefix | local count_tokens on a representative block + Volume I prefix size (02/07) |
...
| 10 | Subscription fleet shares one pooled cap; headless/SDK draws separate API-rate credit | support.claude.com ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/research/context/techniques/19-subscription-and-quota-economics/) ledger) |
