...
Specification: [`prompt.md`](prompt/). This dossier is one of three separated researches: techniques (this folder), the [token-optimization tools hub](/research/context/tools/) (per-tool teardowns, including the former Volume III chapters, now tools-hub chapters [19 — Code-intelligence tools](/research/context/tools/19-code-intelligence-tools/) through [25 — sem and mq](/research/context/tools/25-sem-and-mq-structural-and-markdown-retrieval/)), and the [jackin❯ Context Engine spec](/research/context/engine/).
...
The former Volume III chapters 51–57 left this dossier entirely and now live in the [token-optimization tools hub](/research/context/tools/) as chapters [19 — Code-intelligence tools](/research/context/tools/19-code-intelligence-tools/) through [25 — sem and mq](/research/context/tools/25-sem-and-mq-structural-and-markdown-retrieval/).
...
> **The Volume III per-tool chapters (19–25) now live in the dedicated [token-optimization tools](/research/context/tools/) research** as part of the three-way research split (techniques / tools / engine spec). That hub consolidates the equal-depth design teardowns of caveman, headroom, RTK, **and lean-ctx** (the integrated context runtime added in a later round), a feature has/lacks matrix, best-case-of-each, and a straight answer to whether one product can combine them all. The summaries below link to the chapters' new homes; only the runnable `tools/` instruments remain in this folder.
...
- **[19 — Code-intelligence tools: codedb, fff, CodeGraff, and alternatives](/research/context/tools/19-code-intelligence-tools/)** — deep dive comparing
...
- **[20 — Qdrant and vector databases for agent context](/research/context/tools/20-qdrant-and-vector-databases/)** — Qdrant/vector DB
...
- **[21 — Headroom and the context-compression layer (vs the caveman ecosystem)](/research/context/tools/21-headroom-and-context-compression/)** — deep dive on
  `chopratejas/headroom` (the input-side context-compression layer); the cross-tool comparison to the
  caveman ecosystem and RTK is consolidated in the dedicated [token-optimization tools](/research/context/tools/) folder. Headroom compresses what the model *reads* (tool outputs/logs/RAG/files, the 61%
...
- **[22 — Context-compression literature and market delta (2024–2026)](/research/context/tools/22-context-compression-literature-and-market/)** —
  ...
  accuracy (Squeez, AgentDiet, SWEzze, SWE-Pruner, LongCodeZip) — refuting [24 — Fresh literature & market delta (clean-room re-sweep)](/research/context/techniques/24-fresh-literature-and-market-delta/)'s "no compressor safe
...
- **[23 — Token observability and session visualization (token-optimizer and peers)](/research/context/tools/23-token-observability-and-visualization/)** — the
  *observability* layer (distinct from compression): a deep dive on `alexgreensh/token-optimizer` and a
  ...
  not a Max subscription ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/research/context/techniques/19-subscription-and-quota-economics/)). The JSONL-reading, no-proxy class is the safe measurement front-end of
  the validation harness.
- **[24 — RTK and write-time observation compression](/research/context/tools/24-rtk-and-write-time-observation-compression/)** —
  deep dive on `rtk-ai/rtk` ("Rust Token Killer") — the dossier's RTK record. The cross-tool
  comparison it originally carried now lives, expanded to four tools (adds **lean-ctx**), in the dedicated [token-optimization tools](/research/context/tools/) folder (single source of truth). RTK is the deterministic, Claude-Code-native productization of the
  cache-safe **write-time observation-compression** design point [21 — Headroom and the context-compression layer](/research/context/tools/21-headroom-and-context-compression/) (H1) and [22 — Context-compression literature and market delta](/research/context/tools/22-context-compression-literature-and-market/) named: it compresses
  ...
  per [22 — Fleet, team & multi-tenant cache economics (hosted)](/research/context/techniques/22-fleet-and-multitenant-cache/) §A), corrected to low-double-digit whole-bill, same as the caveman K1 / headroom H-K1 moves.
...
alternatives:
[19 — Code-intelligence tools: codedb, fff, CodeGraff, and alternatives](/research/context/tools/19-code-intelligence-tools/) compares `codedb`, `fff`, the
...
- **Qdrant follow-up:** [20 — Qdrant and vector databases for agent context](/research/context/tools/20-qdrant-and-vector-databases/)
