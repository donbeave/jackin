...
estimate in [03 — Prior art and market scan](/research/context/techniques/03-prior-art-and-market-scan/):267` and [12 — Provider-level features, current and beta](/research/context/techniques/12-provider-features/):165`. Every number
...
Pricing and the modeled session profile are inherited from [01 — Token Economics and Measurement](/research/context/techniques/01-economics-and-measurement/). The
...
- **Token counts** via the free OAuth `count_tokens` endpoint (`/tmp/ctimg.py`, `/tmp/ctpdf.py`)
 ...
 I, [07 — Caching exploitation](/research/context/techniques/07-caching-exploitation/)). Raw `input_tokens` includes a constant ~6–8-token user-message wrapper (Volume I
...
- **Coverage-delta:** New. Volume I's routing file (10) and tokenizer file (05) cover the **text**
...
- **Expected savings:** per full-frame screenshot, roughly 4,760 → 1,520–1,570 tokens = **~−67%** on the image
 ...
 large in **quota** ([19 — Subscription & quota economics: the metric Volume I optimized was the wrong one for a subscriber](/research/context/techniques/19-subscription-and-quota-economics/)) and in window pressure. A 25-page PDF: 78,806 → 68,804 tokens
...
- **Composability:** stacks with Volume I's tokenizer-arbitrage routing (05/10) and subagent fan-out
...
- **Coverage-delta:** New axis. Volume I's context-architecture file (06) argues "don't send it" for
