...
| Reaches shell / test / build / log output | ✗ | ✓ | ✓ | ✓ (77 pattern modules, 74 routed — count corrected in [15 — Verification pass and the extended matrix](/research/context/tools/15-verification-pass-and-extended-matrix/)) |
...
| **headroom** | Production telemetry across **50k+ sessions** (median 4.8% — historical; collection beacon since removed) + academic backing for the write-time pattern | Product percentages are vendor self-report — including the "47.5%" once cited here as independent, which was a conflation with headroom's own codebase-exploration self-benchmark (corrected in [15 — Verification pass and the extended matrix](/research/context/tools/15-verification-pass-and-extended-matrix/)); the ML stage is unbenchmarked on code quality |
...
Next: [06 — Combining](/research/context/tools/06-combining/) — whether one product can be the best of each, why lean-ctx is the real test of that question, and the layered stack that is still the answer for most.
