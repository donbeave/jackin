...
Every prior page in this hub reasoned about the four tools from their docs, README files, and one from-source build of lean-ctx. This page does the thing the [gaps page](/research/context/tools/09-gaps-open-questions-and-next-brief/) kept deferring: it **clones all four repositories and reads the actual source**, then compares the *implementations* of each feature the tools share — not "does tool X have feature Y?" (the [head-to-head matrix](/research/context/tools/05-head-to-head/) already answers that) but *"two tools both claim feature Y — whose code is better, and why?"* The answer to the hub's central question (one integrated tool, or a layered stack of specialists?) turns on exactly this, because "lean-ctx re-implements the other three" is only a real argument if its re-implementations are at least as good as the originals. They mostly are — and the reasons are in the code.
...
**Verdict.** caveman owns output uncontested, and its register is genuinely well-designed (graduated levels, verbatim guards, auto-clarity). Its real, defensible savings are roughly **half the headline** — material, but ~50% over a terse baseline, not 75% from nothing — and *no* harness anywhere yet checks that the compressed answer preserves technical fidelity, which remains the [largest standing quality question](/research/context/tools/09-gaps-open-questions-and-next-brief/) for the output side.
...
None of these change a *conclusion* — they sharpen the numbers. The language-count fixes are patched into the [head-to-head matrix](/research/context/tools/05-head-to-head/); the rest are scoped to this page's evidence.
