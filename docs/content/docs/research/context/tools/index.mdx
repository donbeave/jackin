...
- **Rank by evidence, not stars.** Three repos carry PR-inflated star counts (caveman 74.4k, RTK 63.6k, headroom 33.4k) with abnormally low watcher ratios; **lean-ctx is the youngest and least-inflated** (2.8k★, README-honest about it) but has *no* independent third-party benchmark. On evidence: caveman has a locally reproduced 58.5% output cut; headroom has production telemetry (median 4.8% whole-session — historical; the collection beacon was later removed, and the "independent 47.5%" this hub once cited was a conflation with headroom's own self-benchmark, corrected in [15 — Verification pass and the extended matrix](/research/context/tools/15-verification-pass-and-extended-matrix/)); RTK has no whole-session telemetry and no independent benchmark; lean-ctx reproduces locally here (96–99% on *code* reads, &lt;10% on prose/config) with the most honest self-instrumentation (bounce-netted, signed ledger) but is self-measured on a GPT tokenizer.
...
| Best evidence | Output ultra **−58.5% local** (token) vs 75% claim | **−66.1% self-report** mix; **median 4.8% whole-session** (50k+ sessions, historical — beacon removed; the "47.5% independent" was self-report, corrected in [15 — Verification pass and the extended matrix](/research/context/tools/15-verification-pass-and-extended-matrix/)) | **60–90% per-command, vendor-only**; **no whole-session telemetry, no independent benchmark** | **96–99% on code reads locally reproduced here**; bounce-netted signed ledger; **no independent benchmark, GPT tokenizer** |
...
No one product combines everything and gets the best of each — and lean-ctx, the one tool that genuinely *tries* to, illustrates why rather than refuting it. The three specialists win *because* they specialize: caveman by being a free prompt that touches the 5×-priced output class with zero machinery; RTK by being deterministic and cache-safe-by-construction on the one input slice that is both large and concrete (shell output), at zero ML and zero MCP cost; headroom by paying for an ML stage and a proxy to reach the input sources RTK cannot (native reads, RAG, history) and to make compression reversible. lean-ctx adds something real none of them have — a persistent, queryable code graph (the structural-retrieval lever) plus a verification layer that *proves* the saving — but to get there it carries a 64.7 MB binary, a daemon, a dashboard, several databases, a 77-tool schema, and host writes across dozens of agents: every cost the [combining page](/research/context/tools/06-combining/) predicted a monolith would inherit. So the choice is not "specialists vs the one that does it all"; it is **layered specialists vs an integrated runtime** — the lean stack when you want the cheapest cache-safe win, lean-ctx when you specifically want its code-graph/memory/verification surface and can carry the footprint. Either way, none of the four touches thinking, so none is a 10× story.

## How to read this folder

| Page | What it answers |
|---|---|
| [01 — Caveman design](/research/context/tools/01-caveman-design/) | How the output-register compressor works: the "prompt, not a program" magic, the six levels, the hooks, the cavecrew/cavemem/cavekit ecosystem. |
| [02 — Headroom design](/research/context/tools/02-headroom-design/) | How the broad input pipeline works: the ContentRouter, the typed compressors, the `kompress-base` ML stage, the live-zone cache-stabilization magic, CCR, learn, the four deployment modes. |
| [03 — RTK design](/research/context/tools/03-rtk-design/) | How the deterministic Bash-boundary compressor works: the six-phase lifecycle, the 12 strategies, the code filter, the hook modes, the reach limit. |
| [04 — lean-ctx design](/research/context/tools/04-leanctx-design/) | How the integrated context runtime works: the superset thesis, what it productizes from each of the other three, the code graph + RRF search the three lacked, CFT Φ-scoring, the verification/proof layer, and the monolith-tax footprint — with first-party build + benchmark numbers. |
| [05 — Head-to-head](/research/context/tools/05-head-to-head/) | The feature has/lacks matrix (now four-way), the internals side-by-side, and the best case for each — where each beats the others. |
| [06 — Combining](/research/context/tools/06-combining/) | Is there one product? lean-ctx as the real test of the monolith thesis, the layered stack, the published head-to-head numbers, integrated-runtime-vs-stack by project shape, and the jackin❯ adoption order. |
| [07 — Evidence and claims](/research/context/tools/07-evidence-and-claims/) | Benchmarks, what is real vs self-report, the consolidated claim graveyard, adoption-stat caveats, and the validation harness. |
| [08 — Records, ledger & unverified](/research/context/tools/08-records-ledger-and-unverified/) | The formal per-technique records (C1 / H1–H4 / R1 / L1), the full consolidated source ledger, and the unverified-claims register — vague or vendor-only numbers kept and marked "not proven" so nothing prior is lost. |
| [09 — Gaps, open questions & next brief](/research/context/tools/09-gaps-open-questions-and-next-brief/) | What this research still misses: uncompared axes, capabilities a fresh sweep surfaced, the unanswered questions, and a ready-to-paste `/goal` brief for the next round. |
...
| [11 — Extended comparison axes](/research/context/tools/11-extended-comparison-axes/) | The six axes the earlier pages never compared: security/privacy/supply-chain, project health & sustainability, interaction with Claude Code native context features, build-vs-buy vs an output-style, subscriber tasks-per-cap (which *inverts* the $-per-task order), and non-coding generality. |
| [12 — Implementation deep-dive](/research/context/tools/12-implementation-comparison/) | The first *source-level* pass: all four repos cloned and read, comparing rival **implementations** of each shared feature (shell compression, code outlining, cache-safety, memory, savings accounting, output register) with `file:line` evidence — whose code is better and why. Confirms lean-ctx out-engineers the specialists on most input features (tree-sitter vs regex, bounce-netted signed ledger vs `SUM`, measured cache-safety ratio) yet still has no output register, and logs the source-verified corrections to earlier pages. |
| [13 — Reuse surfaces and deltas](/research/context/tools/13-reuse-surfaces-and-deltas/) | The 2026-07-03 re-sweep through a new lens: which mechanisms are **embeddable in Rust** (crate / vendored corpus / ported kernel) rather than runnable as products. Release deltas since the earlier snapshots (RTK v0.43 `never_worse` guard; headroom's ⅔-executed Rust rewrite and its own ML retreat; lean-ctx's first — unflattering — independent benchmark and its recovery-first turn), the delivery-tax-vs-mechanism split, and the source-verified corrections. Feeds the [jackin❯ Context Engine spec](/research/context/engine/); the two newly-covered tools (sem, mq) are in [25 — sem and mq](/research/context/tools/25-sem-and-mq-structural-and-markdown-retrieval/). |
| [14 — The caveman ecosystem](/research/context/tools/14-caveman-ecosystem/) | Round-2 deep dive: the flagship's claims verified at source (they hold), plus first equal-depth analyses of **cavemem** (refuted as shipped: 33–36% measured vs 75% marketed, savings refunded at read time, retrieval broken by its own vocabulary mismatch, unmaintained) and **caveman-code** (not a "variant" — a full pi-code-fork coding agent with a 4-layer compression stack and the family's only real, caveated agentic benchmark: 1.93× vs Codex at −1/−2 tasks). |
| [15 — Verification pass and extended matrix](/research/context/tools/15-verification-pass-and-extended-matrix/) | Claim-by-claim source verification of the RTK/headroom/lean-ctx pages with the consolidated corrections (the "47.5% independent headroom measurement" was a conflation with its own self-benchmark; lean-ctx counts settled at 77 patterns/13 modes/75.9 MiB; RTK at 77 commands/63 TOML/86 rules with consent-gated telemetry), RTK's 12-strategy taxonomy captured verbatim, codedb folded into a seven-tool extended matrix, and the best-part-from-each selection re-derived for the [engine spec](/research/context/engine/). |
| [16 — Global comparison](/research/context/tools/16-global-comparison/) | **The one-page summary of everything**: all eleven tools (caveman, cavemem, caveman-code, RTK, headroom, lean-ctx, codedb, sem, fff, mq, pxpipe) on one screen — identity strip, the full feature has/lacks matrix across output/observation/retrieval/memory/measurement/safety, the best-implementation-per-feature selection table the engine spec draws from, and the engine-only rows no tool has. Start here for the bird's-eye view; every cell links back to the page that proves it. |
| [17 — pxpipe design](/research/context/tools/17-pxpipe-design/) | How the input-side prompt-rewrite proxy works: "optical context compression" that re-encodes stable input regions into a cheaper image modality (text-to-image token arbitrage) — a new category for the hub, being neither an output-register prompt, nor a write-time observation filter, nor a whole-prompt text compressor. |
| [18 — Latest-versions refresh](/research/context/tools/18-latest-versions-refresh/) | The 2026-07-05 per-tool latest-version verification of the whole hub: the load-bearing deltas since each tool's last sweep, plus the new-tools sweep that surfaced pxpipe. |

The per-tool deep-dive chapters below originated as Volume III of the [token-optimization techniques dossier](/research/context/techniques/) and moved here when the researches were fully separated (techniques / tools / engine spec); formerly dossier chapters 51–57, they were renumbered 19–25 when the researches were made strictly sequential on 2026-07-05.

| Deep-dive chapter | What it covers |
|---|---|
| [19 — Code-intelligence tools](/research/context/tools/19-code-intelligence-tools/) | codedb, fff, CodeGraff, and the alternatives sweep (Serena, Code Context Engine, Claude Context, Sourcegraph, Qodo, Augment, ast-grep) — the retrieval-instead-of-reading tool class, with local outline/symbol-search measurements. |
| [20 — Qdrant and vector databases](/research/context/tools/20-qdrant-and-vector-databases/) | Vector DBs as an optional semantic-memory/RAG backend; why none is a default code-navigation layer; the docs-corpus case. |
| [21 — Headroom deep dive](/research/context/tools/21-headroom-and-context-compression/) | The full headroom record: live-zone cache safety, typed compressors, CCR, H1–H4 technique records, benchmarks, source ledger. |
| [22 — Compression literature and market](/research/context/tools/22-context-compression-literature-and-market/) | The compression-market sweep + 2024–2026 literature, and the cache-safety classification of every compression move. |
| [23 — Token observability and visualization](/research/context/tools/23-token-observability-and-visualization/) | token-optimizer and the JSONL/session-visualization tool class — the measurement front-end of the validation harness. |
| [24 — RTK deep dive](/research/context/tools/24-rtk-and-write-time-observation-compression/) | The full RTK record: write-time observation compression, the R1 record, benchmarks, adoption guardrails, source ledger. |
| [25 — sem and mq](/research/context/tools/25-sem-and-mq-structural-and-markdown-retrieval/) | Entity-level retrieval (sem, the attention ledger) and structural markdown queries (mq) — the docs/prose-bucket lever. |
