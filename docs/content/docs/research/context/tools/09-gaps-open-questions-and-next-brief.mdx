...
These corrections are folded into pages [03 — RTK: design teardown](/research/context/tools/03-rtk-design/) and [07 — Evidence, benchmarks, and the claim graveyard](/research/context/tools/08-records-ledger-and-unverified/); they are listed here too so the "what changed" is auditable.

## B. Dimensions the hub has not compared (missed axes)

> **Update (2026-06-20):** all six axes below are now compared side-by-side in [10 — Extended comparison axes](/research/context/tools/11-extended-comparison-axes/). They are kept listed here as the record of *what was missing*; what still remains is the per-axis *token delta* under a controlled run (harness backlog — see [10 — First-party measurements (this repo + a built lean-ctx)](/research/context/tools/10-first-party-measurements/)).
...
5. **Subscriber (tasks-per-cap) economics per tool.** The dossier has the metric ([19 — Code-intelligence tools](/research/context/tools/19-code-intelligence-tools/)); the hub never asks *which of the three most extends a Max-plan 5-hour window* — a different ranking from $-per-task, and the metric most operators actually feel.
...
This round ran the first first-party measurement ([10 — First-party measurements (this repo + a built lean-ctx)](/research/context/tools/10-first-party-measurements/)) and added the six axes ([10 — First-party measurements (this repo + a built lean-ctx)](/research/context/tools/11-extended-comparison-axes/)). Resolution of §C's questions:

| Open question (§C) | Status | Where |
|---|---|---|
| Which tool wins per dollar on our workload? | **OPEN (partial)** — the full 6-arm A/B is not self-runnable in one session; local-transcript measurement done | [10 — First-party measurements (this repo + a built lean-ctx)](/research/context/tools/10-first-party-measurements/) |
| Share of observation tokens that flow through Bash? | **ANSWERED** — 16.5% on this (docs/research) workload; native `Read` 76.2% (RTK-blind). Workload-specific | [10 — First-party measurements (this repo + a built lean-ctx)](/research/context/tools/10-first-party-measurements/) |
| Share of output that is thinking? | **PARTIAL** — thinking is redacted in JSONL; exact split needs `count_tokens`; dossier n=1 54.8% stands; visible prose is empirically tiny | [10 — First-party measurements (this repo + a built lean-ctx)](/research/context/tools/10-first-party-measurements/) |
...
| Where does each tool's data go (threat model)? | **ANSWERED** — data-egress / supply-chain / injection compared | [11 §a](/research/context/tools/11-extended-comparison-axes/) |
| Is the "90%+ / 30 min → 3 hr" stack real in dollars on a controlled run? | **PARTIAL** — it is a tasks-per-cap/occupancy win, not a dollar cut; the controlled $ A/B is still open | [11 §e](/research/context/tools/11-extended-comparison-axes/) |
...
This round added the fourth tool, **lean-ctx**, after recognizing it was missing entirely from the three-way comparison. New artifacts: the [04 — lean-ctx design](/research/context/tools/04-leanctx-design/) teardown; a from-source build + benchmark on [page 10](/research/context/tools/10-first-party-measurements/) (the first time a tool in this hub was compiled and run, not just read about); lean-ctx columns/rows across every matrix (index, [05 — Head-to-head: where each wins, what each lacks](/research/context/tools/05-head-to-head/), [07 — Evidence, benchmarks, and the claim graveyard](/research/context/tools/07-evidence-and-claims/), [11 — Extended comparison axes](/research/context/tools/11-extended-comparison-axes/)); an L1 record + ledger + unverified rows on [page 08](/research/context/tools/08-records-ledger-and-unverified/); and a reframed [combining page](/research/context/tools/06-combining/) treating lean-ctx as the live test of the "is there one product?" thesis. The folder was also renamed `caveman-headroom-rtk` → `token-optimization-tools` so future tools join without another rename. **Still open:** the controlled multi-arm A/B (now including a lean-ctx arm) and the lean-ctx-specific questions in §C — the integrated-runtime-vs-stack verdict remains *reasoned*, not *measured*.
...
Back to the [overview](/research/context/tools/), or the [evidence and claims](/research/context/tools/07-evidence-and-claims/) page.
