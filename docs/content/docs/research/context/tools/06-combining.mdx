...
The combination measured as additive (1.33B + 0.19B ≈ 1.52B), with headroom's ~200–500-token proxy-metadata cost the only overhead — confirming the two input specialists compose at different interception points. lean-ctx was not in this measurement (too young); whether its single-runtime approach beats the two-tool stack on the same workload is exactly the [open question](/research/context/tools/09-gaps-open-questions-and-next-brief/) the harness must answer.
