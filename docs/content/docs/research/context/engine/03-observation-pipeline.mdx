...
The write-time layer: shrink every observation **once, at ingestion, before it is first cached** — the only cache-safe input-compression design point on hosted Claude ([tools hub 02 — Headroom: design teardown](/research/context/tools/02-headroom-design/)/[tools hub 03 — RTK: design teardown](/research/context/tools/03-rtk-design/); [tools hub 21 — Headroom and the context-compression layer (vs the caveman ecosystem)](/research/context/tools/21-headroom-and-context-compression/)/[tools hub 24 — RTK and write-time observation compression](/research/context/tools/24-rtk-and-write-time-observation-compression/)). This layer targets the 61%-of-dollars cache bucket and is where the engine composes RTK's breadth, lean-ctx's safety, and headroom's typed routing into one pipeline.
...
The [verification pass](/research/context/tools/15-verification-pass-and-extended-matrix/) surfaced battle-tested micro-contracts this pipeline adopts wholesale:
