...
**Current state**: credential, secret, supply-chain, and control-surface work tracked separately so it remains independently reviewable.

## Why this is separate

This page owns jackin❯ code-health work whose subject matter is sensitive-boundary: credential flow, redaction, host/control boundaries, workflow and dependency hardening, and supply-chain provenance. The [sensitive-boundary study](/research/platform/security/sensitive-boundary-code-health-study/) records why this work remains isolated from routine code-health execution. Routine linting, module extraction, tests, docs freshness, performance budgets, and telemetry shape stay outside this item.
...
2. **Telemetry and error redaction.** Golden/property tests feed canary credentials through launch/auth paths and assert credential-shaped strings never appear in operator notices, error chains, or OTLP-exported logs, traces, and metrics. Local diagnostics JSONL and the legacy `clog!`/`cdebug!` emission surfaces have been removed; the [application observability contract](/reference/runtime/diagnostics/) defines the governed OTLP boundary without adding another sink to secure.
...
1. **Redaction before every boundary.** One shared redaction layer ahead of operator output/error rendering and OTLP provider submission: key-pattern and value-pattern scrubbing plus typed summaries instead of raw Docker inspect JSON, environment arrays, or generated Dockerfiles. Product telemetry has no file sink or large-payload artifact escape hatch; oversized or prohibited signals follow the rejection/truncation contract in the [application observability reference](/reference/runtime/diagnostics/). The telemetry-redaction policy tests above are the gate.
...
The detailed executor plans that include sensitive examples live under <RepoFile path="security-review/README.md">security-review/README.md</RepoFile>. Keep those isolated from ordinary roadmap work so a code-health executor can work without loading them.
