...
**Current state**: `doctor`, preflight, `status`/`ps`, JSON output on core read-only commands, `load --dry-run`, and structured `JackinError` rendering shipped; first-run wizard/onboarding remains deferred
...
- [Application observability](/reference/runtime/diagnostics/) — current-invocation correlation and backend-owned telemetry for operator failures.
