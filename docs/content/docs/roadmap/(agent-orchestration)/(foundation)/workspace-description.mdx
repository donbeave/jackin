...
**Current state**: no persisted `description` field exists on `WorkspaceConfig` or `WorkspaceEdit` yet; small, well-scoped consistency work across schema, CLI, and console
...
- Research: [Workspace description field: prior art and shape](/research/agents/orchestration/workspace-description-field/) — multicode comparison, config/CLI/console sketch, open design questions.
