...
**Current state**: per-mount Git isolation (`shared | worktree | clone`) is shipped; storage-kind modes (`tmpfs`, per-instance `ephemeral` volumes) remain an unimplemented design proposal
...
- Research: [Ephemeral mount modes — design proposal](/research/platform/isolation/ephemeral-mount-modes/) (multicode comparison, schema/backend design, open questions)
- [Agent Orchestration Program](/research/agents/orchestration/program-research/)
- [Selectable sandbox backends](/research/platform/security/sandbox-backends/selectable-sandbox-backends/) — cross-backend translation home for a future Apple-container implementation
