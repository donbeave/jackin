...
5. **Non-Docker backend parity.** No egress enforcement exists yet for OrbStack, smolvm, Kubernetes, or SSH-remote backends; a host-side proxy design for microVM-style backends and a Kubernetes/remote policy mapping remain unstarted. See [Network egress policy design](/research/platform/security/network-egress/network-egress-policy-design/) for the backend strategy and external references behind this.

## Related work

- [Selectable sandbox backends](/research/platform/security/sandbox-backends/selectable-sandbox-backends/) — Docker Sandboxes and microVM comparison this item's backend parity depends on
...
- [Network egress policy design](/research/platform/security/network-egress/network-egress-policy-design/) — design rationale, enforcement-quality vocabulary, and source materials
