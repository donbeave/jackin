...
**Current state**: `jackin-exec`, the credential picker, and MCP execution primitives exist as reusable building blocks; the operator-configured `[[tool]]` surface itself has no config parsing, no keybinding registration, and no firing/substitution code yet (Phase 2, [Agent Orchestration Program](/research/agents/orchestration/program-research/))
...
- Research: [Custom Operator Tools: Design Exploration](/research/product/operator-experience/host-affordances/custom-operator-tools-design/) — multicode's `[[tool]]` design, recommended config shape, and open questions.
