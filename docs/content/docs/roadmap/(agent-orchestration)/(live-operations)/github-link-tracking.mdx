...
**Current state**: Capsule already shows focused-instance PR context (title, CI URL, HEAD-aware invalidation); durable cross-instance GitHub link tracking, SQLite cache, fetcher loop, and console-wide live status have not started (Phase 2, [Agent Orchestration Program](/research/agents/orchestration/program-research/))
...
1. **Fetcher + cache.** Add an Octocrab-based fetcher with a per-state refresh cadence and a per-instance SQLite cache (depends on the not-yet-landed [persistent storage layer](/research/agents/orchestration/memory/persistent-storage-layer/)).
...
See the [design exploration](/research/product/operator-experience/github-link-tracking-design/) for the multicode-inspired cadence table, schema, and open design questions (per-instance vs shared cache, token scope guidance, GitHub App alternative).
...
- [Design exploration: GitHub link tracking](/research/product/operator-experience/github-link-tracking-design/) — cadence table, schema, fetcher design, open questions.
- [Agent tag protocol](/roadmap/agent-tag-protocol/) — input this feature consumes.
- [Persistent storage layer](/research/agents/orchestration/memory/persistent-storage-layer/) — cache home, not yet built.
