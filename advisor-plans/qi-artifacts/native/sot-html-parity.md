# HTML composition vs native structure

| HTML surface | Native | Parity |
|--------------|--------|--------|
| Status ≤3 burn-first dual-stack | statusBarGlanceRows + StatusItemRendering | **match** (fixture) |
| Overview\|Providers | PopoverTabGrid | **match** |
| mode-overview multi-account | PopoverOverviewTab | **match** |
| OV-5 calendar when known | overviewResetDisplay | **match** selected/glance |
| Providers strip + detail | TabGrid + PopoverProviderTab | **match** |
| Open Usage CTA | PopoverFooter | **match** |
| Usage glass sidebar + solid | craft roles match; window-ID evidence only | **BLOCKED** Dark glass / active state |
| Structural Usage bar | nest/detail/overview/toolbar | **accepted** |
| Meters 3-status; brand plates | severityTint / brandChrome | **match** |
| No spend/price UI | parity matrix | **match** |

## Token map
`--jk` → Color.jackinPhosphor · `--glass` → GlassFallbacks · status bands → severityTint

## SB-5 residual
Bar priority = rank order (SB-13/17), not colored chip fills (FB1-6 template mono). SB-P4 OPEN.
