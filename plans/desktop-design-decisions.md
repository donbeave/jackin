...
| FB1-64 | **Open usage page** on provider detail: one control opens the provider’s official usage console in the browser. URLs locked in `OFFICIAL_USAGE_URLS.md` / `ProviderUsageLinks`. **Limit Reset Credits** show Available + Next expires (Rust `detail_label` segments), not a bare hero | CONFIRMED |
| FB1-65 | **Native macOS toolbar craft:** Usage window uses real **NSToolbar** (unified titlebar) via `NSHostingController` + SwiftUI `.toolbar` / `.windowToolbarStyle(.unified)` — never a custom floating capsule chrome strip. Status items are **NSStatusItem** menu-bar toolbar (template mono dual stack, full system strip) — not glass chips | CONFIRMED |
| FB1-66 | **System menu bar is display chrome, not window chrome:** craft mocks follow HIG *The menu bar* — leading  (+ app menus when `.regular`), trailing extras (our status items) then Control Center + clock. Status scene = `.accessory`; Usage scene = `.regular` with window on wallpaper under the bar. Refs: `MACOS_CHROME_REFERENCES.md` | CONFIRMED |
...
| FB1-48 | **Provider vs account are distinct systems:** provider = primary full-fill + brand plate + **name only** (multi-account caption); account nest under **selected** provider carries glance **% + mini meter**; multi = radio; single = quiet row; **no** detail chip-strip dupe; never glance progress on the provider row | CONFIRMED |
