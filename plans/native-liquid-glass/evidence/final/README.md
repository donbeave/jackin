...
Status: **D-005 core, accessibility, and Clear evidence complete; Tinted replacement pending**

The fail-closed matrix rebuilt and verified the canonical application from clean pushed source `6bdb98924f51607442f730f5242f3d1d041a2c5b`. All current core and accessibility sidecars record that source and executable SHA-256 `58b271ece95f28e81e9682fa0e09418bc4445b266c09b323241bc2bfc92f2569`. The validator reports `core=36 accessibility=8 failures=0` after checking image hashes, source/application provenance, window layers, titles, on-screen state, activation/key state, requested setting values, and restoration receipts.
...
At the recorded source, 69 Swift package tests plus two generated-baseline tests pass. The real-host suite passes 15/15 with zero application runtime warnings, including centered-title geometry and all three accessibility audits. The UI-only `--ui-test` launch flag isolates XCTest windows from unrelated full-screen hosts; capture scripts cannot request it, and every final Usage capture remains a real layer-0 window.
...
Reduce Transparency, Increase Contrast, Reduce Motion, and Differentiate Without Color have branch-head real-host captures under [`accessibility/`](accessibility/). [`settings-accessibility-before.txt`](settings-accessibility-before.txt) and all four `settings-accessibility-after-*.txt` receipts share SHA-256 `92d340a5d72d58db9ed095fc032ce328fc183cb7bf5b7bd1caa541d72c552d2b`, proving byte-identical restoration. [The historical blocker record](blocked-accessibility-captures.md) explains the earlier host failure. [`appearance/`](appearance/) contains the operator-confirmed A08 Clear receipt and current D-005 captures; the A09 Tinted receipt remains valid operator evidence but its two app captures still await D-005 replacement. macOS exposes no public read API, so no placeholder, guessed preference state, or offscreen renderer substitutes for native material proof.
